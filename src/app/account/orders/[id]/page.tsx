...
const STATUS_COLOURS: Record<string, string> = {
  ...
  cancelled: "text-brand-purple-dark/80 bg-white/5",
};
...
export default function AccountOrderPage() {
  ...
  if (loading) {
    return <main className="max-w-2xl mx-auto px-6 py-16 text-[12px] text-brand-purple-dark/80">Loading order…</main>;
  }

  if (error || !order) {
    return (
      <main className="max-w-2xl mx-auto px-6 py-16 text-center space-y-3">
        <h1 className="font-display text-2xl text-brand-purple-dark">Order not found</h1>
  ...
  const statusColour = STATUS_COLOURS[statusKey] ?? "text-brand-purple-dark/80 bg-white/5";

  return (
    <main className="max-w-2xl mx-auto px-6 py-10 space-y-6">
      <header>
        <p className="text-[10px] tracking-[0.32em] uppercase text-brand-orange mb-1">Order</p>
        <h1 className="font-display text-3xl text-brand-purple-dark">{order.id}</h1>
      ...
      <section className="rounded-xl border border-pink-200 bg-white/[0.02] divide-y divide-white/5">
        {order.items.map((item, i) => (
          <div key={i} className="px-4 py-3 flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              <p className="text-[13px] text-brand-purple-dark truncate">{item.name}</p>
              {item.description && (
                <p className="text-[11px] text-brand-purple-dark/80 mt-0.5">{item.description}</p>
              )}
              <p className="text-[11px] text-brand-purple-dark/80 mt-0.5">
              ...
              {item.licenseKey && (
                <p className="text-[10px] font-mono text-brand-purple-dark/80 mt-1.5">
                  License: <span className="select-all">{item.licenseKey}</span>
                </p>
              )}
            </div>
            <p className="text-[13px] text-brand-purple-dark tabular-nums">
              {formatMoney(item.unitAmount * item.quantity, item.currency)}
            </p>
          </div>
        ))}
        <div className="px-4 py-3 flex items-center justify-between">
          <p className="text-[12px] tracking-wider uppercase text-brand-purple-dark/80">Total</p>
          <p className="text-[15px] font-display text-brand-purple-dark tabular-nums">
      ...
      {order.shippingAddress && (
        <section className="rounded-xl border border-pink-200 bg-white/[0.02] p-4">
          <h2 className="text-[10px] tracking-[0.28em] uppercase text-brand-purple-dark/80 mb-2">Shipping to</h2>
          ...
          {order.trackingNumber && (
            <p className="text-[11px] text-brand-purple-dark/80 mt-2">
      ...
      <footer className="text-[11px] text-brand-purple-dark/80 text-center pt-4 border-t border-pink-200/50">
