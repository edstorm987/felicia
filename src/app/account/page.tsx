...
function AccountContent() {
  ...
  if (!hydrated) {
    return (
      <>
        <Navbar />
        <main className="w-full pt-32 min-h-screen bg-pink-50" />
        <Footer />
      </>
    );
  }
...
function AuthForm({ onLogin }: { onLogin: (s: Session) => void }) {
  ...
  const formInner = (
    <div className="w-full max-w-md mx-auto px-6 sm:px-8 py-14 sm:py-20">
      ...
      <h1 className="font-display font-bold text-brand-purple-dark text-3xl sm:text-4xl xl:text-5xl leading-tight mb-3"
        style={cfg.textColor ? { color: cfg.textColor } : undefined}>
        {headline}
      </h1>
      <p className="text-brand-purple-dark/80 leading-relaxed mb-8"
      ...
      {cfg.enableSignup && (
        <div className="grid grid-cols-2 mb-6 rounded-xl bg-white p-1 border border-pink-200/50">
      ...
      <div className="flex items-center gap-3 my-6">
        ...
        <span className="text-[10px] tracking-[0.28em] uppercase text-brand-purple-dark/80">or use email</span>
        <div className="flex-1 h-px bg-white/8" />
      </div>
      ...
      <p className="text-xs text-brand-purple-dark/80 mt-6 text-center">
        By continuing you agree to our{" "}
        <Link href="/privacy" className="text-brand-purple-dark/80 hover:underline">Privacy Policy</Link>.
      </p>
  ...
  return (
    <>
      {customCss}
      <Navbar />
      <main className="w-full pt-20 sm:pt-24 bg-pink-50 min-h-screen"
...
function Dashboard({ user, initialTab, onLogout, onRefresh }: {
  ...
  return (
    <>
      <Navbar />
      <main className="w-full min-h-screen bg-pink-50 pt-16 sm:pt-[4.5rem] lg:pt-20 2xl:pt-24">
        ...
        <section className="w-full pt-8 pb-10 sm:pt-10 sm:pb-12 bg-pink-50/50 border-b border-pink-200/50">
          <div className="max-w-5xl mx-auto px-6 sm:px-10 lg:px-12">
            <div className="flex items-center justify-between gap-4 flex-wrap">
              <div>
                ...
                <h1 className="font-display font-bold text-brand-purple-dark text-3xl sm:text-4xl">
                  Welcome back, {displayName}
                </h1>
                <p className="text-brand-purple-dark/80 text-sm mt-1">
                  {user.email}
                  {user.provider === "google" && <span className="ml-2 text-brand-purple-dark/80">· via Google</span>}
                </p>
              </div>
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <button
                  onClick={onLogout}
                  className="text-xs tracking-wide text-brand-purple-dark/80 hover:text-brand-purple-dark border border-pink-200 px-4 py-2 rounded-lg transition-colors"
                >
                  Log out
                </button>
              </div>
              ...
              {([
                ...
                <button
                  ...
                  className={`px-5 py-2.5 rounded-lg text-sm font-medium transition-all ${
                    tab === t.id
                      ? "bg-pink-50/50 border border-pink-200 text-brand-purple-dark"
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between mb-2">
        <h2 className="font-display font-bold text-brand-purple-dark text-xl sm:text-2xl">Your Orders</h2>
        <span className="text-brand-purple-dark/80 text-sm">{MOCK_ORDERS.length} orders</span>
      </div>

      <p className="text-[11px] text-brand-purple-dark/80 -mt-1 mb-2">
        Showing demo orders. Once Shopify is wired, this list comes straight from <code className="text-brand-purple-dark/80">customer.orders</code>.
      </p>

      {MOCK_ORDERS.map(order => {
        const open = expanded === order.id;
        return (
          <div key={order.id} className="rounded-2xl bg-white border border-pink-200/50 overflow-hidden">
            <button
            ...
            >
              <div className="flex items-center gap-4 flex-wrap">
                <div>
                  <p className="text-sm font-medium text-brand-purple-dark">{order.id}</p>
                  <p className="text-xs text-brand-purple-dark/80 mt-0.5">{order.date}</p>
                </div>
              ...
              <div className="flex items-center gap-4 shrink-0">
                ...
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
                  className={`text-brand-purple-dark/80 transition-transform duration-200 ${open ? "rotate-180" : ""}`}>
            ...
            {open && (
              <div className="border-t border-pink-200/50 px-5 sm:px-6 py-5 space-y-4">
                <div className="flex flex-col gap-2">
                  {order.items.map((item, i) => (
                    <div key={i} className="flex items-center justify-between gap-4">
                      <p className="text-sm text-brand-purple-dark/80">{item.name} {item.qty > 1 && <span className="text-brand-purple-dark/80">×{item.qty}</span>}</p>
                      <p className="text-sm text-brand-purple-dark shrink-0">£{item.price.toFixed(2)}</p>
                    </div>
                  ))}
                </div>
                <div className="pt-3 border-t border-pink-200/50 flex items-center justify-between">
                  <div>
                    <p className="text-[11px] tracking-widest uppercase text-brand-purple-dark/80 mb-1">Tracking</p>
                    <p className="text-sm text-brand-purple-dark/80 font-mono">{order.tracking}</p>
                  </div>
                  <div className="flex gap-2">
                    <Link
                      href="/products"
                      className="text-xs px-4 py-2 rounded-lg border border-pink-200 text-brand-purple-dark/80 hover:text-brand-purple-dark hover:border-white/25 transition-colors"
                    >
                      Buy again
                    </Link>
                    <a
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          <div key={s.label} className="flex flex-col p-5 rounded-2xl bg-white border border-pink-200/50">
            <span className="font-display text-2xl sm:text-3xl font-bold text-brand-amber">{s.value}</span>
            <span className="text-[11px] tracking-wide text-brand-purple-dark/80 mt-1">{s.label}</span>
          </div>
        ))}
      </div>

      {/* Tier */}
      <div className="p-6 rounded-2xl bg-white border border-pink-200/50">
        <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
          <div>
            <p className="text-[11px] tracking-[0.2em] uppercase text-brand-purple-dark/80 mb-1">Current tier</p>
            <p className="font-display font-bold text-brand-purple-dark text-xl">{currentTier.label}</p>
            <p className="text-brand-orange text-sm mt-0.5">{currentTier.reward}</p>
          </div>
          {nextTier && (
            <div className="text-right">
              <p className="text-[11px] tracking-[0.2em] uppercase text-brand-purple-dark/80 mb-1">Next tier</p>
              <p className="text-brand-purple-dark text-sm font-medium">{nextTier.label}</p>
              <p className="text-brand-purple-dark/80 text-xs mt-0.5">{nextTier.min - totalReferrals} more referral{nextTier.min - totalReferrals !== 1 ? "s" : ""}</p>
            </div>
          )}
        </div>
      ...
      <div className="p-6 rounded-2xl bg-white border border-pink-200/50">
        <p className="text-[11px] tracking-[0.2em] uppercase text-brand-purple-dark/80 mb-2">Your referral discount code</p>
        <p className="text-sm text-brand-purple-dark/80 mb-4">
        ...
        <div className="flex gap-2 mb-4">
          <div className="flex-1 bg-pink-50 border border-pink-200 rounded-xl px-5 py-4 text-brand-amber text-2xl font-display font-bold tracking-[0.18em] text-center">
            {code.code}
          </div>
          <button
            onClick={copy}
        ...
        <p className="text-[11px] text-brand-purple-dark/80 mt-4">
      ...
      {/* Referral history */}
      <div>
        <h3 className="font-display font-bold text-brand-purple-dark text-lg mb-4">Referral History</h3>
        <div className="rounded-2xl bg-white border border-pink-200/50 overflow-hidden">
          <div className="grid grid-cols-4 px-5 py-3 border-b border-pink-200/50 text-[11px] tracking-[0.18em] uppercase text-brand-purple-dark/80">
            <span>Friend</span>
            <span>Date</span>
            <span>Status</span>
          ...
          {MOCK_REFERRALS.map((r, i) => (
            <div key={i} className="grid grid-cols-4 px-5 py-4 border-b border-white/[0.04] last:border-0 items-center">
              <span className="text-sm text-brand-purple-dark">{r.name}</span>
              <span className="text-sm text-brand-purple-dark/80">{r.date}</span>
              <StatusBadge status={r.status} />
              <span className={`text-sm text-right font-medium ${r.earned > 0 ? "text-brand-amber" : "text-brand-purple-dark/80"}`}>
                {r.earned > 0 ? `+£${r.earned}` : "—"}
              </span>
            </div>
          ))}
      ...
      <div className="p-5 rounded-2xl border border-brand-purple/20 bg-brand-purple-muted/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <p className="text-sm font-medium text-brand-purple-dark">Next payout: <span className="text-brand-amber">1 May 2026</span></p>
          <p className="text-xs text-brand-purple-dark/80 mt-1">Earnings paid monthly via bank transfer or gift credit. Email hello@luvandker.com to set up bank transfer.</p>
        </div>
        <a
...
function StatusBadge({ status }: { status: string }) {
  const colours: Record<string, string> = {
    ...
    Pending:    "bg-white/8 text-brand-purple-dark/80",
  };
  return (
    <span className={`text-[11px] tracking-wide px-2.5 py-1 rounded-full ${colours[status] ?? "bg-white/8 text-brand-purple-dark/80"}`}>
      {status}
    </span>
  );
}
...
function Field({ label, right, children }: { label: string; right?: React.ReactNode; children: React.ReactNode }) {
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <label className="block text-[11px] tracking-[0.22em] uppercase text-brand-purple-dark/80">{label}</label>
        {right}
      </div>
      {children}
    </div>
...
const inputCls = "w-full bg-white border border-pink-200 rounded-xl px-4 py-3.5 text-sm text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-brand-orange/40 transition-colors";
