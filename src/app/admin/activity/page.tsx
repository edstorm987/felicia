...
const CATEGORY_COLOURS: Record<ActivityCategory, string> = {
  ...
  settings:  "bg-white/10 text-brand-purple-dark/80",
...
export default function AdminActivityPage() {
  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 max-w-5xl space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-2">
            <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Activity log</h1>
            <Tip
              id="activity.header"
        ...
        <button
          ...
          className="text-xs px-3 py-1.5 rounded-lg border border-pink-200 text-brand-purple-dark/80 hover:text-brand-purple-dark disabled:opacity-40 disabled:cursor-not-allowed"
        >
          Clear log
        </button>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {actors.length > 1 && (
          <select
            ...
            className="bg-white border border-pink-200 rounded-lg px-3 py-2 text-sm text-brand-purple-dark focus:outline-none"
          >
      ...
      {/* Empty state */}
      {entries.length === 0 && (
        <div className="rounded-2xl border border-dashed border-pink-200 px-6 py-16 text-center">
          <p className="text-brand-purple-dark/80 text-sm">No activity yet.</p>
          <p className="text-brand-purple-dark/80 text-xs mt-1">
      ...
      {entries.length > 0 && visible.length === 0 && (
        <div className="text-center py-10 text-brand-purple-dark/80 text-sm">
            ...
            {group.items.map(e => (
              <div key={e.id} className="flex items-start gap-3 px-5 py-3.5 hover:bg-white/[0.02]">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-brand-purple-dark leading-snug">
                    {e.resourceLink ? (
                      <Link href={e.resourceLink} className="hover:underline">{e.action}</Link>
                    ) : e.action}
                  </p>
                  <p className="text-[11px] text-brand-purple-dark/80 mt-0.5">
                    {e.actorName} <span className="text-brand-purple-dark/80">· {e.actorEmail}</span>
                  </p>
                </div>
                <span className="text-[11px] text-brand-purple-dark/80 shrink-0 mt-1" title={new Date(e.ts).toLocaleString()}>
                  {relativeTime(e.ts)}
                </span>
              </div>
            ))}
