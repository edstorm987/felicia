...
function PayoutsPageInner() {
  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 max-w-5xl space-y-6">
      <Link href="/admin/affiliates" className="text-xs text-brand-purple-dark/80 hover:text-brand-purple-dark inline-block">
        ← Affiliate program
      </Link>
      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Payouts</h1>
      ...
      {loading ? (
        ...
        <>
          ...
          <section>
            <h2 className="text-[11px] tracking-[0.22em] uppercase text-brand-purple-dark/80 mb-2">
                    ...
                    <li key={a.id} className="px-4 py-3 flex flex-wrap items-center gap-3">
                      <div className="flex-1 min-w-[12rem]">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="text-[13px] text-brand-purple-dark">{a.name}</span>
                      ...
                      <div className="text-right">
                        <div className={`text-[14px] font-display tabular-nums ${owed > 0 ? "text-brand-amber" : "text-brand-purple-dark/80"}`}>
                          {fmt(owed)}
                        </div>
                        <div className="text-[10px] uppercase tracking-[0.18em] text-brand-purple-dark/80">
                ...
                {payouts.map(p => {
                  ...
                  return (
                    <li key={p.id} className="px-4 py-3 flex flex-wrap items-center gap-3">
                      <div className="flex-1 min-w-[12rem]">
                        <div className="text-[13px] text-brand-purple-dark">
                          {aff?.name ?? <em className="text-brand-purple-dark/80">Unknown affiliate ({p.affiliateId})</em>}
                        </div>
                        <p className="text-[11px] text-brand-purple-dark/80">
                      ...
                      <div className="text-right">
                        <div className="text-[14px] text-brand-purple-dark font-display tabular-nums">
                          {fmt(p.amount, p.currency)}
                        </div>
                        <div className="text-[10px] text-brand-purple-dark/80 tabular-nums">
...
function PayoutModal({ orgId, affiliate, onClose, onSuccess }: PayoutModalProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <h2 id="payout-title" className="font-display text-xl text-brand-purple-dark">Record payout</h2>
        ...
        >
          <input
            ...
            required
            className="w-full bg-white/5 border border-pink-200 rounded-xl px-3 py-2 text-sm text-brand-purple-dark tabular-nums focus:outline-none focus:border-brand-orange/50"
          />
        </Field>
        ...
        <div className="flex items-center justify-end gap-2 pt-2">
          ...
          <button
            type="submit"
            disabled={busy}
            className="text-[11px] uppercase tracking-[0.2em] text-brand-purple-dark bg-brand-amber hover:bg-brand-amber/90 rounded-lg px-3 py-1.5 disabled:opacity-50"
          >
