    ...
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      ...
      <header className="flex items-baseline justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">Assets</h1>
      ...
      >
        ...
        <p className="text-[11px] text-brand-purple-dark/80 mt-1">Up to 4 MB per file. Stored as data URIs in cloud state.</p>
      </div>
      ...
      {assets.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {assets.map(asset => (
            <article key={asset.id} className="rounded-xl border border-pink-200 bg-white/[0.02] overflow-hidden flex flex-col">
              <div className="aspect-square bg-white/[0.04] flex items-center justify-center">
                {asset.contentType.startsWith("image/")
                  ...
                  : <span className="text-[10px] text-brand-purple-dark/80 px-2 text-center break-all">{asset.contentType}</span>
                }
              </div>
              <div className="p-2 space-y-1.5">
                <p className="text-[11px] text-brand-purple-dark/80 font-mono truncate" title={asset.filename}>{asset.filename}</p>
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-pink-200 rounded px-1.5 py-1 text-[10px] text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-brand-orange/50"
                />
                <div className="flex gap-1.5">
                  <button
                    onClick={() => navigator.clipboard?.writeText(asset.dataUrl)}
                    className="flex-1 px-2 py-1 rounded bg-white/5 hover:bg-white/10 text-[10px] text-brand-purple-dark/80"
                    title="Copy data URL"
                  >
                    Copy
                  </button>
