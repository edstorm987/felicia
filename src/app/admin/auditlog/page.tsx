    ...
    <div className="p-6 sm:p-8 lg:p-10 max-w-6xl space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Audit log</h1>
              ...
              <li key={e.id} className="px-4 py-3">
                <div className="flex flex-wrap items-start gap-3">
                  <span className={`text-[9px] uppercase tracking-[0.18em] px-2 py-0.5 rounded-full shrink-0 mt-0.5 ${
                    CATEGORY_COLOURS[e.category] ?? "bg-white/10 text-brand-purple-dark/80"
                  }`}>
                    {e.category}
                  </span>
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] text-brand-purple-dark">{e.action}</p>
                    <p className="text-[11px] text-brand-purple-dark/80 mt-0.5">
                      <span className="text-brand-purple-dark/80">{e.actorName}</span>
                      <span className="text-brand-purple-dark/80"> · {e.actorEmail}</span>
                      {e.resourceLink && (
                        <>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {hasDiff && (
                      <button
                        ...
                        className="text-[10px] uppercase tracking-[0.18em] text-brand-purple-dark/80 hover:text-brand-purple-dark"
                      >
                        {isExpanded ? "Hide diff" : "Show diff"}
                      </button>
                    )}
                    <span className="text-[10px] text-brand-purple-dark/80 tabular-nums">
                    ...
                    {Object.entries(e.diff!).map(([key, val]) => {
                      ...
                      return (
                        <div key={key} className="text-[11px] font-mono">
                          <span className="text-brand-purple-dark/80">{key}:</span>{" "}
