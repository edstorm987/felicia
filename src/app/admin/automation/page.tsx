    ...
    <main className="max-w-4xl mx-auto px-6 py-8 space-y-6">
      <header className="flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">Rules</h1>
          <p className="text-[12px] text-brand-purple-dark/80 mt-1">{rules.filter(r => r.enabled).length} active rules</p>
        </div>
        <Link href="/admin/automation/runs" className="text-[11px] text-cyan-300/80 hover:text-cyan-200">Run history →</Link>
      </header>

      {rules.length === 0 ? (
        <p className="text-[12px] text-brand-purple-dark/80">No rules yet — define your first via the API or import a template.</p>
      ) : (
        <ul className="space-y-1">
          {rules.map(r => (
            <li key={r.id} className="rounded-lg border border-pink-200/50 bg-white/[0.02] px-4 py-3 flex items-center gap-3">
              <span className={`text-[10px] tracking-wider uppercase px-2 py-1 rounded-md ${
                r.enabled ? "bg-emerald-500/10 text-emerald-300" : "bg-white/5 text-brand-purple-dark/80"
              }`}>
                {r.enabled ? "active" : "off"}
              </span>
              <div className="flex-1 min-w-0">
                <p className="text-[13px] text-brand-purple-dark truncate">{r.name}</p>
                <p className="text-[10px] font-mono text-brand-purple-dark/80">
