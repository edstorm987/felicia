  ...
  return (
    <main className="max-w-3xl mx-auto px-6 py-8 space-y-6">
      <header>
        ...
        <h1 className="font-display text-3xl text-brand-purple-dark">Run history</h1>
        <p className="text-[12px] text-brand-purple-dark/80 mt-1">Recent rule activity recorded in the audit log.</p>
      </header>
      ...
      {loading ? (
        ...
        <ul className="space-y-1">
          {entries.map(e => (
            <li key={e.id} className="rounded-lg border border-pink-200/50 bg-white/[0.02] px-4 py-3 flex items-center gap-3">
              <div className="flex-1 min-w-0">
                <p className="text-[13px] text-brand-purple-dark truncate">{e.action}</p>
                {e.actorEmail && <p className="text-[10px] text-brand-purple-dark/80 truncate">{e.actorEmail}</p>}
              </div>
              <span className="text-[10px] text-brand-purple-dark/80 tabular-nums shrink-0">{new Date(e.ts).toLocaleString()}</span>
            </li>
          ))}
        </ul>
      )}
