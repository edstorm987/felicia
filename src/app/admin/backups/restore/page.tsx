...
function RestoreInner() {
  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 max-w-3xl space-y-6">
      <Link href="/admin/backups" className="text-xs text-brand-purple-dark/80 hover:text-brand-purple-dark inline-block">
        ← Backups
      </Link>

      <header>
        <p className="text-[11px] tracking-[0.28em] uppercase text-brand-amber mb-2">Backups</p>
        <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Restore</h1>
        ...
        <section className="rounded-2xl border border-pink-200/50 bg-white p-5 sm:p-6 space-y-4">
          <label className="block">
            ...
            <select
              ...
              className="mt-1 w-full bg-white/5 border border-pink-200 rounded-xl px-3 py-2 text-sm text-brand-purple-dark font-mono focus:outline-none focus:border-brand-orange/50"
            >
          ...
          {target && (
            <>
              <div className="rounded-lg border border-pink-200/50 bg-white/80 p-3 text-[12px] text-brand-purple-dark/80 space-y-1">
                ...
                <div><span className="text-brand-purple-dark/80">Adapter:</span> {target.adapter}</div>
                {target.notes && <div><span className="text-brand-purple-dark/80">Notes:</span> {target.notes}</div>}
              </div>

              <label className="block">
                <span className="text-[11px] uppercase tracking-[0.18em] text-brand-purple-dark/80">
                ...
                <input
                  ...
                  className="mt-1 w-full bg-white/5 border border-pink-200 rounded-xl px-3 py-2 text-sm font-mono text-brand-purple-dark focus:outline-none focus:border-brand-orange/50"
                />
              </label>
