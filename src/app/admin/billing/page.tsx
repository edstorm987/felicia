    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <header className="flex items-baseline justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">Plan & subscription</h1>
      ...
      <section className="rounded-2xl border border-pink-200 bg-white/[0.02] p-5">
        <p className="text-[11px] uppercase tracking-[0.2em] text-brand-purple-dark/80 mb-3">Current</p>
        {subscription ? (
          <div className="space-y-2">
            <p className="text-brand-purple-dark font-medium">
            ...
            <p className="text-[12px] text-brand-purple-dark/80">Started {new Date(subscription.startedAt).toLocaleDateString()}</p>
            <button
              onClick={handleCancel}
              disabled={!!busy || subscription.status === "canceled"}
              className="text-[11px] text-brand-purple-dark/80 hover:text-red-400 disabled:opacity-40"
            >
      ...
      <section>
        <p className="text-[11px] uppercase tracking-[0.2em] text-brand-purple-dark/80 mb-3">Plans</p>
        <div className="grid sm:grid-cols-3 gap-3">
          {plans.map(plan => {
            ...
            return (
              <div
                key={plan.id}
                className={`rounded-2xl border p-4 ${active ? "border-brand-orange/60 bg-brand-orange/10" : "border-pink-200 bg-white/[0.02]"}`}
              >
                ...
                <button
                  ...
                  className={`w-full px-3 py-2 rounded-xl text-sm font-semibold transition-colors disabled:opacity-40 ${
                    active
                      ? "bg-white/10 text-brand-purple-dark/80 cursor-default"
