  ...
  if (!post) {
    return (
      <div className="p-6 sm:p-8 lg:p-10 max-w-3xl">
        <Link href="/admin/blog" className="text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark">← Blog</Link>
        <p className="mt-6 text-brand-purple-dark/80">Post not found.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 max-w-4xl space-y-6">
      <div className="flex items-center justify-between">
        <Link href="/admin/blog" className="text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark">← Blog</Link>
        <div className="text-[11px] text-brand-purple-dark/80 flex items-center gap-3">
          <span className={`px-2 py-0.5 rounded-full font-bold ${
            ...
            "bg-white/10 text-brand-purple-dark/80"
          }`}>{post.status}</span>
      ...
      <div className="space-y-3">
        <input
          ...
          className="w-full bg-transparent border-0 px-0 py-2 text-3xl sm:text-4xl font-display font-bold text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none"
        />
        <div className="flex items-center gap-2">
          <span className="text-[11px] text-brand-purple-dark/80 font-mono">/blog/</span>
          <input
            value={post.slug}
            onChange={e => { setSlugAuto(false); setPost({ ...post, slug: e.target.value }); }}
            onBlur={() => patch({ slug: post.slug })}
            placeholder="post-slug"
            className="flex-1 bg-transparent border-0 px-0 py-1 text-xs text-brand-purple-dark/80 font-mono focus:outline-none"
          />
          {!slugAuto && (
            <button
              type="button"
              onClick={() => { setSlugAuto(true); const s = slugify(post.title); setPost({ ...post, slug: s }); patch({ slug: s }); }}
              className="text-[10px] text-brand-purple-dark/80 hover:text-brand-orange underline underline-offset-4"
        ...
        <textarea
          ...
          className="w-full bg-pink-50 border border-pink-200 rounded-lg px-3 py-2.5 text-sm text-brand-purple-dark/80 focus:outline-none focus:border-brand-orange/40 resize-y leading-relaxed"
        />
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <p className="text-xs text-brand-purple-dark/80">Body</p>
          <p className="text-[11px] text-brand-purple-dark/80">
            <span className="text-brand-purple-dark/80 font-medium">{readStats.words.toLocaleString()}</span> words
            <span className="mx-2 text-brand-purple-dark/80">·</span>
            <span className="text-brand-purple-dark/80 font-medium">~{readStats.minutes}</span> min read
          </p>
        </div>
        <RichEditor
          value={post.bodyHtml}
          ...
          <div className="flex flex-wrap items-end gap-3">
            <Field label="Publish at">
              <input
                ...
                className="bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-sm text-brand-purple-dark"
              />
            </Field>
            <button
              type="button"
            ...
            {(post.status === "scheduled" || scheduleDraft) && (
              <button
                ...
                className="text-[11px] text-brand-purple-dark/80 hover:text-brand-orange underline underline-offset-4"
              >
        ...
        <div className="p-5 space-y-3">
          ...
          <Field label="JSON-LD schema (optional)">
            <textarea
              ...
              className="w-full bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-xs text-brand-purple-dark font-mono"
            />
          </Field>
        </div>
      </details>
...
function CoverImageEditor({ post, onChange }: { post: BlogPost; onChange: (src: string) => void }) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-[11px] tracking-widest uppercase text-brand-purple-dark/80">Featured image</p>
      <div className="flex gap-3 items-start">
        <div className="w-32 h-20 rounded-lg overflow-hidden bg-pink-50 border border-pink-200 shrink-0">
          {preview ? (
        ...
        <div className="flex-1 space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <label className={`text-[11px] px-3 py-1.5 rounded-md border cursor-pointer ${uploading ? "border-pink-200 text-brand-purple-dark/80" : "border-brand-orange/30 text-brand-orange hover:bg-brand-orange/10"}`}>
              {uploading ? "Uploading…" : "Upload new"}
              <input type="file" accept="image/*" onChange={handleFile} className="hidden" disabled={uploading} />
            </label>
            {post.coverImage && (
              <button onClick={() => onChange("")} className="text-[11px] px-3 py-1.5 rounded-md text-brand-purple-dark/80 hover:text-brand-orange">Remove</button>
            )}
          </div>
        </div>
      </div>
            ...
            <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
              {media.map(item => (
                <button
                  ...
                  className="relative aspect-square rounded-md overflow-hidden border border-pink-200 hover:border-brand-orange/50"
                  title={item.name}
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
