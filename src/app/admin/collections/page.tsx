  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-3xl">
      <div>
        ...
        <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Collections</h1>
      ...
      <section className="rounded-2xl border border-pink-200/50 bg-white p-5 sm:p-6">
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={c.slug} className="flex items-center justify-between gap-3 px-4 py-3 rounded-xl bg-white/3 border border-pink-200/50">
              <div>
                <p className="text-sm text-brand-purple-dark">{c.label}</p>
                <p className="text-[11px] text-brand-purple-dark/80">{c.sub}</p>
              </div>
              <span className="text-[10px] text-brand-purple-dark/80 font-mono">{c.slug}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-pink-200/50 bg-white p-5 sm:p-6">
        ...
        {collections.filter(c => !BUILT_IN.includes(c.slug)).length === 0 && !isNew && (
          <p className="text-sm text-brand-purple-dark/80 py-4 text-center">No custom collections yet.</p>
        )}
        ...
        <div className="space-y-2">
          {collections.filter(c => !BUILT_IN.includes(c.slug)).map(col => (
            <div key={col.slug} className={`flex items-center justify-between gap-3 px-4 py-3 rounded-xl border ${col.archived ? "border-pink-200/50 opacity-50" : "border-pink-200/50 bg-white/3"}`}>
              ...
              <span className="text-[10px] text-brand-purple-dark/80 font-mono shrink-0">{col.slug}</span>
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-white/80 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white border border-pink-200 rounded-2xl p-6 space-y-4">
            ...
            <label className="block">
              <span className="block text-[10px] tracking-[0.22em] uppercase text-brand-purple-dark/80 mb-1.5">Slug (URL handle) *</span>
              <input
                value={editing.slug}
                readOnly={!isNew}
            ...
            <label className="block">
              <span className="block text-[10px] tracking-[0.22em] uppercase text-brand-purple-dark/80 mb-1.5">Subtitle</span>
              <input
                value={editing.sub}
            ...
            <label className="flex items-center gap-3 p-3 rounded-lg border border-pink-200 cursor-pointer hover:border-pink-300">
              ...
              />
              <span className="text-sm text-brand-purple-dark">Archive (hide from shop)</span>
            </label>
