  ...
  if (loading) {
    return (
      <div className="p-6 sm:p-8 lg:p-10 max-w-5xl">
        ...
        <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Compliance</h1>
        <p className="text-brand-purple-dark/80 text-sm mt-3">Loading compliance state from the cloud store…</p>
      </div>
    );
  }
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="ml-auto text-[10px] uppercase tracking-wider text-brand-purple-dark/80">
                  Audit · {m.retentionLabel}
                </span>
              </div>
              <p className="text-[12px] text-brand-purple-dark/80 leading-relaxed mb-2">{m.blurb}</p>
              <ul className="text-[11px] text-brand-purple-dark/80 space-y-0.5">
                {m.rules.map((r, i) => (
                  <li key={i}>• {r}</li>
                ))}
              </ul>
            ...
            return (
              <div key={c.id} className="px-5 py-3 flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] text-brand-purple-dark font-medium">{c.label}</p>
                  <p className="text-[11px] text-brand-purple-dark/80 leading-relaxed mt-0.5">{c.detail}</p>
                  {c.fixHint && (
                    <p className="text-[11px] text-brand-purple-dark/80 leading-relaxed mt-1 italic">→ {c.fixHint}</p>
                  )}
                </div>
