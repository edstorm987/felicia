...
const STAGE_TONE: Record<Deal["stage"], string> = {
  lead:      "bg-white/5 text-brand-purple-dark/80",
  ...
  return (
    <main className="max-w-5xl mx-auto px-6 py-8 space-y-6">
      <header>
        ...
        <h1 className="font-display text-3xl text-brand-purple-dark">Deals</h1>
        <p className="text-[12px] text-brand-purple-dark/80 mt-1">{deals.length} deal{deals.length === 1 ? "" : "s"} · pipeline value £{deals.reduce((a, d) => a + (d.value ?? 0), 0).toFixed(2)}</p>
      </header>
      ...
      <section className="grid grid-cols-2 sm:grid-cols-5 gap-2">
        {(["lead", "qualified", "proposal", "won", "lost"] as const).map(s => (
          ...
          >
            ...
            {totals[s].value > 0 && <p className="text-[10px] text-brand-purple-dark/80 mt-0.5">£{totals[s].value.toFixed(0)}</p>}
          </button>
        ))}
      </section>
      ...
      {loading ? (
        ...
        <p className="text-[12px] text-brand-purple-dark/80">{filter === "all" ? "No deals yet. Create one from a contact's profile or via the API." : `No deals in "${filter}".`}</p>
      ) : (
        <ul className="space-y-1">
          {visible.map(d => (
            <li key={d.id} className="rounded-lg border border-pink-200/50 bg-white/[0.02] px-4 py-3 flex items-center gap-3">
              <div className="flex-1 min-w-0">
                <p className="text-[13px] text-brand-purple-dark truncate">{d.title}</p>
                <p className="text-[10px] text-brand-purple-dark/80 font-mono truncate">{d.contactId}</p>
              </div>
              {d.value !== undefined && (
                <span className="text-[12px] tabular-nums text-brand-purple-dark/80 shrink-0">£{d.value.toFixed(2)}</span>
              )}
              <span className={`text-[10px] tracking-wider uppercase px-2 py-1 rounded ${STAGE_TONE[d.stage]} shrink-0`}>{d.stage}</span>
              <span className="text-[10px] text-brand-purple-dark/80 tabular-nums shrink-0">{new Date(d.createdAt).toLocaleDateString()}</span>
            </li>
          ))}
        </ul>
      )}
      ...
      {!loading && deals.length > 0 && (
        <p className="text-[10px] text-brand-purple-dark/80">Tip: tap a stage card above to filter the list.</p>
      )}
    </main>
  );
}
