    ...
    <main className="max-w-5xl mx-auto px-6 py-8 space-y-6">
      <header className="flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">Pipeline</h1>
        </div>
      ...
      <section>
        <h2 className="text-[10px] tracking-[0.28em] uppercase text-brand-purple-dark/80 mb-3">Upcoming tasks</h2>
        {openTasks.length === 0 ? (
          <p className="text-[12px] text-brand-purple-dark/80">No open tasks. </p>
        ) : (
          <ul className="space-y-1">
            {openTasks.slice(0, 10).map(t => (
              <li key={t.id} className="rounded-lg border border-pink-200/50 bg-white/[0.02] px-4 py-2 flex items-center justify-between text-[12px]">
                <span className="text-brand-purple-dark truncate">{t.title}</span>
      ...
      <section>
        <h2 className="text-[10px] tracking-[0.28em] uppercase text-brand-purple-dark/80 mb-3">Recent contacts</h2>
        {contacts.length === 0 ? (
          <p className="text-[12px] text-brand-purple-dark/80">No contacts — they auto-import from forms + e-commerce + newsletter.</p>
        ) : (
          <ul className="space-y-1">
            {contacts.slice(0, 10).map(c => (
              <li key={c.id} className="rounded-lg border border-pink-200/50 bg-white/[0.02] px-4 py-2 text-[12px]">
                <span className="text-brand-purple-dark">{c.name ?? c.email}</span>
                {c.name && <span className="text-brand-purple-dark/80 ml-2">{c.email}</span>}
                {c.tags.length > 0 && (
                  <span className="ml-3 text-[10px] text-brand-purple-dark/80">
                    {c.tags.map(t => `#${t}`).join("  ")}
                  </span>
                )}
              </li>
