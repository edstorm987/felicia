  ...
  return (
    <main className="max-w-3xl mx-auto px-6 py-8 space-y-6">
      <header>
        ...
        <h1 className="font-display text-3xl text-brand-purple-dark">Tasks</h1>
        <p className="text-[12px] text-brand-purple-dark/80 mt-1">{open.length} open · {done.length} recently done</p>
      </header>

      <section className="rounded-xl border border-pink-200/50 bg-white/[0.02] p-4 space-y-2">
        <p className="text-[11px] tracking-wider uppercase text-brand-purple-dark/80">Quick add</p>
        <div className="flex gap-2 flex-wrap">
          <input
            ...
            className="flex-1 min-w-[220px] bg-white/5 border border-pink-200 rounded-md px-3 py-1.5 text-[12px] text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-cyan-400/40"
          />
          <input
            type="datetime-local"
            value={draftDue}
            onChange={e => setDraftDue(e.target.value)}
            className="bg-white/5 border border-pink-200 rounded-md px-3 py-1.5 text-[12px] text-brand-purple-dark focus:outline-none focus:border-cyan-400/40"
          />
          <button
      ...
      {loading ? (
        ...
        <>
          <section className="space-y-1">
            <h2 className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">Open</h2>
            {open.length === 0 ? (
              <p className="text-[12px] text-brand-purple-dark/80">All caught up.</p>
            ) : (
              <ul className="space-y-1">
                {open.map(t => {
                  const overdue = t.dueAt && t.dueAt < Date.now();
                  return (
                    <li key={t.id} className={`rounded-lg border px-4 py-3 flex items-center gap-3 ${overdue ? "border-amber-400/30 bg-amber-500/5" : "border-pink-200/50 bg-white/[0.02]"}`}>
                      <div className="flex-1 min-w-0">
                        <p className="text-[13px] text-brand-purple-dark">{t.title}</p>
                        {(t.contactId || t.dealId) && (
                          <p className="text-[10px] text-brand-purple-dark/80 font-mono">
                            {t.contactId ? `contact: ${t.contactId}` : ""}{t.contactId && t.dealId ? " · " : ""}{t.dealId ? `deal: ${t.dealId}` : ""}
                          </p>
                        )}
                      </div>
                      {t.dueAt && (
                        <span className={`text-[11px] tabular-nums shrink-0 ${overdue ? "text-amber-300" : "text-brand-purple-dark/80"}`}>
          ...
          {done.length > 0 && (
            <section className="space-y-1 pt-2">
              <h2 className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">Recently done</h2>
              <ul className="space-y-1">
                {done.map(t => (
                  <li key={t.id} className="rounded-lg border border-pink-200/50 bg-white/[0.01] px-4 py-2 flex items-center gap-3 opacity-65">
                    <span className="text-emerald-300 shrink-0">✓</span>
                    <span className="text-[12px] text-brand-purple-dark/80 line-through flex-1 truncate">{t.title}</span>
                    {t.doneAt && <span className="text-[10px] text-brand-purple-dark/80 tabular-nums shrink-0">{new Date(t.doneAt).toLocaleDateString()}</span>}
                  </li>
                ))}
              </ul>
            </section>
