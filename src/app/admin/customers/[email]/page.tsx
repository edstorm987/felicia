...
const STATUS_COLORS: Record<OrderStatus, string> = {
  ...
  cancelled: "bg-white/5 text-brand-purple-dark/80",
};
...
export default function CustomerDetailPage() {
  ...
  if (!customer) {
    return (
      <div className="p-8 max-w-xl">
        <Link href="/admin/customers" className="text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark">← Customers</Link>
        <h1 className="font-display text-2xl text-brand-purple-dark mt-4 mb-2">Customer not found</h1>
        <p className="text-sm text-brand-purple-dark/80">No orders found for <code className="text-brand-purple-dark/80">{email}</code>.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-5xl">
      {/* Breadcrumb */}
      <Link href="/admin/customers" className="text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark transition-colors">
        ← All customers
      </Link>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="font-display text-2xl sm:text-3xl text-brand-purple-dark">{customer.name}</h1>
          <p className="text-sm text-brand-purple-dark/80 mt-0.5">{customer.email}</p>
          <div className="flex flex-wrap gap-2 mt-3">
            {tags.map(t => (
              <span key={t} className="inline-flex items-center gap-1 text-[11px] px-2.5 py-0.5 rounded-full bg-white/8 text-brand-purple-dark/80">
                {t}
                <button onClick={() => removeTag(t)} className="text-brand-purple-dark/80 hover:text-red-400 transition-colors">×</button>
              </span>
            ))}
            <form onSubmit={e => { e.preventDefault(); addTag(); }} className="inline-flex">
              <input
                ...
                placeholder="+ tag"
                className="text-[11px] px-2 py-0.5 rounded-full bg-white/5 border border-pink-200 text-brand-purple-dark/80 placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-white/25 w-20"
              />
            </form>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          <div key={s.label} className="rounded-2xl bg-white border border-pink-200/50 px-4 py-4">
            <p className="font-display text-xl font-bold text-brand-amber truncate">{s.value}</p>
            <p className="text-[11px] tracking-wide text-brand-purple-dark/80 mt-0.5">{s.label}</p>
          </div>
        ))}
      </div>
        ...
        <div className="rounded-2xl border border-white/8 bg-white overflow-hidden">
          ...
          <div className="divide-y divide-white/5">
            {customer.allOrders.length === 0 && (
              <p className="px-5 py-8 text-center text-sm text-brand-purple-dark/80">No orders.</p>
            )}
        ...
        <div className="rounded-2xl border border-white/8 bg-white overflow-hidden">
          <div className="px-5 py-4 border-b border-pink-200/50">
            <p className="text-sm text-brand-purple-dark/80 leading-relaxed">
              ...
              <span className="text-brand-purple-dark/80"> Last order: {lastDate}</span>
            </p>
          </div>
          <div className="divide-y divide-white/5">
            {flags.map(flag => {
              ...
              return (
                <div key={flag.id} className="flex items-center justify-between gap-4 px-5 py-3.5">
                  <div className="min-w-0">
                    <p className="text-sm text-brand-purple-dark truncate">{flag.name}</p>
                    <p className="text-[11px] text-brand-purple-dark/80 truncate">{flag.id}</p>
                  </div>
                        ...
                        <button
                          ...
                          className={`text-[11px] px-2.5 py-1 rounded-full border transition-colors ${
                            active
                              ...
                              : "bg-transparent border-pink-200 text-brand-purple-dark/80 hover:text-brand-purple-dark"
                          }`}
                        >
                          {label}
                        </button>
        ...
        <div className="rounded-2xl border border-white/8 bg-white overflow-hidden">
          ...
          <div className="p-5">
            <textarea
              ...
              className="w-full bg-pink-50 border border-pink-200 rounded-xl px-4 py-3 text-sm text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-brand-orange/40 resize-y leading-relaxed"
            />
            <p className="text-[11px] text-brand-purple-dark/80 mt-2">Auto-saved • Not visible to the customer</p>
          </div>
        </div>
      )}
    </div>
...
function OrderRow({ order, onStatusChange }: {
  ...
  return (
    <div>
      ...
      >
        <div className="md:grid md:grid-cols-[1fr_140px_100px_110px_160px] md:gap-3 md:items-center flex flex-col gap-1">
          <div>
            ...
            <p className="text-[11px] text-brand-purple-dark/80 font-mono truncate md:hidden">
              {order.items.map(i => i.name).join(", ")}
            </p>
          </div>
          <span className="text-xs text-brand-purple-dark/80">
            {new Date(order.createdAt).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}
          </span>
          <span className="text-sm font-medium text-brand-purple-dark md:text-right">£{order.total.toFixed(2)}</span>
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            <Link
              ...
              className="text-[11px] px-2.5 py-1 rounded-lg border border-pink-200 text-brand-purple-dark/80 hover:text-brand-purple-dark transition-colors"
            >
              View
            </Link>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
              className={`text-brand-purple-dark/80 transition-transform duration-200 ml-auto ${open ? "rotate-180" : ""}`}>
      ...
      {open && (
        <div className="border-t border-pink-200/50 px-5 py-4 space-y-4 bg-white/[0.01]">
          {/* Items */}
          <div className="space-y-1.5">
            {order.items.map((item, i) => (
              <div key={i} className="flex items-center justify-between gap-3 text-sm">
                <span className="text-brand-purple-dark/80">{item.name}{item.variant ? ` · ${item.variant}` : ""}</span>
                <span className="text-brand-purple-dark/80 shrink-0">
          ...
          {order.shippingAddress && (
            <div className="text-xs text-brand-purple-dark/80 leading-relaxed">
              <span className="text-brand-purple-dark/80 uppercase tracking-wider text-[10px]">Shipping · </span>
              {[
          ...
          <div className="flex items-center gap-2 flex-wrap pt-1">
            <span className="text-[10px] uppercase tracking-wider text-brand-purple-dark/80">Change status:</span>
            {statuses.map(s => (
              <button
                key={s}
                ...
                className={`text-[11px] px-2.5 py-1 rounded-full border capitalize transition-colors ${
                  order.status === s
                    ...
                    : "border-pink-200 text-brand-purple-dark/80 hover:text-brand-purple-dark"
                }`}
              >
                {s}
              </button>
