  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 max-w-5xl space-y-6">
      ...
      <div>
        ...
        <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Customise the panel</h1>
        ...
        {TABS.map(t => (
          <button
            ...
            className={`px-4 py-2.5 text-sm font-medium whitespace-nowrap border-b-2 transition-colors -mb-px ${
              tab === t.id ? "border-brand-orange text-brand-purple-dark" : "border-transparent text-brand-purple-dark/80 hover:text-brand-purple-dark/80"
            }`}
          >
            {t.label}
          </button>
      ...
      {tab === "branding" && (
        <div className="space-y-5">
          <Card title="Names & logo" tip="The panel name and short name shown in the sidebar header.">
            ...
            {branding.logoUrl && (
              <div className="mt-2 flex items-center gap-3 px-3 py-2 rounded-lg bg-white/5 border border-pink-200">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={branding.logoUrl} alt="Logo preview" className="h-8 w-auto object-contain" />
                <span className="text-xs text-brand-purple-dark/80">Logo preview</span>
              </div>
            )}
          </Card>
                  ...
                  <button
                    ...
                    className={`text-left rounded-xl border p-4 transition-colors ${
                      active
                        ...
                        : "border-pink-200 bg-white/[0.02] hover:border-white/25"
                    }`}
                  >
                    <p className={`text-[13px] font-semibold ${active ? "text-brand-purple-dark" : "text-brand-purple-dark/80"}`}>
                      {opt.label}
                    </p>
                    <p className="text-[11px] text-brand-purple-dark/80 mt-1 leading-relaxed">{opt.description}</p>
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-2xl border border-cyan-400/20 bg-cyan-500/[0.04] p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="text-[13px] text-brand-purple-dark">Want full control over the login layout?</p>
              ...
              {(["centered", "split", "minimal"] as LoginLayout[]).map(l => (
                <button
                  ...
                  className={`px-3 py-3 rounded-xl border text-xs capitalize transition-colors ${
                    login.layout === l ? "border-brand-orange bg-brand-orange/10 text-brand-purple-dark" : "border-pink-200 text-brand-purple-dark/80 hover:border-white/25"
                  }`}
                >
                  {l}
                </button>
    ...
    <div className="px-4 py-3">
      {editing ? (
        <div className="space-y-2">
          ...
          <label className="flex items-center gap-2 text-xs text-brand-purple-dark/80">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-brand-purple-dark truncate">{tab.label}</p>
            <p className="text-[11px] text-brand-purple-dark/80 font-mono truncate">{tab.embedUrl}</p>
          </div>
          <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-white/8 text-brand-purple-dark/80 shrink-0">{tab.group}</span>
          <div className="flex items-center gap-1 shrink-0">
            <button disabled={isFirst} onClick={() => moveCustomTab(tab.id, -1)} className="px-1.5 py-1 text-brand-purple-dark/80 hover:text-brand-purple-dark disabled:opacity-25">↑</button>
            <button disabled={isLast} onClick={() => moveCustomTab(tab.id, 1)} className="px-1.5 py-1 text-brand-purple-dark/80 hover:text-brand-purple-dark disabled:opacity-25">↓</button>
            <button onClick={() => setEditing(true)} className="text-[11px] px-2 py-1 text-brand-purple-dark/80 hover:text-brand-purple-dark">Edit</button>
            <button
              onClick={async () => { if (await confirm({ title: `Delete "${tab.label}"?`, danger: true, confirmLabel: "Delete tab" })) deleteCustomTab(tab.id); }}
              className="text-[11px] px-2 py-1 text-brand-purple-dark/80 hover:text-brand-orange"
            >
              Delete
            </button>
          </div>
...
function ColorRow({ label, value, onChange, tip }: { label: string; value: string; onChange: (v: string) => void; tip?: string }) {
  return (
    <div className="flex items-center justify-between gap-3 py-2 border-b border-pink-200/50 last:border-0">
      <span className="text-sm text-brand-purple-dark/80 flex-1 flex items-center gap-1.5">
        {label}
        {tip && <Tip text={tip} />}
      </span>
  ...
  return (
    <div className="space-y-5">
      <Card title="How it works" tip="The admin sidebar is organised as top-level panels. Click a panel to drill in; click 'All panels' to come back.">
        <p className="text-sm text-brand-purple-dark/80 leading-relaxed">
          The sidebar opens with a list of <strong className="text-brand-purple-dark">panels</strong> (Store, Website, Users, Settings).
          Clicking a panel <strong className="text-brand-purple-dark">drills in</strong> and shows that panel&apos;s items;
          a back button returns to the top level. Each item can be a <strong className="text-brand-purple-dark">link</strong> or a
          <strong className="text-brand-purple-dark"> folder</strong>; folders can hold further folders, up to
          <strong className="text-brand-purple-dark"> {MAX_SIDEBAR_DEPTH} levels deep</strong> (counting the panel as level 1).
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2">
          ...
          <button onClick={onReset} className="text-xs px-3 py-2 rounded-lg border border-pink-200 text-brand-purple-dark/80 hover:text-brand-purple-dark">
            Reset to defaults
          </button>
        </div>
      </Card>
...
function PanelEditor({
  ...
  return (
    <div className="rounded-2xl border border-pink-200 bg-white/[0.02] overflow-hidden">
      <div className="px-4 py-3 border-b border-pink-200/50 bg-white/[0.02] flex items-center gap-3">
        <input
          value={panel.icon}
        ...
        <div className="flex items-center gap-1">
          <button disabled={isFirst} onClick={() => onMove(-1)} className="px-1.5 py-1 text-brand-purple-dark/80 hover:text-brand-purple-dark disabled:opacity-25" title="Move up">↑</button>
          <button disabled={isLast} onClick={() => onMove(1)} className="px-1.5 py-1 text-brand-purple-dark/80 hover:text-brand-purple-dark disabled:opacity-25" title="Move down">↓</button>
          <button onClick={onDelete} className="text-[11px] px-2 py-1 text-brand-purple-dark/80 hover:text-brand-orange">Delete</button>
        </div>
      </div>
      <div className="p-4 space-y-3">
        ...
        <div className="space-y-1.5 pt-1">
          ...
          {panel.items.length === 0 && (
            <p className="text-xs text-brand-purple-dark/80 px-3 py-3 text-center">
...
function ItemEditor({
  ...
  if (item.type === "link") {
    return (
      <div className="flex flex-wrap items-center gap-2 px-2 py-1.5 rounded-lg border border-pink-200/50 bg-white/[0.015]">
        <span className="text-[10px] uppercase tracking-wider text-brand-purple-dark/80 w-12 shrink-0">Link</span>
        <input
          value={item.label}
        ...
        />
        <div className="flex items-center gap-1">
          <button disabled={isFirst} onClick={() => onMove(-1)} className="px-1.5 py-1 text-brand-purple-dark/80 hover:text-brand-purple-dark disabled:opacity-25">↑</button>
          <button disabled={isLast} onClick={() => onMove(1)} className="px-1.5 py-1 text-brand-purple-dark/80 hover:text-brand-purple-dark disabled:opacity-25">↓</button>
          <button onClick={onDelete} className="text-[11px] px-2 py-1 text-brand-purple-dark/80 hover:text-brand-orange">×</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-pink-200 bg-white/[0.02]">
      <div className="flex flex-wrap items-center gap-2 px-2 py-1.5 border-b border-pink-200/50">
        ...
        />
        <span
          className="text-[9px] uppercase tracking-wider text-brand-purple-dark/80 px-2 py-0.5 rounded-full border border-pink-200"
      ...
      <div className="p-2 space-y-1.5">
        {group.items.length === 0 && (
          <p className="text-xs text-brand-purple-dark/80 px-3 py-2 text-center">Empty folder.</p>
        )}
        ...
        <div className="flex flex-wrap gap-2 pt-1 pl-3">
          <button onClick={addLinkChild} className="text-[11px] px-3 py-1.5 rounded-lg border border-pink-200 text-brand-purple-dark/80 hover:text-brand-purple-dark hover:border-white/25">
            + Link
          </button>
          <button
            ...
            className="text-[11px] px-3 py-1.5 rounded-lg border border-pink-200 text-brand-purple-dark/80 hover:text-brand-purple-dark hover:border-white/25 disabled:opacity-30 disabled:cursor-not-allowed"
          >
            + Folder
          </button>
        </div>
