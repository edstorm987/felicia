    ...
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      <header className="flex items-baseline justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">Customise widgets</h1>
          <p className="text-[12px] text-brand-purple-dark/80 mt-1">Org: <span className="text-brand-purple-dark/80 font-medium">{orgName || "—"}</span></p>
        </div>
        <div className="flex items-center gap-2 text-[11px] text-brand-purple-dark/80">
          {busy && <span>Saving…</span>}
          {savedAt && !busy && <span className="text-brand-purple-dark/80">Saved</span>}
      ...
      <section className="space-y-2">
        <p className="text-[11px] uppercase tracking-[0.2em] text-brand-purple-dark/80">Active widgets</p>
        {widgets.length === 0 && (
          <p className="text-[12px] text-brand-purple-dark/80">No widgets — add one below.</p>
        )}
        {widgets.map((w, i) => (
          <div key={w.id} className="rounded-xl border border-pink-200 bg-white/[0.02] p-3 flex items-center gap-3">
            <div className="flex flex-col gap-0.5">
              <button onClick={() => move(w.id, -1)} disabled={i === 0} className="text-[10px] text-brand-purple-dark/80 hover:text-brand-orange disabled:opacity-20">▲</button>
              <button onClick={() => move(w.id, 1)} disabled={i === widgets.length - 1} className="text-[10px] text-brand-purple-dark/80 hover:text-brand-orange disabled:opacity-20">▼</button>
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm text-brand-purple-dark font-medium truncate">{w.title || w.type}</p>
              <p className="text-[10px] text-brand-purple-dark/80 font-mono">{w.type}</p>
            </div>
            <div className="flex items-center gap-1">
              {[1, 2, 3].map(n => (
                <button
                  ...
                  className={`w-7 h-7 rounded-lg text-[11px] font-mono ${w.span === n ? "bg-brand-orange text-white" : "bg-white/5 text-brand-purple-dark/80 hover:text-brand-purple-dark"}`}
                >
                  {n}
                </button>
              ))}
            ...
            <button
              ...
              className={`px-2 py-1 rounded-lg text-[11px] ${w.visible ? "bg-white/10 text-brand-purple-dark/80" : "bg-white/5 text-brand-purple-dark/80 line-through"}`}
            >
              {w.visible ? "Visible" : "Hidden"}
            </button>
            <button onClick={() => removeWidget(w.id)} className="text-[11px] text-brand-purple-dark/80 hover:text-red-400">Remove</button>
          </div>
        ))}
      </section>

      <section className="space-y-2">
        <p className="text-[11px] uppercase tracking-[0.2em] text-brand-purple-dark/80">Add widget</p>
        <div className="grid sm:grid-cols-3 gap-2">
          {WIDGET_LIBRARY.map(w => (
            <button
              key={w.type}
              onClick={() => addWidget(w.type, w.defaultSpan)}
              className="rounded-xl border border-pink-200 bg-white/[0.02] hover:bg-white/[0.05] p-3 text-left"
            >
              <p className="text-sm text-brand-purple-dark">{w.label}</p>
              <p className="text-[10px] text-brand-purple-dark/80 font-mono">{w.type} · span {w.defaultSpan}</p>
            </button>
          ))}
        </div>
      </section>
