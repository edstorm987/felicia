...
function DonationsPageInner() {
  ...
  if (!stats) return <main className="p-6 text-[12px] text-brand-purple-dark/80">No data.</main>;
    ...
    <main className="max-w-4xl mx-auto px-6 py-8 space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">Donor activity</h1>
        </div>
      ...
      <section>
        <h2 className="text-[10px] tracking-[0.28em] uppercase text-brand-purple-dark/80 mb-3">Recent donations</h2>
        {stats.recentDonations.length === 0 ? (
          <p className="text-[12px] text-brand-purple-dark/80">No donations yet.</p>
        ) : (
          <div className="space-y-1">
            {stats.recentDonations.map(d => (
              <article key={d.id} className="rounded-lg border border-pink-200/50 bg-white/[0.02] px-4 py-3 flex items-center gap-3">
                <span className="text-[15px] font-display text-brand-purple-dark tabular-nums w-20">{fmt(d.amount, d.currency)}</span>
                <div className="flex-1 min-w-0">
                  <p className="text-[12px] text-brand-purple-dark truncate">
                  ...
                  {d.message && <p className="text-[10px] text-brand-purple-dark/80 truncate">&ldquo;{d.message}&rdquo;</p>}
                </div>
