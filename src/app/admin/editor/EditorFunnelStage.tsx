...
export default function EditorFunnelStage({ funnel, onChange, onDeleted }: Props) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto py-6 space-y-6">
      <header className="flex items-start gap-3 flex-wrap">
        <div className="flex-1 min-w-0">
          ...
          <input
            ...
            className="font-display text-3xl text-brand-purple-dark placeholder:text-brand-purple-dark/80 bg-transparent border-b border-transparent hover:border-pink-200 focus:border-cyan-400/40 focus:outline-none w-full"
          />
          <textarea
            ...
            rows={2}
            className="mt-2 w-full bg-transparent text-[12px] text-brand-purple-dark/80 placeholder:text-brand-purple-dark/80 focus:outline-none resize-none"
          />
        </div>
      ...
      <section className="rounded-xl border border-pink-200/50 bg-white/[0.02] p-4 grid grid-cols-3 gap-4">
      ...
      <section className="space-y-2">
        <div className="flex items-center justify-between">
          <h2 className="text-[11px] tracking-wider uppercase text-brand-purple-dark/80">Steps</h2>
          <button
            onClick={addStep}
        ...
        {draft.steps.length === 0 && (
          <p className="text-[12px] text-brand-purple-dark/80 italic px-1 py-3">
        ...
        <ol className="space-y-2">
          {draft.steps.map((s, i) => (
            <li
              key={s.id}
              className="rounded-lg border border-pink-200/50 bg-pink-50/50 p-3 flex items-start gap-3"
            >
              ...
              <div className="flex-1 min-w-0 grid grid-cols-1 sm:grid-cols-3 gap-2">
                <input
                  ...
                  className="bg-white/5 border border-pink-200 rounded-md px-2.5 py-1.5 text-[12px] text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-cyan-400/40"
                />
                <input
                  value={s.path}
                  onChange={e => patchStep(s.id, { path: e.target.value })}
                  placeholder="/path or /products/*"
                  className="bg-white/5 border border-pink-200 rounded-md px-2.5 py-1.5 text-[12px] font-mono text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-cyan-400/40"
                />
                <select
                  value={s.type}
                  onChange={e => patchStep(s.id, { type: e.target.value as StepType })}
                  className="bg-white/5 border border-pink-200 rounded-md px-2.5 py-1.5 text-[12px] text-brand-purple-dark focus:outline-none focus:border-cyan-400/40"
                >
              ...
              <div className="flex flex-col items-end gap-0.5 shrink-0 text-[10px] text-brand-purple-dark/80">
                <span>↘ <strong className="text-brand-purple-dark/80 tabular-nums">{s.reached}</strong> reached</span>
                <span>✓ <strong className="text-brand-purple-dark/80 tabular-nums">{s.completed}</strong> completed</span>
              </div>
              <div className="flex flex-col items-center gap-0.5 shrink-0">
                <button onClick={() => moveStep(s.id, -1)} disabled={i === 0} className="w-6 h-6 rounded text-brand-purple-dark/80 hover:text-brand-purple-dark hover:bg-white/5 disabled:opacity-20" title="Move up">↑</button>
                <button onClick={() => moveStep(s.id, 1)} disabled={i === draft.steps.length - 1} className="w-6 h-6 rounded text-brand-purple-dark/80 hover:text-brand-purple-dark hover:bg-white/5 disabled:opacity-20" title="Move down">↓</button>
                <button onClick={() => removeStep(s.id)} className="w-6 h-6 rounded text-brand-purple-dark/80 hover:text-red-300 hover:bg-red-500/10" title="Delete step">×</button>
              </div>
            </li>
          ))}
        </ol>
...
function Stat({ label, value, accent }: { label: string; value: string | number; accent?: boolean }) {
  return (
    <div>
      <p className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80 mb-1">{label}</p>
      <p className={`font-display text-2xl tabular-nums ${accent ? "text-cyan-200" : "text-brand-purple-dark"}`}>
        {value}
      </p>
    </div>
  );
