...
export default function EditorOutliner({
  ...
  return (
    <aside className="w-64 shrink-0 border-r border-pink-200/50 bg-pink-50/50 hidden md:flex flex-col overflow-hidden">
      <header className="px-4 py-3 border-b border-pink-200/50 flex items-start gap-2">
        <div className="flex-1 min-w-0">
          <p className="text-[10px] tracking-[0.32em] uppercase text-cyan-400 mb-0.5">Structure</p>
          <p className="text-[13px] text-brand-purple-dark font-medium truncate" title={siteName}>{siteName}</p>
        </div>
        <button
          onClick={onSiteSettings}
          title="Site settings"
          aria-label="Site settings"
          className="w-7 h-7 rounded-md text-brand-purple-dark/80 hover:text-cyan-300 hover:bg-cyan-500/10 flex items-center justify-center text-[12px]"
        >
          ⚙
        </button>
      </header>
      ...
      <div className="px-3 py-2 border-b border-pink-200/50">
        <div className="relative">
          <input
            ...
            placeholder="Search…"
            className="w-full bg-white/5 border border-pink-200 rounded-md pl-7 pr-2 py-1.5 text-[12px] text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-cyan-400/40"
          />
          <span className="absolute left-2 top-1/2 -translate-y-1/2 text-[11px] text-brand-purple-dark/80">⌕</span>
          {query && (
            <button
              onClick={() => setQuery("")}
              aria-label="Clear search"
              className="absolute right-1.5 top-1/2 -translate-y-1/2 w-5 h-5 rounded text-brand-purple-dark/80 hover:text-brand-purple-dark hover:bg-white/5 flex items-center justify-center text-[12px]"
            >×</button>
          )}
        </div>
      </div>
      ...
      <footer className="shrink-0 px-4 py-2 border-t border-pink-200/50 text-[10px] text-brand-purple-dark/80">
  ...
  return (
    <div className="mb-2">
      <div className="px-2 flex items-center gap-1 group">
        <button
          onClick={onToggle}
          className="flex-1 flex items-center gap-1.5 px-2 py-1 rounded-md text-[10px] tracking-wider uppercase text-brand-purple-dark/80 hover:text-brand-purple-dark hover:bg-white/5"
        >
          <span className={`inline-block transition-transform ${open ? "rotate-90" : ""}`}>▸</span>
          <span>{label}</span>
          <span className="text-brand-purple-dark/80 normal-case tracking-normal text-[10px]">({count})</span>
        </button>
        <button
          onClick={onAdd}
          title={addTitle}
          className="w-6 h-6 rounded-md flex items-center justify-center text-brand-purple-dark/80 hover:text-cyan-300 hover:bg-cyan-500/10"
        >
          +
        </button>
      </div>
...
function Row({
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-[12px] truncate ${active ? "text-cyan-100" : "text-brand-purple-dark/80"}`}>{label}</p>
        {hint && <p className="text-[10px] font-mono text-brand-purple-dark/80 truncate">{hint}</p>}
      </div>
      {badge && (
        <span className="text-[9px] tracking-wider uppercase text-brand-purple-dark/80 px-1 py-0.5 rounded bg-white/5 border border-pink-200 shrink-0">
          {badge}
        </span>
      )}
        ...
        {actions.map((a, i) => (
          <button
            ...
            className={`w-5 h-5 rounded flex items-center justify-center text-[11px] ${
              a.danger
                ? "text-brand-purple-dark/80 hover:text-red-300 hover:bg-red-500/10"
                : "text-brand-purple-dark/80 hover:text-cyan-300 hover:bg-cyan-500/10"
            }`}
          >
            {a.icon}
          </button>
...
function Empty({ hint }: { hint: string }) {
  return (
    <p className="mx-4 my-1.5 text-[10px] text-brand-purple-dark/80 italic">{hint}</p>
  );
}
