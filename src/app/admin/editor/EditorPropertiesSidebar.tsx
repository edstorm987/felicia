...
export default function EditorPropertiesSidebar({
  ...
  if (!selected) {
    return (
      <aside className="w-72 shrink-0 border-l border-pink-200/50 bg-pink-50/50 p-5 hidden lg:flex flex-col items-center justify-center text-center">
        <div className="w-12 h-12 rounded-full bg-white/5 flex items-center justify-center mb-3 text-2xl">↖</div>
        <p className="text-[12px] text-brand-purple-dark/80">Click any element marked editable in the preview to start.</p>
        <p className="text-[10px] text-brand-purple-dark/80 mt-3 leading-relaxed">
          Look for outlined regions on hover. Anything with a
          <code className="font-mono mx-1 text-brand-purple-dark/80">data-portal-edit</code>
  ...
  const inputClass = "w-full bg-white/5 border border-pink-200 rounded-md px-3 py-2 text-[12px] text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-cyan-400/40";

  return (
    <aside className="w-80 shrink-0 border-l border-pink-200/50 bg-pink-50/50 flex flex-col">
      <header className="px-4 py-3 border-b border-pink-200/50 flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="text-[10px] tracking-[0.32em] uppercase text-cyan-400 mb-1">{typeLabel(selected.type)}</p>
          <p className="text-[13px] text-brand-purple-dark font-medium truncate">{selected.label ?? selected.key}</p>
          <p className="text-[10px] font-mono text-brand-purple-dark/80 truncate mt-0.5">{selected.key}</p>
        </div>
        <button onClick={onClose} className="text-brand-purple-dark/80 hover:text-brand-purple-dark text-lg leading-none -mt-1" aria-label="Close panel">×</button>
      </header>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        {selected.type === "image-src" && draft && (
          ...
          <img src={draft} alt="" className="rounded-md border border-pink-200/50 max-h-32 object-contain mx-auto" />
        )}
      </div>

      <footer className="px-4 py-3 border-t border-pink-200/50 flex items-center justify-end gap-2">
        {dirty && (
          <button
            onClick={revert}
