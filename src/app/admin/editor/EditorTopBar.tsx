...
export default function EditorTopBar({
  ...
  return (
    <header className="shrink-0 flex items-center gap-3 px-4 py-2 border-b border-pink-200/50 bg-pink-50/50">
      {/* Back chip */}
      ...
      {isPage ? (
        <>
          ...
          {!isSimple && (
            <div className="flex items-center gap-0.5 border border-pink-200 rounded-lg p-0.5 bg-white/[0.02]">
...
function SelectChip({
  ...
  return (
    <label className="flex items-center gap-1.5" title={title}>
      <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">{icon} {label}</span>
      <select
        value={value}
        onChange={e => onChange(e.target.value)}
        className={`bg-white/5 border border-pink-200 rounded-md px-2 py-1 text-[12px] text-brand-purple-dark focus:outline-none focus:border-cyan-400/40 ${wide ? "min-w-[200px]" : ""}`}
      >
  ...
  return (
    <button
      ...
      className={`px-2.5 py-1 rounded-md text-[11px] flex items-center gap-1 transition-colors ${
        active ? "bg-cyan-500/15 text-cyan-200 border border-cyan-400/20" : "text-brand-purple-dark/80 hover:text-brand-purple-dark"
      }`}
    >
      <span>{icon}</span>
      <span>{label}</span>
  ...
  return (
    <button
      ...
      className={`px-2 py-1 rounded text-[12px] transition-colors ${
        active ? "bg-cyan-500/15 text-cyan-200" : "text-brand-purple-dark/80 hover:text-brand-purple-dark"
      }`}
    >
      {icon}
    </button>
    ...
    <button
      ...
      className={`w-8 h-8 rounded-md flex items-center justify-center text-[13px] transition-colors disabled:opacity-30 disabled:cursor-not-allowed ${
        highlighted
          ...
          : "bg-white/5 hover:bg-white/10 text-brand-purple-dark/80"
      }`}
    >
      {children}
    </button>
