...
function VisualEditorPageInner() {
  ...
  return (
    <main className="h-[calc(100vh-0px)] flex flex-col bg-pink-50">
      <EditorTopBar
        ...
        {target.kind === "funnel" ? (
          <div className="flex-1 min-w-0 overflow-auto bg-[#050505] flex items-start justify-center p-6">
            {currentFunnel ? (
              ...
              <div className="text-center text-[12px] text-brand-purple-dark/80 mt-12">Funnel not found.</div>
            )}
          </div>
          ...
          <>
            <div className="flex-1 min-w-0 overflow-auto bg-[#050505] flex items-start justify-center p-6">
              {mode === "code" ? (
                ...
                <div className="text-center text-[12px] text-brand-purple-dark/80 mt-12 max-w-md">
                ...
                <div className="text-center text-[12px] text-brand-purple-dark/80 mt-12 max-w-md">
                  ...
                  <p className="mt-2 text-brand-purple-dark/80">
      ...
      <footer className="shrink-0 px-4 py-2 border-t border-pink-200/50 bg-pink-50/50 text-[10px] text-brand-purple-dark/80 flex items-center gap-4">
        {target.kind === "funnel" ? (
          <span>Funnel editor — auto-saves changes. Step paths support globs (e.g. <code className="font-mono text-brand-purple-dark/80">/products/*</code>).</span>
        ) : mode === "live" ? (
          <>
            <span>Cmd/Ctrl+E to toggle edit mode inside the iframe</span>
            <span className="opacity-50">·</span>
            <span>Click any element marked <code className="font-mono text-brand-purple-dark/80">data-portal-edit</code> to edit</span>
          </>
        ) : mode === "block" ? (
...
function CodeStage({
  ...
  if (!loaded) {
    return <div className="text-center text-[12px] text-brand-purple-dark/80 mt-12">Loading…</div>;
  }
  ...
  return (
    <div className="w-full max-w-5xl flex flex-col h-[calc(100vh-160px)]">
      <div className="flex items-center gap-2 mb-2">
        <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">blocks.json</span>
        <span className="text-[10px] text-brand-purple-dark/80 font-mono truncate">{pageDoc?.slug}</span>
        <div className="flex-1" />
      ...
      <textarea
        ...
        spellCheck={false}
        className="flex-1 w-full font-mono text-[12px] leading-relaxed bg-[#0a0e1a] border border-pink-200 rounded-lg p-4 text-brand-purple-dark focus:outline-none focus:border-cyan-400/40"
      />
    </div>
  );
}
    ...
    >
      <div onClick={e => e.stopPropagation()} className="w-full max-w-md rounded-2xl border border-cyan-400/20 bg-[#0a0e1a] p-5 space-y-4">
        <header className="flex items-center justify-between">
          ...
          <button onClick={onClose} disabled={phase === "running"} className="text-brand-purple-dark/80 hover:text-brand-purple-dark text-lg leading-none disabled:opacity-30">×</button>
        </header>

        {phase === "idle" && (
          <>
            <h2 className="font-display text-xl text-brand-purple-dark">Ship {site.name} to GitHub</h2>

            {preview === null ? (
              <p className="text-[12px] text-brand-purple-dark/80">Reading current state…</p>
            ) : preview.changedContentKeys.length === 0 && preview.changedPages.length === 0 ? (
              <div className="rounded-lg border border-pink-200/50 bg-white/[0.02] p-3 text-[12px] text-brand-purple-dark/80">
              ...
              <div className="rounded-lg border border-cyan-400/15 bg-cyan-500/5 p-3 space-y-2">
                ...
                {preview.changedContentKeys.length > 0 && (
                  <details className="text-[12px] text-brand-purple-dark/80">
                    <summary className="cursor-pointer hover:text-brand-purple-dark">
                      {preview.changedContentKeys.length} content edit{preview.changedContentKeys.length === 1 ? "" : "s"}
                    </summary>
                    <ul className="mt-1 ml-3 space-y-0.5 font-mono text-[10px] text-brand-purple-dark/80 max-h-32 overflow-y-auto">
                ...
                {preview.changedPages.length > 0 && (
                  <details className="text-[12px] text-brand-purple-dark/80">
                    ...
                    <ul className="mt-1 ml-3 space-y-0.5 text-[11px] text-brand-purple-dark/80">
                      {preview.changedPages.map(p => (
                        <li key={p.id}>· <span className="text-brand-purple-dark/80">{p.title}</span> <span className="font-mono text-brand-purple-dark/80">{p.slug}</span></li>
                      ))}
                    </ul>
                  </details>
                )}
            ...
            <p className="text-[11px] text-brand-purple-dark/80 leading-relaxed">
              ...
              <code className="font-mono text-brand-purple-dark/80">portal.overrides.json</code>,{" "}
              <code className="font-mono text-brand-purple-dark/80">portal.pages.json</code>, and{" "}
              <code className="font-mono text-brand-purple-dark/80">portal.site.json</code>.
              Merge to deploy.
            </p>
            <label className="block">
              <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">Commit note (optional)</span>
              <textarea
                value={message}
                onChange={e => setMessage(e.target.value)}
                rows={2}
                placeholder="e.g. Updated hero copy + new product photo"
                className="mt-1 w-full bg-white/5 border border-pink-200 rounded-md px-3 py-2 text-[12px] text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-cyan-400/40"
              />
            </label>
            <div className="flex items-center justify-end gap-2 pt-1">
              <button onClick={onClose} className="text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark px-3 py-1.5">
                Cancel
              </button>
              <button
                onClick={() => void run()}
            ...
            <p className="text-[10px] text-brand-purple-dark/80 leading-relaxed">
        ...
        {phase === "running" && (
          <div className="text-center py-6 space-y-3">
            ...
            <p className="text-[12px] text-brand-purple-dark/80">{step}</p>
          </div>
        )}
        ...
        {phase === "done" && result?.ok && (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-display text-lg text-brand-purple-dark">Pull request opened</h2>
            </div>
            <p className="text-[12px] text-brand-purple-dark/80">
            ...
            {result.files && result.files.length > 0 && (
              <details className="text-[11px] text-brand-purple-dark/80">
                <summary className="cursor-pointer hover:text-brand-purple-dark/80">{result.files.length} file{result.files.length === 1 ? "" : "s"} included</summary>
                <ul className="mt-2 space-y-0.5 font-mono text-brand-purple-dark/80">
                  {result.files.map(f => <li key={f.path}>{f.path}</li>)}
                </ul>
              </details>
            )}
            <div className="flex justify-end pt-1">
              <button onClick={onClose} className="px-3 py-1.5 rounded-md text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark">
                Close
              </button>
            </div>
          </div>
        ...
        {phase === "error" && (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-display text-lg text-brand-purple-dark">Publish failed</h2>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-1">
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-md text-[11px] bg-white/5 hover:bg-white/10 text-brand-purple-dark/80"
              >
                Try again
              </button>
            </div>
...
function NewPageModal({
  ...
  return (
    <ModalShell title="New page" onClose={busy ? () => {} : onClose}>
      <label className="block">
        <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">Title</span>
        <input
          value={title}
          onChange={e => handleTitle(e.target.value)}
          placeholder="About us"
          autoFocus
          className="mt-1 w-full bg-white/5 border border-pink-200 rounded-md px-3 py-2 text-[13px] text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-cyan-400/40"
        />
      </label>
      <label className="block">
        <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">Slug</span>
        <input
          value={slug}
          onChange={e => { slugTouched.current = true; setSlug(e.target.value); }}
          placeholder="/about"
          className="mt-1 w-full bg-white/5 border border-pink-200 rounded-md px-3 py-2 text-[12px] font-mono text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-cyan-400/40"
        />
      </label>
...
function NewFunnelModal({
  ...
  return (
    <ModalShell title="New funnel" onClose={busy ? () => {} : onClose}>
      ...
      <label className="block">
        <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">Name</span>
        <input
          value={name}
...
function PageSettingsModal({
  ...
  return (
    <ModalShell title="Page settings" onClose={busy ? () => {} : onClose} wide>
      <div className="grid grid-cols-2 gap-3">
        <label className="block">
          ...
          <input
            ...
            className="mt-1 w-full bg-white/5 border border-pink-200 rounded-md px-3 py-2 text-[13px] text-brand-purple-dark focus:outline-none focus:border-cyan-400/40"
          />
        </label>
        <label className="block">
          <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">Slug</span>
          <input
            ...
            className="mt-1 w-full bg-white/5 border border-pink-200 rounded-md px-3 py-2 text-[12px] font-mono text-brand-purple-dark focus:outline-none focus:border-cyan-400/40"
          />
        </label>
      </div>
      <label className="block">
        <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">SEO description</span>
        <textarea
          value={description}
      ...
      <details>
        <summary className="text-[11px] text-brand-purple-dark/80 cursor-pointer hover:text-brand-purple-dark">Custom head / foot scripts</summary>
        <div className="mt-2 space-y-2">
          <label className="block">
            <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">Custom head</span>
            <textarea
              value={customHead}
              onChange={e => setCustomHead(e.target.value)}
              rows={3}
              placeholder="<script>…</script> or <link …>"
              className="mt-1 w-full bg-white/5 border border-pink-200 rounded-md px-3 py-2 text-[11px] font-mono text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-cyan-400/40"
            />
          </label>
          <label className="block">
            <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">Custom foot</span>
            <textarea
              value={customFoot}
      ...
      {isPro && (
        <details className="rounded-lg border border-cyan-400/20 bg-cyan-500/5 px-3 py-2">
          ...
          <div className="mt-3 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <label className="block">
                <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">Theme override</span>
                <select
                  value={themeId}
                  onChange={e => setThemeId(e.target.value)}
                  className="mt-1 w-full bg-white/5 border border-pink-200 rounded-md px-3 py-2 text-[12px] text-brand-purple-dark focus:outline-none focus:border-cyan-400/40"
                >
              ...
              <div className="block">
                <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80 block">Layout</span>
                <div className="mt-1 space-y-1">
                  <label className="flex items-center gap-2 text-[12px] text-brand-purple-dark cursor-pointer select-none">
                    <input
                      type="checkbox"
                      checked={hideNav}
            ...
            <label className="block">
              <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">Page CSS</span>
              <textarea
                value={customCss}
      ...
      {!loaded && <p className="text-[11px] text-brand-purple-dark/80">Loading…</p>}
      {error && <p className="text-[11px] text-red-300">{error}</p>}
      <ModalActions
        onCancel={onClose}
        onSubmit={() => void submit()}
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <header className="flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-brand-purple-dark/80 hover:text-brand-purple-dark text-lg leading-none">×</button>
        </header>
        {children}
      </div>
    </div>
...
function ModalActions({
  ...
  return (
    <div className="flex items-center justify-end gap-2 pt-1">
      <button onClick={onCancel} className="text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark px-3 py-1.5">
        Cancel
      </button>
      <button
        onClick={onSubmit}
  ...
  return (
    <ModalShell title="Site settings" onClose={onClose} wide>
      <div className="grid grid-cols-2 gap-3">
        ...
        <label className="block">
          <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">Tagline</span>
          <input
            value={tagline}
      ...
      <label className="block">
        <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">Description</span>
        <textarea
          value={description}
          onChange={e => setDescription(e.target.value)}
          rows={2}
      ...
      <div className="grid grid-cols-1 gap-3">
        <label className="block">
          <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">Primary domain</span>
          <input
            value={primaryDomain}
        ...
        <label className="block">
          <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">All domains (one per line)</span>
          <textarea
            value={domainsText}
      ...
      <details>
        <summary className="text-[11px] text-brand-purple-dark/80 cursor-pointer hover:text-brand-purple-dark">Site-wide custom head / body scripts</summary>
        <p className="mt-2 text-[10px] text-brand-purple-dark/80 leading-relaxed">
        ...
        <div className="mt-2 space-y-2">
          ...
          <label className="block">
            <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80">Custom body</span>
            <textarea
              value={customBody}
