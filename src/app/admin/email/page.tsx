  ...
  return (
    <main className="max-w-4xl mx-auto px-6 py-8 space-y-6">
      <header>
        ...
        <h1 className="font-display text-3xl text-brand-purple-dark">Transactional email</h1>
        <p className="text-[12px] text-brand-purple-dark/80 mt-1">Compose one-offs, edit your templates, see what's gone out.</p>
      </header>

      <div className="flex gap-1 border-b border-pink-200/50">
        {(["compose", "templates", "log", "test"] as const).map(t => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-3 py-2 text-[12px] capitalize transition-colors ${tab === t ? "text-brand-purple-dark border-b-2 border-cyan-400" : "text-brand-purple-dark/80 hover:text-brand-purple-dark"}`}
          >
            {t}
          </button>
        ))}
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  <p className="text-[10px] tracking-[0.22em] uppercase text-brand-purple-dark/80">{t.id}</p>
                  {t.isOverridden && (
                    <span className="text-[9px] tracking-wider uppercase px-1.5 py-0.5 rounded bg-amber-400/15 text-amber-300">edited</span>
                  )}
                </div>
                <p className="text-[13px] text-brand-purple-dark font-mono break-words">{t.subject}</p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                {!isEditing && (
                  <button
                    type="button"
                    onClick={() => startEdit(t)}
                    className="text-[11px] px-3 py-1.5 rounded-md border border-pink-200 text-brand-purple-dark/80 hover:text-brand-purple-dark hover:border-white/25"
                  >
                    Edit
                  </button>
                )}
                {t.isOverridden && !isEditing && (
                  <button
                    ...
                    className="text-[11px] px-3 py-1.5 rounded-md border border-pink-200 text-brand-purple-dark/80 hover:text-brand-purple-dark/80 hover:border-white/25 disabled:opacity-40"
                  >
                    Reset
                  </button>
                )}
            ...
            {isEditing && (
              <div className="space-y-2 pt-2 border-t border-pink-200/50">
                <label className="block">
                  <span className="block text-[10px] tracking-[0.18em] uppercase text-brand-purple-dark/80 mb-1 inline-flex items-center gap-1.5">
                    Subject
                  ...
                  <input
                    ...
                    className="w-full bg-white/5 border border-pink-200 rounded-md px-3 py-2 text-[12px] text-brand-purple-dark font-mono focus:outline-none focus:border-cyan-400/40"
                  />
                </label>
                <label className="block">
                  <span className="block text-[10px] tracking-[0.18em] uppercase text-brand-purple-dark/80 mb-1 inline-flex items-center gap-1.5">
                ...
                <div className="flex items-center justify-end gap-2 pt-1">
                  <button
                    ...
                    onClick={cancelEdit}
                    disabled={busy}
                    className="text-[11px] px-3 py-1.5 rounded-md text-brand-purple-dark/80 hover:text-brand-purple-dark disabled:opacity-40"
                  >
                    Cancel
                  </button>
                  <button
...
function ComposePanel() {
  ...
  return (
    <section className="rounded-xl border border-pink-200/50 bg-white/[0.02] p-4 space-y-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <label className="block">
          <span className="block text-[10px] tracking-[0.18em] uppercase text-brand-purple-dark/80 mb-1">Recipient</span>
          <input
            type="email"
            value={to}
            onChange={e => setTo(e.target.value)}
            placeholder="them@example.com"
            className="w-full bg-white/5 border border-pink-200 rounded-md px-3 py-2 text-[12px] text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-cyan-400/40"
          />
        </label>
        <label className="block">
          <span className="block text-[10px] tracking-[0.18em] uppercase text-brand-purple-dark/80 mb-1">Prefill from template</span>
          <select
            value={templateId}
            onChange={e => applyTemplate(e.target.value)}
            className="w-full bg-white/5 border border-pink-200 rounded-md px-3 py-2 text-[12px] text-brand-purple-dark focus:outline-none focus:border-cyan-400/40"
          >
      ...
      <label className="block">
        <span className="block text-[10px] tracking-[0.18em] uppercase text-brand-purple-dark/80 mb-1">Subject</span>
        <input
          type="text"
          value={subject}
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="block text-[10px] tracking-[0.18em] uppercase text-brand-purple-dark/80">Body</span>
              ...
              <button
                ...
                className={`text-[10px] tracking-wider uppercase px-2 py-0.5 rounded ${
                  bodyMode === m
                    ...
                    : "text-brand-purple-dark/80 hover:text-brand-purple-dark/80"
                }`}
              >
                {m}
              </button>
        ...
        <textarea
          ...
          className="w-full bg-white/5 border border-pink-200 rounded-md px-3 py-2 text-[12px] text-brand-purple-dark placeholder:text-brand-purple-dark/80 font-mono focus:outline-none focus:border-cyan-400/40"
        />
      </div>
...
function LogPanel() {
  ...
  if (loading) return <PageSpinner wrap={false} />;
  if (log.length === 0) return <p className="text-[12px] text-brand-purple-dark/80">No email sent yet from this org.</p>;

  return (
    <section className="space-y-1">
      {log.map(e => (
        <article key={e.id} className="rounded-lg border border-pink-200/50 bg-white/[0.02] px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[12px] text-brand-purple-dark truncate">{e.subject}</p>
            <p className="text-[10px] text-brand-purple-dark/80 truncate">
              {e.to.join(", ")} · {e.provider}{e.templateId ? ` · ${e.templateId}` : ""}
            </p>
            {e.error && <p className="text-[10px] text-red-300/80 mt-0.5">{e.error}</p>}
          </div>
          <span className="text-[10px] text-brand-purple-dark/80 tabular-nums shrink-0">
...
function TestSendPanel() {
  ...
  return (
    <section className="rounded-xl border border-pink-200/50 bg-white/[0.02] p-4 space-y-3 max-w-md">
      <p className="text-[12px] text-brand-purple-dark/80">Send a test email to verify your provider config.</p>
      <input
        type="email"
        value={to}
        onChange={e => setTo(e.target.value)}
