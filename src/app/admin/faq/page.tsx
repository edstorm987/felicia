    ...
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-4xl">
      ...
      <div className="flex items-end justify-between gap-3 flex-wrap">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Frequently asked questions</h1>
          <p className="text-brand-purple-dark/80 text-sm mt-1">{totalItems} answer{totalItems === 1 ? "" : "s"} across {groups.length} section{groups.length === 1 ? "" : "s"}.</p>
        </div>
        <div className="flex gap-2">
          <Link href="/faq" target="_blank" className="text-xs px-3 py-2 rounded-lg border border-pink-200 text-brand-purple-dark/80 hover:text-brand-purple-dark">View live →</Link>
          <button
    ...
    <div className="rounded-2xl border border-pink-200/50 bg-white overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-pink-200/50 bg-pink-50/50/40">
        ...
        <input
          ...
          className="flex-1 bg-transparent text-sm font-display font-semibold text-brand-purple-dark focus:outline-none placeholder:text-brand-purple-dark/80"
        />
        <div className="flex items-center gap-1 text-[11px]">
          <button disabled={!canUp} onClick={() => moveGroup(group.id, -1)} aria-label="Move section up" title="Move up" className="px-2 py-0.5 text-brand-purple-dark/80 hover:text-brand-purple-dark disabled:opacity-25">↑</button>
          <button disabled={!canDown} onClick={() => moveGroup(group.id, 1)} aria-label="Move section down" title="Move down" className="px-2 py-0.5 text-brand-purple-dark/80 hover:text-brand-purple-dark disabled:opacity-25">↓</button>
          <button
            onClick={async () => {
              if (await confirm({
            ...
            className="px-2 py-0.5 text-brand-purple-dark/80 hover:text-brand-orange"
          >
            Delete
          </button>
        </div>
      ...
      <div className="p-4 space-y-3">
        {group.items.length === 0 && (
          <p className="text-[11px] text-brand-purple-dark/80 italic">No questions in this section yet.</p>
        )}
    ...
    <div className="rounded-lg border border-pink-200/50 bg-pink-50 p-3 space-y-2">
      <div className="flex items-center gap-2">
        ...
        <input
          ...
          aria-label="Question"
          className="flex-1 bg-transparent text-sm font-medium text-brand-purple-dark focus:outline-none placeholder:text-brand-purple-dark/80"
        />
        <div className="flex items-center gap-1 text-[11px]">
          <button disabled={!canUp} onClick={() => moveItem(groupId, item.id, -1)} aria-label="Move question up" title="Move up" className="px-1.5 text-brand-purple-dark/80 hover:text-brand-purple-dark disabled:opacity-25">↑</button>
          <button disabled={!canDown} onClick={() => moveItem(groupId, item.id, 1)} aria-label="Move question down" title="Move down" className="px-1.5 text-brand-purple-dark/80 hover:text-brand-purple-dark disabled:opacity-25">↓</button>
          <button onClick={() => deleteItem(groupId, item.id)} aria-label="Delete question" title="Delete" className="px-1.5 text-brand-purple-dark/80 hover:text-brand-orange">×</button>
        </div>
      </div>
      ...
      <textarea
        ...
        aria-label="Answer"
        rows={3}
        className="w-full bg-transparent text-xs text-brand-purple-dark/80 focus:outline-none resize-y leading-relaxed placeholder:text-brand-purple-dark/80"
      />
    </div>
  );
}
