...
const STATUS_STYLE: Record<FlagStatus, string> = {
  ...
  off:     "bg-white/8 text-brand-purple-dark/80",
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-2">
            <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Feature flags</h1>
        ...
        {categories.map(cat => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs whitespace-nowrap transition-colors ${
              filter === cat ? "bg-brand-orange text-white font-semibold" : "bg-white/5 text-brand-purple-dark/80 hover:text-brand-purple-dark"
            }`}
          >
          ...
          <div key={flag.id} className="flex items-start gap-4 px-5 py-4 hover:bg-white/[0.02]">
              ...
              {(["on", "off", "rollout"] as FlagStatus[]).map(s => (
                <button
                  ...
                  className={`text-[10px] px-2 py-0.5 rounded-full font-semibold transition-colors ${
                    flag.status === s ? STATUS_STYLE[s] : "bg-white/5 text-brand-purple-dark/80 hover:text-brand-purple-dark/80"
                  }`}
                >
                  {STATUS_LABELS[s]}
                </button>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap mb-0.5">
                <span className="text-sm font-medium text-brand-purple-dark">{flag.name}</span>
                <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-white/8 text-brand-purple-dark/80">
                  {CATEGORY_LABELS[flag.category]}
                </span>
                {flag.isBuiltIn && <span className="text-[10px] text-brand-purple-dark/80">built-in</span>}
              </div>
              <p className="text-xs text-brand-purple-dark/80">{flag.description}</p>
              <p className="text-[11px] text-brand-purple-dark/80 font-mono mt-0.5">{flag.id}</p>
              ...
              {flag.status === "rollout" && (
                <div className="flex items-center gap-3 mt-2">
                  ...
                  <span className="text-[11px] text-brand-purple-dark/80">of users</span>
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              {!flag.isBuiltIn && (
                <button
                  ...
                  className="text-[11px] text-brand-purple-dark/80 hover:text-brand-orange"
                >
                  Delete
                </button>
              )}
      ...
      <div className="grid grid-cols-3 gap-3">
        {(["on", "off", "rollout"] as FlagStatus[]).map(s => (
          <div key={s} className="rounded-xl border border-white/8 bg-white px-4 py-3 text-center">
            ...
            <p className="text-xs text-brand-purple-dark/80 mt-0.5">{STATUS_LABELS[s]}</p>
          </div>
        ))}
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-md bg-pink-50/50 border border-pink-200 rounded-2xl overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          <div>
            <label className="block text-xs text-brand-purple-dark/80 mb-1.5">Name</label>
            <input value={name} onChange={e => setName(e.target.value)}
              className="w-full bg-white/5 border border-pink-200 rounded-xl px-3 py-2.5 text-sm text-brand-purple-dark focus:outline-none focus:border-brand-orange/50" />
          </div>
          <div>
            <label className="block text-xs text-brand-purple-dark/80 mb-1.5">Description</label>
            <textarea value={description} onChange={e => setDescription(e.target.value)} rows={2}
              className="w-full bg-white/5 border border-pink-200 rounded-xl px-3 py-2.5 text-sm text-brand-purple-dark focus:outline-none focus:border-brand-orange/50 resize-none" />
          </div>
          <div>
            <label className="block text-xs text-brand-purple-dark/80 mb-1.5">Category</label>
            <select value={category} onChange={e => setCategory(e.target.value as FlagCategory)}
              className="w-full bg-white/5 border border-pink-200 rounded-xl px-3 py-2.5 text-sm text-brand-purple-dark focus:outline-none">
          ...
          {/* Per-user override */}
          <div>
            <label className="block text-xs text-brand-purple-dark/80 mb-1.5">Add user override</label>
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-white/5 border border-pink-200 rounded-xl px-3 py-2 text-sm text-brand-purple-dark focus:outline-none focus:border-brand-orange/50"
              />
              <select
                value={emailEnabled ? "on" : "off"}
                onChange={e => setEmailEnabled(e.target.value === "on")}
                className="bg-white/5 border border-pink-200 rounded-xl px-2 py-2 text-sm text-brand-purple-dark focus:outline-none"
              >
