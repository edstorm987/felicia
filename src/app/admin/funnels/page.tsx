        ...
        <div className="p-5 space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-semibold uppercase tracking-[0.15em] text-brand-purple-dark/80">Steps</p>
            ...
            {steps.length === 0 && (
              <div className="text-center py-6 border border-dashed border-pink-200 rounded-xl">
                <p className="text-xs text-brand-purple-dark/80">Add at least 2 steps to define the funnel flow.</p>
                ...
                <div key={step.id} className="flex gap-2 items-start">
                  ...
                  <div className="flex-1 grid grid-cols-[auto_1fr_1fr] gap-2 items-center">
                    <select value={step.type} onChange={(e) => patchStep(step.id, { type: e.target.value as StepType })}
                      className="text-sm bg-white/5 border border-pink-200 rounded-xl px-2 py-2 text-brand-purple-dark focus:outline-none focus:border-brand-orange/50">
    ...
    <div className="p-6 sm:p-8 lg:p-10 max-w-5xl space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Funnels</h1>
          <p className="text-brand-purple-dark/80 text-sm mt-1">Build multi-step funnels and track drop-off at each stage.</p>
        </div>
      ...
      {funnels.length === 0 && (
        <div className="text-center py-16 border border-dashed border-pink-200 rounded-2xl">
          <p className="text-brand-purple-dark/80 text-sm">No funnels yet.</p>
              ...
              <div className="flex flex-wrap items-center gap-3 px-5 py-4 border-b border-pink-200/50">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-semibold text-brand-purple-dark">{funnel.name}</h3>
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${STATUS_STYLE[funnel.status]}`}>{funnel.status}</span>
                  </div>
                  {funnel.description && <p className="text-xs text-brand-purple-dark/80 mt-0.5">{funnel.description}</p>}
                  <p className="text-xs text-brand-purple-dark/80 mt-0.5">{funnel.steps.length} steps · {totalReached} entered · {cr}% overall conversion</p>
                </div>
                <div className="flex gap-2 items-center">
                  ...
                  <button onClick={() => setModal(funnel)} className="text-xs px-3 py-1.5 border border-pink-200 text-brand-purple-dark/80 hover:text-brand-purple-dark rounded-lg">Edit</button>
                    ...
                    return (
                      <div key={step.id} className="flex items-center">
                        <div className="flex flex-col items-center text-center w-32">
                          ...
                          <p className="text-xs font-medium text-brand-purple-dark/80 leading-tight">{step.name}</p>
                          <p className="text-[10px] text-brand-purple-dark/80 mt-0.5 leading-tight">{step.path}</p>
                          <p className="text-xs font-mono text-brand-purple-dark/80 mt-1.5">{step.reached.toLocaleString()}</p>
                          <p className="text-[10px] text-brand-purple-dark/80">reached</p>
                        </div>
...
function F({ label, children }: { label?: string; children: React.ReactNode }) {
  return (
    <div>
      {label && <label className="block text-xs text-brand-purple-dark/80 mb-1.5">{label}</label>}
      {children}
    </div>
  );
}
