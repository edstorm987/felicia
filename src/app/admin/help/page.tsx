  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 max-w-4xl space-y-6">
      <header>
        ...
        <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">How everything works</h1>
        <p className="text-brand-purple-dark/80 text-sm mt-1 max-w-prose leading-relaxed">
          Every admin page that has a written explainer. Search by topic (&ldquo;Stripe&rdquo;, &ldquo;backup&rdquo;, &ldquo;refund&rdquo;) or browse the list. The floating <kbd className="px-1.5 py-0.5 rounded bg-white/8 text-brand-purple-dark/80 text-[10px] font-mono">?</kbd> button on any admin page opens the same content scoped to where you are.
        </p>
      </header>
      ...
      <input
        ...
        className="w-full bg-white/5 border border-pink-200 rounded-xl px-4 py-3 text-sm text-brand-purple-dark focus:outline-none focus:border-brand-orange/50"
      />
            ...
            >
              <Link href={route} className="block">
                <div className="flex items-baseline justify-between gap-3 mb-1 flex-wrap">
                  <h2 className="text-[15px] font-display text-brand-purple-dark">{doc.title}</h2>
                  <code className="text-[10px] font-mono text-brand-purple-dark/80">{route}</code>
                </div>
                {doc.intro && (
                  <p className="text-[12px] text-brand-purple-dark/80 leading-relaxed">{doc.intro}</p>
                )}
              </Link>
              {doc.sections.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {doc.sections.map((s, i) => (
                    <span
                      key={i}
                      className="text-[10px] uppercase tracking-[0.18em] text-brand-purple-dark/80 px-2 py-0.5 rounded-full bg-white/5"
                    >
                      {s.heading}
                    </span>
                  ))}
      ...
      <p className="text-[11px] text-brand-purple-dark/80 mt-8 max-w-prose leading-relaxed">
        Don&rsquo;t see what you&rsquo;re looking for? Each page&rsquo;s ? button gives the most up-to-date guidance for that page. If a page has no doc yet, mention it to your admin — adding one is a few lines in <code className="font-mono text-brand-purple-dark/80">src/lib/admin/helpDocs.ts</code>.
      </p>
    </div>
  );
}
