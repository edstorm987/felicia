  ...
  return (
    <main className="max-w-4xl mx-auto px-6 py-8 space-y-6">
      <header>
        ...
        <h1 className="font-display text-3xl text-brand-purple-dark">i18n</h1>
        <p className="text-[12px] text-brand-purple-dark/80 mt-1">Edit translations for each enabled locale. Default locale fills in for missing keys.</p>
      </header>

      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-[11px] text-brand-purple-dark/80">Locale</span>
        {data.locales.map(l => (
          <button
            key={l}
            onClick={() => void load(l)}
            className={`px-2.5 py-1 rounded-md text-[11px] transition-colors ${
              l === locale
                ...
                : "bg-white/5 text-brand-purple-dark/80 hover:text-brand-purple-dark"
            }`}
          >
      ...
      <section className="rounded-xl border border-pink-200/50 bg-white/[0.02] divide-y divide-white/5">
        {data.keys.length === 0 && (
          <div className="px-4 py-8 text-center text-[12px] text-brand-purple-dark/80">
        ...
        {data.keys.map(key => {
          ...
          return (
            <div key={key} className="px-4 py-2.5 flex items-center gap-3">
              <span className={`text-[11px] font-mono w-56 truncate ${empty ? "text-amber-300/65" : "text-brand-purple-dark/80"}`}>{key}</span>
              <input
                type="text"
                value={value}
                onChange={e => setEdits(prev => ({ ...prev, [key]: e.target.value }))}
                className="flex-1 bg-white/5 border border-pink-200 rounded-md px-2.5 py-1 text-[12px] text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-cyan-400/40"
      ...
      <section className="rounded-xl border border-pink-200/50 bg-white/[0.02] p-4 space-y-2">
        <h3 className="text-[10px] tracking-[0.28em] uppercase text-brand-purple-dark/80">Add key</h3>
        <div className="flex gap-2">
          <input
            ...
            className="w-56 bg-white/5 border border-pink-200 rounded-md px-2.5 py-1 text-[12px] font-mono text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-cyan-400/40"
          />
          <input
            type="text"
            value={newValue}
