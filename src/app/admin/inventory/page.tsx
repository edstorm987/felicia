    ...
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-7xl">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Stock</h1>
      ...
      <div className="rounded-2xl border border-white/8 bg-white overflow-hidden">
        ...
        <div className="divide-y divide-white/5">
              ...
              <div key={i.sku} className="px-5 py-4">
                <div className="md:grid md:grid-cols-[1fr_140px_80px_80px_80px_80px_100px_140px] md:gap-3 md:items-center flex flex-col gap-2">
                  <div className="min-w-0">
                    <p className="text-sm text-brand-purple-dark truncate">{i.name}</p>
                    <p className="text-[11px] text-brand-purple-dark/80 capitalize">{i.range} · £{i.price.toFixed(2)}</p>
                  </div>
                  <span className="text-xs text-brand-purple-dark/80 font-mono truncate">{i.sku}</span>
                  <span className={`text-sm md:text-right ${i.unlimited ? "text-brand-amber" : "text-brand-purple-dark"}`}>
                    {i.unlimited ? "∞" : i.onHand}
                  </span>
                  <span className="text-sm text-brand-purple-dark/80 md:text-right">{i.unlimited ? "—" : i.reserved}</span>
                  <span className={`text-sm md:text-right ${out ? "text-brand-orange font-semibold" : low ? "text-brand-amber font-semibold" : i.unlimited ? "text-brand-amber" : "text-brand-purple-dark"}`}>
          ...
          {filtered.length === 0 && (
            <p className="text-brand-purple-dark/80 text-sm py-8 text-center">No SKUs match.</p>
          )}
        </div>
      </div>
    </div>
