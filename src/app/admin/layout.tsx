...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
      ...
      return (
        <div className="min-h-screen bg-pink-50 flex items-center justify-center px-6">
          <div className="max-w-md text-center">
            ...
            <h1 className="font-display text-3xl text-brand-purple-dark mb-3">Admin only</h1>
            <p className="text-sm text-brand-purple-dark/80 mb-6">
  ...
  return (
    <div data-admin-panel className="min-h-screen bg-pink-50 text-brand-purple-dark flex">
      ...
      <aside data-admin-sidebar className="hidden md:flex w-60 lg:w-64 shrink-0 flex-col bg-pink-50/50 border-r border-pink-200/50">
        <Link href="/" className="px-6 py-6 border-b border-pink-200/50 flex items-center gap-3">
          ...
          <div className="min-w-0">
            <span className="font-display text-base font-bold text-brand-purple-dark block truncate">
        ...
        <div className="px-3 pb-3 space-y-1.5">
          ...
          <button
            ...
            className="w-full flex items-center justify-between gap-2 px-3 py-2 rounded-lg border border-pink-200 bg-white/[0.02] hover:bg-white/[0.04] text-xs text-brand-purple-dark/80 hover:text-brand-purple-dark transition-colors"
          >
        ...
        <div className="p-4 border-t border-pink-200/50 text-[11px] text-brand-purple-dark/80 space-y-1">
          ...
          <div className="flex items-center gap-3 pt-1 flex-wrap">
            ...
            <Link href="/" className="hover:text-brand-purple-dark transition-colors">View site →</Link>
            {branding.githubRepoUrl && (
              <a href={branding.githubRepoUrl} target="_blank" rel="noopener noreferrer" className="hover:text-brand-purple-dark transition-colors" title="View repository">
                Repo ↗
              </a>
            )}
          </div>
      ...
      {/* Mobile top bar */}
      <div className="md:hidden fixed top-0 inset-x-0 z-30 bg-pink-50/50 border-b border-pink-200/50 px-4 py-3 flex items-center justify-between">
        ...
        <Link href="/" className="text-xs text-brand-purple-dark/80">Site →</Link>
      </div>
...
function TopLevelView({
  ...
  return (
    <div className="space-y-3">
      <p className="px-1 text-[10px] tracking-[0.22em] uppercase text-brand-purple-dark/80">Panels</p>
        ...
        {panels.map(panel => (
          ...
          >
            ...
            <span className="flex-1 min-w-0">
              <span className="block text-sm font-semibold text-brand-purple-dark">{panel.label}</span>
              {panel.description && (
                <span className="block text-[11px] text-brand-purple-dark/80 truncate">{panel.description}</span>
              )}
            </span>
            <span className="text-brand-purple-dark/80 group-hover:text-brand-orange transition-colors">→</span>
          </button>
        ))}
      </div>
      ...
      {Object.keys(customGroups).length > 0 && (
        <div className="space-y-1.5 pt-2">
          <p className="px-1 text-[10px] tracking-[0.22em] uppercase text-brand-purple-dark/80">Custom</p>
          {Object.entries(customGroups).map(([groupLabel, tabs]) => (
            <div key={groupLabel} className="space-y-0.5">
              {groupLabel !== "Custom" && (
                <p className="px-3 text-[10px] tracking-[0.22em] uppercase text-brand-purple-dark/80 mb-1 mt-1">{groupLabel}</p>
              )}
              {tabs.map(t => {
                ...
                if (t.openInNewTab) {
                  return (
                    <a
                      ...
                      className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors text-brand-purple-dark/80 hover:bg-white/5 hover:text-brand-purple-dark border border-transparent"
                    >
                      <span>{t.icon}</span>
                      <span className="flex-1 truncate">{t.label}</span>
                      <span className="text-[10px] text-brand-purple-dark/80">↗</span>
                    </a>
                  );
                }
                return (
                  <Link
                    ...
                    className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors ${
                      active
                        ? "bg-brand-orange/15 text-brand-purple-dark border border-brand-orange/30"
                        : "text-brand-purple-dark/80 hover:bg-white/5 hover:text-brand-purple-dark border border-transparent"
                    }`}
                  >
                    <span>{t.icon}</span>
                    <span className="flex-1 truncate">{t.label}</span>
...
function PanelView({
  ...
  return (
    <div className="space-y-2">
      <button
        onClick={onBack}
        className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs text-brand-purple-dark/80 hover:text-brand-purple-dark hover:bg-white/5 transition-colors"
      >
        <span>←</span>
        <span>All panels</span>
      </button>

      <div className="px-3 pt-1 pb-2 flex items-center gap-2 border-b border-pink-200/50">
        <span className="text-xl">{panel.icon}</span>
        <div className="min-w-0">
          <p className="text-sm font-semibold text-brand-purple-dark truncate">{panel.label}</p>
          {panel.description && (
            <p className="text-[10px] text-brand-purple-dark/80 truncate">{panel.description}</p>
          )}
        </div>
      </div>
  ...
  return (
    <div>
      <button
        ...
        className="w-full flex items-center justify-between pr-3 py-2 rounded-lg text-sm text-brand-purple-dark/80 hover:bg-white/5 hover:text-brand-purple-dark transition-colors"
        style={{ paddingLeft: indentPx(depth) }}
        title={group.label}
      >
        <span className="flex items-center gap-2 min-w-0">
          <span className="text-brand-purple-dark/80 text-[10px] w-3 inline-block shrink-0">{open ? "▾" : "▸"}</span>
...
function MobileTabBar({
  ...
  return (
    <nav className="md:hidden fixed bottom-0 inset-x-0 z-30 bg-pink-50/50 border-t border-pink-200/50 grid" style={{ gridTemplateColumns: `repeat(${visible.length}, minmax(0, 1fr))` }}>
      {visible.map(panel => {
        ...
        return (
          <button
            key={panel.id}
            onClick={() => onPick(panel.id)}
            className={`relative text-center py-2.5 text-[11px] flex flex-col items-center gap-0.5 ${active ? "text-brand-orange" : "text-brand-purple-dark/80"}`}
          >
