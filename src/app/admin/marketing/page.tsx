  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-7xl">
      <div>
        ...
        <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Sources, discounts &amp; affiliates</h1>
        ...
        {([
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-medium transition-colors ${
              tab === t.id ? "bg-brand-orange/20 text-brand-purple-dark" : "text-brand-purple-dark/80 hover:text-brand-purple-dark"
            }`}
          >
            {t.label}
          </button>
    ...
    <div className={`p-4 rounded-xl border ${src.archived ? "bg-white/[0.02] border-pink-200/50" : "bg-pink-50 border-white/8"}`}>
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="min-w-0">
          ...
          <p className="text-[11px] text-brand-purple-dark/80 truncate">
            {src.trackingSlug ? <>Slug: <span className="font-mono text-brand-amber">{src.trackingSlug}</span></> : "Manual selection only"}
          </p>
        </div>
        <div className="flex gap-1.5 shrink-0">
          <button onClick={() => setEditing(true)} className="text-[10px] text-brand-purple-dark/80 hover:text-brand-purple-dark px-2 py-1 rounded border border-pink-200">Edit</button>
          {!src.archived ? (
            <button onClick={() => updateSource(src.id, { archived: true })} className="text-[10px] text-brand-purple-dark/80 hover:text-brand-orange px-2 py-1 rounded border border-pink-200">Archive</button>
          ) : (
          ...
          <button onClick={async () => { if (await confirm({ title: `Delete source "${src.label}"?`, message: "Existing orders attributed to it keep the reference but the source is gone from the picker.", danger: true, confirmLabel: "Delete" })) deleteSource(src.id); }} className="text-[10px] text-brand-purple-dark/80 hover:text-brand-orange px-2 py-1 rounded border border-pink-200">×</button>
        </div>
      </div>
      <div className="flex gap-4 pt-2 border-t border-pink-200/50 mt-2">
        <span className="text-[11px] text-brand-purple-dark/80">{count} orders</span>
...
function SourceForm({ initial, onClose }: { initial?: OrderSource; onClose: () => void }) {
  ...
  return (
    <div className="p-4 rounded-xl border border-brand-amber/25 bg-brand-amber/5 space-y-2">
      <input value={label} onChange={e => setLabel(e.target.value)} placeholder="Label (e.g. TikTok ad — Apr 2026)" className="w-full bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-sm text-brand-purple-dark" />
      <input value={slug} onChange={e => setSlug(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ""))} placeholder="Tracking slug (optional, used in ?src=…)" className="w-full bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-sm text-brand-purple-dark font-mono" />
      <input value={desc} onChange={e => setDesc(e.target.value)} placeholder="Description (optional)" className="w-full bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-xs text-brand-purple-dark/80" />
      <div className="flex gap-2">
        <button onClick={save} className="text-xs px-3 py-1.5 rounded-lg bg-brand-orange text-white">Save</button>
        <button onClick={onClose} className="text-xs px-3 py-1.5 rounded-lg border border-pink-200 text-brand-purple-dark/80">Cancel</button>
      </div>
    </div>
  );
}
...
function CopyRow({ label, url }: { label: string; url: string }) {
  ...
  return (
    <div className="flex items-center gap-3 p-2.5 rounded-lg bg-pink-50 border border-pink-200/50">
      <span className="text-xs text-brand-purple-dark/80 w-32 shrink-0 truncate">{label}</span>
      <code className="text-[11px] text-brand-amber flex-1 truncate font-mono">{url}</code>
      <button
        onClick={() => { navigator.clipboard.writeText(url); setCopied(true); setTimeout(() => setCopied(false), 1500); }}
        className="text-[10px] px-2 py-1 rounded bg-white/5 text-brand-purple-dark/80 hover:bg-white/10"
      >
        {copied ? "Copied" : "Copy"}
      </button>
    </div>
  ...
  return (
    <div className="space-y-5">
      <div className="flex justify-between">
        <p className="text-sm text-brand-purple-dark/80">{discounts.filter(d => !d.archived).length} active · {discounts.filter(d => d.archived).length} archived</p>
      ...
      <div className="space-y-2">
        ...
        {discounts.length === 0 && <p className="text-sm text-brand-purple-dark/80">No discount codes yet.</p>}
      </div>
    </div>
  );
}
    ...
    <div className={`p-4 rounded-xl border ${d.archived || expired || usedUp ? "bg-white/[0.02] border-pink-200/50" : "bg-white border-white/8"}`}>
      <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_auto_auto_auto] gap-3 md:items-center">
        <div className="min-w-0">
          ...
          {d.note && <p className="text-[11px] text-brand-purple-dark/80 truncate">{d.note}</p>}
        </div>
        <span className="text-xs text-brand-purple-dark/80 font-display">{valueLabel}</span>
        <span className="text-[11px] text-brand-purple-dark/80">
          {d.uses} uses{d.usageLimit ? ` / ${d.usageLimit}` : ""}
          {stats && stats.count > 0 ? ` · £${stats.revenue.toFixed(0)}` : ""}
        </span>
        <span className="text-[10px] tracking-widest uppercase">
          {d.archived ? <span className="text-brand-purple-dark/80">archived</span> :
           expired   ? <span className="text-brand-purple-dark/80">expired</span> :
           usedUp    ? <span className="text-brand-purple-dark/80">limit reached</span> :
                       <span className="text-brand-amber">active</span>}
        </span>
        <div className="flex gap-1.5">
          <button onClick={() => setEditing(true)} className="text-[10px] text-brand-purple-dark/80 hover:text-brand-purple-dark px-2 py-1 rounded border border-pink-200">Edit</button>
          {!d.archived ? (
            <button onClick={() => updateDiscount(d.code, { archived: true })} className="text-[10px] text-brand-purple-dark/80 hover:text-brand-orange px-2 py-1 rounded border border-pink-200">Archive</button>
          ) : (
          ...
          <button onClick={async () => { if (await confirm({ title: `Delete code "${d.code}"?`, message: "Existing orders that used the code keep their discount, but no new uses can apply it.", danger: true, confirmLabel: "Delete" })) deleteDiscount(d.code); }} className="text-[10px] text-brand-purple-dark/80 hover:text-brand-orange px-2 py-1 rounded border border-pink-200">×</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="p-4 rounded-xl border border-brand-amber/25 bg-brand-amber/5 space-y-2">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        <input value={code} onChange={e => setCode(e.target.value.toUpperCase())} placeholder="Code (e.g. SUMMER20)" disabled={!!initial} className="bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-sm text-brand-purple-dark font-mono disabled:opacity-50" />
        <select value={type} onChange={e => setType(e.target.value as DiscountType)} className="bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-sm text-brand-purple-dark">
      ...
      {type !== "freeship" && (
        <input value={value} onChange={e => setValue(e.target.value)} type="number" placeholder={type === "percent" ? "% off" : "£ off"} className="w-full bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-sm text-brand-purple-dark" />
      )}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        <input value={minSubtotal} onChange={e => setMinSubtotal(e.target.value)} type="number" placeholder="Min spend (£)" className="bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-xs text-brand-purple-dark" />
        <input value={usageLimit} onChange={e => setUsageLimit(e.target.value)} type="number" placeholder="Usage limit" className="bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-xs text-brand-purple-dark" />
        <input value={expiresAt} onChange={e => setExpiresAt(e.target.value)} type="date" className="bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-xs text-brand-purple-dark" />
      </div>
      <input value={note} onChange={e => setNote(e.target.value)} placeholder="Internal note (optional)" className="w-full bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-xs text-brand-purple-dark/80" />
      <div className="flex gap-2">
...
function AffiliatesTab({ orders }: { orders: Order[] }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex justify-between">
        <p className="text-sm text-brand-purple-dark/80">{affs.filter(a => !a.archived).length} active partners</p>
      ...
      <div className="space-y-3">
        ...
        {affs.length === 0 && <p className="text-sm text-brand-purple-dark/80">No affiliates yet. Add a partner to start tracking commissions.</p>}
      </div>
    </div>
  );
}
    ...
    <div className={`p-5 rounded-xl border ${a.archived ? "bg-white/[0.02] border-pink-200/50" : "bg-white border-white/8"}`}>
      <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
        <div>
          ...
          <p className="text-[11px] text-brand-purple-dark/80">{a.email}</p>
        </div>
        <div className="flex gap-1.5">
          <button onClick={() => setEditing(true)} className="text-[10px] text-brand-purple-dark/80 hover:text-brand-purple-dark px-2 py-1 rounded border border-pink-200">Edit</button>
          {owed > 0 && (
          ...
          {!a.archived ? (
            <button onClick={() => updateAffiliate(a.id, { archived: true })} className="text-[10px] text-brand-purple-dark/80 hover:text-brand-orange px-2 py-1 rounded border border-pink-200">Archive</button>
          ) : (
          ...
          <button onClick={async () => { if (await confirm({ title: `Delete affiliate "${a.name}"?`, message: "Their click + commission history is removed.", danger: true, confirmLabel: "Delete" })) deleteAffiliate(a.id); }} className="text-[10px] text-brand-purple-dark/80 hover:text-brand-orange px-2 py-1 rounded border border-pink-200">×</button>
        </div>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 text-[11px]">
        ...
        <Stat label="Owed" value={<span className={owed > 0 ? "text-brand-amber" : "text-brand-purple-dark/80"}>£{owed.toFixed(2)}</span>} />
      </div>
      {a.trackingSlug && (
        <div className="mt-3">
...
function Stat({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div>
      <p className="text-[9px] tracking-[0.22em] uppercase text-brand-purple-dark/80 mb-0.5">{label}</p>
      <div className="text-brand-purple-dark/80">{value}</div>
    </div>
  );
}
...
function AffiliateForm({ initial, onClose }: { initial?: Affiliate; onClose: () => void }) {
  ...
  return (
    <div className="p-4 rounded-xl border border-brand-amber/25 bg-brand-amber/5 space-y-2">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        <input value={name} onChange={e => setName(e.target.value)} placeholder="Name" className="bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-sm text-brand-purple-dark" />
        <input value={email} onChange={e => setEmail(e.target.value)} type="email" placeholder="Email" className="bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-sm text-brand-purple-dark" />
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        <input value={commissionPct} onChange={e => setCommissionPct(e.target.value)} type="number" placeholder="Commission %" className="bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-sm text-brand-purple-dark" />
        <input value={code} onChange={e => setCode(e.target.value.toUpperCase())} placeholder="Linked code (optional)" className="bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-sm text-brand-purple-dark font-mono" />
        <input value={slug} onChange={e => setSlug(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ""))} placeholder="Tracking slug (?aff=…)" className="bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-sm text-brand-purple-dark font-mono" />
      </div>
      <input value={payout} onChange={e => setPayout(e.target.value)} placeholder="Payout method (e.g. Bank transfer, PayPal)" className="w-full bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-xs text-brand-purple-dark/80" />
      <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Notes (optional)" rows={2} className="w-full bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-xs text-brand-purple-dark/80" />
      <div className="flex gap-2">
        <button onClick={save} className="text-xs px-3 py-1.5 rounded-lg bg-brand-orange text-white">{initial ? "Save" : "Create"}</button>
        <button onClick={onClose} className="text-xs px-3 py-1.5 rounded-lg border border-pink-200 text-brand-purple-dark/80">Cancel</button>
      </div>
