    ...
    <div className="p-6 sm:p-8 lg:p-10 max-w-5xl space-y-6">
      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Members</h1>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <input
          ...
          className="flex-1 min-w-[12rem] bg-white/5 border border-pink-200 rounded-xl px-3 py-2 text-sm text-brand-purple-dark focus:outline-none focus:border-brand-orange/50"
        />
        <select
          value={tierFilter}
          onChange={e => setTierFilter(e.target.value)}
          className="bg-white/5 border border-pink-200 rounded-xl px-3 py-2 text-sm text-brand-purple-dark focus:outline-none focus:border-brand-orange/50"
        >
          <option value="all">All tiers</option>
          {tiers.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
        </select>
        <label className="flex items-center gap-2 text-[12px] text-brand-purple-dark/80 cursor-pointer select-none">
          <input
            type="checkbox"
          ...
          {filtered.map(m => (
            <li key={m.id} className="px-4 py-3 flex flex-wrap items-center gap-3">
              <div className="flex-1 min-w-[12rem]">
                <div className="text-[13px] text-brand-purple-dark">{m.name ?? m.email}</div>
                {m.name && (
                  <Link
                    href={`/admin/customers/${encodeURIComponent(m.email)}`}
                    className="text-[11px] text-brand-purple-dark/80 hover:text-cyan-200 font-mono"
                  >
                    {m.email}
                  </Link>
                )}
              ...
              <div className="text-[11px] text-brand-purple-dark/80 tabular-nums w-24">
              ...
              <select
                ...
                className="bg-white/5 border border-pink-200 rounded-lg px-2 py-1 text-[12px] text-brand-purple-dark focus:outline-none focus:border-brand-orange/50 disabled:opacity-50"
              >
