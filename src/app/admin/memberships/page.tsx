    ...
    <main className="max-w-4xl mx-auto px-6 py-8 space-y-6">
      <header className="flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">Members</h1>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {grouped.map(({ tier, members }) => (
            <section key={tier.id} className="rounded-xl border border-pink-200/50 bg-white/[0.02] p-4">
              <header className="flex items-center justify-between mb-3">
                <div>
                  <h2 className="text-[14px] text-brand-purple-dark font-medium">{tier.name}</h2>
                  <p className="text-[11px] text-brand-purple-dark/80 mt-0.5">{tier.benefits.join(" · ")}</p>
                </div>
                <span className="text-[11px] text-brand-purple-dark/80 tabular-nums">{members.length} members</span>
              </header>
              {members.length === 0 ? (
                <div className="py-3">
                  <p className="text-[11px] text-brand-purple-dark/80">No members in this tier yet.</p>
                  <p className="text-[10px] text-brand-purple-dark/80 mt-1">
                    Members join via the storefront <code className="font-mono text-brand-purple-dark/80">/membership</code> page or you can{" "}
                ...
                <ul className="space-y-1">
                  {members.slice(0, 20).map(m => (
                    <li key={m.id} className="flex items-center justify-between text-[12px]">
                      <div>
                        <span className="text-brand-purple-dark">{m.name ?? m.email}</span>
                        {m.name && <span className="text-brand-purple-dark/80 ml-2">{m.email}</span>}
                      </div>
