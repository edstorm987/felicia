...
function MembershipTiersPageInner() {
  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 max-w-5xl space-y-6">
      <Link href="/admin/memberships" className="text-xs text-brand-purple-dark/80 hover:text-brand-purple-dark inline-block">
        ← Memberships
      </Link>
      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Tiers</h1>
          <p className="text-brand-purple-dark/80 text-sm mt-1 max-w-prose leading-relaxed">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {dirty && (
            <button
              ...
              onClick={discard}
              disabled={saving}
              className="text-[11px] uppercase tracking-[0.2em] text-brand-purple-dark/80 hover:text-brand-purple-dark px-3 py-1.5 disabled:opacity-50"
            >
              Discard
            </button>
          )}
          ...
          <button
            ...
            className="text-[11px] uppercase tracking-[0.2em] text-brand-purple-dark bg-brand-amber hover:bg-brand-amber/90 rounded-lg px-3 py-1.5 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
            {saving ? "Saving…" : "Save"}
          </button>
        </div>
      ...
      {loading ? (
        ...
        <section className="rounded-2xl border border-pink-200/50 bg-white p-8 text-center">
          ...
          <button
            type="button"
            onClick={addTier}
            className="mt-4 text-[11px] uppercase tracking-[0.2em] text-brand-purple-dark bg-brand-amber hover:bg-brand-amber/90 rounded-lg px-3 py-1.5"
          >
            + Add first tier
          </button>
        </section>
      ...
      {!loading && draft.length > 0 && (
        <p className="text-[11px] text-brand-purple-dark/80 max-w-prose leading-relaxed">
    ...
    <li className="rounded-2xl border border-pink-200/50 bg-white p-5 space-y-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button
            ...
            onClick={() => onMove(-1)}
            disabled={index === 0}
            className="text-brand-purple-dark/80 hover:text-brand-purple-dark disabled:opacity-25 disabled:cursor-not-allowed w-7 h-7 flex items-center justify-center rounded hover:bg-white/5"
          >
            ↑
          </button>
          <button
          ...
          <button
            ...
            title="Delete tier"
            onClick={onDelete}
            className="text-brand-purple-dark/80 hover:text-red-400 w-7 h-7 flex items-center justify-center rounded hover:bg-white/5"
          >
            ×
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        >
          <input
            ...
            className="w-full bg-white/5 border border-pink-200 rounded-xl px-3 py-2 text-sm font-mono text-brand-purple-dark focus:outline-none focus:border-brand-orange/50"
          />
        </Field>
        ...
        >
          <div className="flex gap-2">
            <input
              ...
              placeholder="0"
              className="w-32 bg-white/5 border border-pink-200 rounded-xl px-3 py-2 text-sm text-brand-purple-dark focus:outline-none focus:border-brand-orange/50"
            />
            <input
              value={tier.currency ?? ""}
              ...
              maxLength={3}
              className="w-20 bg-white/5 border border-pink-200 rounded-xl px-3 py-2 text-sm uppercase font-mono text-brand-purple-dark focus:outline-none focus:border-brand-orange/50"
            />
          </div>
        </Field>
        ...
        >
          <select
            ...
            disabled={!isPaid}
            className="w-full bg-white/5 border border-pink-200 rounded-xl px-3 py-2 text-sm text-brand-purple-dark focus:outline-none focus:border-brand-orange/50 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <option value="">—</option>
      ...
      <Field label="Benefits" hint="One per line. Shown as bullet points on the join page.">
        <textarea
          ...
          className="w-full bg-white/5 border border-pink-200 rounded-xl px-3 py-2 text-sm text-brand-purple-dark focus:outline-none focus:border-brand-orange/50 leading-relaxed"
        />
      </Field>
    </li>
  );
...
function Field({ label, hint, tip, children }: { label: string; hint?: string; tip?: string; children: React.ReactNode }) {
  return (
    <label className="block space-y-1">
      <span className="text-[11px] uppercase tracking-[0.18em] text-brand-purple-dark/80 inline-flex items-center gap-1.5">
        {label}
        {tip && <Tip text={tip} />}
      </span>
      {children}
      {hint && <span className="block text-[11px] text-brand-purple-dark/80">{hint}</span>}
    </label>
  );
}
