    ...
    <main className="max-w-3xl mx-auto px-6 py-8 space-y-4">
      <header className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">Activity feed</h1>
          <p className="text-[12px] text-brand-purple-dark/80 mt-1">{unread} unread of {list.length}</p>
        </div>
      ...
      <div className="flex items-center gap-1 border-b border-pink-200/50 pb-2">
      ...
      {visible.length === 0 ? (
        <p className="text-[12px] text-brand-purple-dark/80">All caught up — nothing new.</p>
      ) : (
        <ul className="space-y-1">
          {visible.map(n => (
            <li key={n.id} className={`rounded-lg border p-3 flex items-start gap-3 ${
              n.read ? "border-pink-200/50 bg-white/[0.02] opacity-65" : "border-cyan-400/20 bg-cyan-500/5"
            }`}>
              <span className={`mt-1 w-2 h-2 rounded-full shrink-0 ${CATEGORY_DOTS[n.category] ?? "bg-white/30"}`} />
              <div className="flex-1 min-w-0">
                <p className="text-[13px] text-brand-purple-dark">{n.title}</p>
                <p className="text-[11px] text-brand-purple-dark/80 mt-0.5">{n.body}</p>
                <p className="text-[10px] text-brand-purple-dark/80 mt-1 tabular-nums">{new Date(n.createdAt).toLocaleString()}</p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                ...
                {!n.read && (
                  <button onClick={() => markRead(n.id)} className="text-[10px] text-brand-purple-dark/80 hover:text-brand-purple-dark">
                    Read
                  </button>
                )}
              </div>
  ...
  return (
    <button
      ...
      className={`px-2.5 py-1 rounded-md text-[11px] capitalize transition-colors ${
        current === value ? "bg-cyan-500/15 text-cyan-200 border border-cyan-400/20" : "text-brand-purple-dark/80 hover:text-brand-purple-dark"
      }`}
    >
      {label}
    </button>
