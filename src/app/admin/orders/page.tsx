  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-7xl">
      <div>
        ...
        <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">All orders</h1>
      ...
      <div className="flex flex-wrap gap-2 items-center">
          ...
          {RANGE_PRESETS.map(p => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                range === p.id ? "bg-brand-amber/20 text-brand-amber" : "text-brand-purple-dark/80 hover:text-brand-purple-dark"
              }`}
            >
              {p.label}
            </button>
        ...
        {range === "custom" && (
          <div className="flex gap-2 items-center">
            <input type="date" value={from} onChange={e => setFrom(e.target.value)} className="bg-white border border-pink-200 rounded-lg px-2 py-1.5 text-xs text-brand-purple-dark" />
            <span className="text-brand-purple-dark/80 text-xs">→</span>
            <input type="date" value={to} onChange={e => setTo(e.target.value)} className="bg-white border border-pink-200 rounded-lg px-2 py-1.5 text-xs text-brand-purple-dark" />
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
        <div className="flex flex-wrap gap-1 p-1 rounded-xl bg-white border border-pink-200/50 w-fit">
          {FILTERS.map(f => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                filter === f.id ? "bg-brand-orange/20 text-brand-purple-dark" : "text-brand-purple-dark/80 hover:text-brand-purple-dark"
              }`}
            >
              {f.label}
            </button>
        ...
        <div className="flex gap-2 flex-wrap">
          <select value={sourceFilter} onChange={e => setSourceFilter(e.target.value)} className="bg-white border border-pink-200 rounded-lg px-3 py-2 text-xs text-brand-purple-dark">
            <option value="">All sources</option>
            {sources.map(s => <option key={s.id} value={s.id}>{s.label}</option>)}
          </select>
          <label className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white border border-pink-200 text-xs text-brand-purple-dark cursor-pointer">
          ...
          <input
            ...
            className="bg-white border border-pink-200 rounded-lg px-3 py-2 text-sm text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-brand-orange/40 w-full sm:w-64"
          />
        </div>
      </div>
          ...
          {filtered.length === 0 && (
            <div className="px-5 py-10 text-center">
              {orders.length === 0 ? (
                ...
                <p className="text-sm text-brand-purple-dark/80">No orders match the current filter.</p>
              )}
            </div>
          )}
          {filtered.map(o => (
            ...
            >
              <div className="md:grid md:grid-cols-[110px_1fr_120px_120px_110px_100px] md:gap-4 md:items-center flex flex-col gap-1">
                <span className="text-sm text-brand-purple-dark font-mono">{o.id}</span>
                <div className="min-w-0">
                  <p className="text-sm text-brand-purple-dark truncate">{o.customerName}</p>
                  <p className="text-[11px] text-brand-purple-dark/80 truncate">{o.customerEmail}</p>
                </div>
                <span className="text-[11px] text-brand-purple-dark/80 truncate">
                  {sourceLabel(o.source)}
                  {o.discountCode && <span className="block text-brand-amber font-mono">{o.discountCode}</span>}
                </span>
                <span className="text-xs text-brand-purple-dark/80">{new Date(o.createdAt).toLocaleDateString()}</span>
                <StatusPill status={o.status} />
                <span className="text-sm text-brand-purple-dark md:text-right">£{o.total.toFixed(2)}</span>
              </div>
            </Link>
          ))}
        </div>
