    ...
    <div className="p-6 sm:p-8 lg:p-10 max-w-5xl space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-2">
            <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Organisations</h1>
          ...
          <p className="text-brand-purple-dark/80 text-sm mt-1">{orgs.length} {orgs.length === 1 ? "org" : "orgs"}</p>
        </div>
      ...
      {creating && (
        <div className="rounded-2xl border border-brand-orange/30 bg-brand-orange/5 p-5 space-y-3">
          <p className="text-sm font-medium text-brand-purple-dark">New organisation</p>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { setCreating(false); setNewName(""); setNewOwner(""); }} className="text-xs px-4 py-2 text-brand-purple-dark/80 hover:text-brand-purple-dark">Cancel</button>
          </div>
        </div>
      )}
            ...
            <button onClick={() => setEditing(editing === o.id ? null : o.id)} className="w-full px-5 py-4 flex items-center gap-4 text-left hover:bg-white/[0.02]">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <p className="text-sm font-medium text-brand-purple-dark">{o.name}</p>
                  ...
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-semibold uppercase tracking-wider ${o.status === "active" ? "bg-green-500/20 text-green-400" : o.status === "trialing" ? "bg-brand-amber/20 text-brand-amber" : "bg-white/8 text-brand-purple-dark/80"}`}>
                    {o.status}
                  </span>
                </div>
                {o.ownerEmail && <p className="text-[11px] text-brand-purple-dark/80 truncate font-mono mt-0.5">{o.ownerEmail}</p>}
              </div>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={`text-brand-purple-dark/80 shrink-0 transition-transform ${editing === o.id ? "rotate-180" : ""}`}>
