  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 space-y-8 max-w-7xl">
      <div>
        ...
        <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Overview</h1>
        <p className="text-brand-purple-dark/80 text-sm mt-1">Last 30 days · GBP</p>
      </div>
      ...
      {isFirstRun && (
        <section className="rounded-2xl border border-cyan-400/25 bg-gradient-to-br from-cyan-500/10 via-blue-500/5 to-transparent p-5 sm:p-6 relative">
          <button
            ...
            aria-label="Dismiss"
            className="absolute top-3 right-3 text-brand-purple-dark/80 hover:text-brand-purple-dark text-lg leading-none"
          >
            ×
          </button>
          <p className="text-[10px] tracking-[0.32em] uppercase text-cyan-400 mb-1">Welcome to {org?.name ?? "your portal"}</p>
          <h2 className="font-display text-xl sm:text-2xl text-brand-purple-dark mb-2">Let&apos;s get you set up.</h2>
          <p className="text-[13px] text-brand-purple-dark/80 mb-4 max-w-2xl">
        ...
        <div className="flex items-end gap-1 h-32">
          {stats.revenueByDay.map(d => (
            <div key={d.day} className="flex-1 group relative">
              ...
              />
              <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 px-2 py-1 rounded bg-pink-50 text-[10px] text-brand-purple-dark opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
                {d.day} · £{d.revenue.toFixed(2)}
              </span>
            </div>
          ))}
        </div>
        <div className="flex justify-between text-[10px] text-brand-purple-dark/80 mt-2">
          ...
          <div className="space-y-1">
            {recent.length === 0 && (
              <div className="py-8 text-center">
                <p className="text-sm text-brand-purple-dark/80">No orders yet.</p>
                <p className="text-xs text-brand-purple-dark/80 mt-1.5">
            ...
            {recent.map(o => (
              ...
              >
                <div className="min-w-0">
                  <p className="text-sm text-brand-purple-dark truncate">{o.id} · {o.customerName}</p>
                  <p className="text-[11px] text-brand-purple-dark/80">{new Date(o.createdAt).toLocaleDateString()} · {o.items.length} item{o.items.length === 1 ? "" : "s"}</p>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  <StatusPill status={o.status} />
                  <span className="text-sm text-brand-purple-dark w-16 text-right">£{o.total.toFixed(2)}</span>
                </div>
              </Link>
            ))}
          </div>
            ...
            {stats.topProducts.map((p, i) => (
              <div key={p.name}>
                <div className="flex items-center justify-between text-sm mb-1">
                  <span className="text-brand-purple-dark truncate mr-2">{i + 1}. {p.name}</span>
                  <span className="text-brand-purple-dark/80 shrink-0">£{p.revenue.toFixed(0)}</span>
                </div>
        ...
        {sourceBreakdown.length === 0 ? (
          ...
          <div className="space-y-2.5">
            {sourceBreakdown.map(s => (
              <div key={s.label}>
                <div className="flex items-center justify-between text-sm mb-1">
                  <span className="text-brand-purple-dark truncate mr-2">{s.label}</span>
                  <span className="text-brand-purple-dark/80 shrink-0 text-xs">{s.count} order{s.count === 1 ? "" : "s"} · £{s.revenue.toFixed(0)}</span>
                </div>
                <div className="h-1 bg-white/5 rounded-full overflow-hidden">
...
function OnboardingStep({ num, title, hint, href }: { num: number; title: string; hint: string; href: string }) {
  return (
    <Link
      href={href}
      className="rounded-xl border border-pink-200 bg-white/[0.03] hover:bg-white/[0.06] hover:border-cyan-400/30 transition-colors p-3 flex flex-col gap-1"
    >
      <span className="flex items-center gap-2">
        ...
        <span className="text-[12px] font-medium text-brand-purple-dark">{title}</span>
      </span>
      <span className="text-[10px] text-brand-purple-dark/80 leading-relaxed">{hint}</span>
    </Link>
  );
}
...
export function StatusPill({ status }: { status: Order["status"] }) {
  const map: Record<Order["status"], string> = {
    pending:   "bg-brand-cream/10 text-brand-purple-dark/80",
    paid:      "bg-brand-amber/15 text-brand-amber",
    fulfilled: "bg-brand-orange/15 text-brand-orange",
    refunded:  "bg-white/5 text-brand-purple-dark/80",
    cancelled: "bg-white/5 text-brand-purple-dark/80 line-through",
  };
