    ...
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-5xl">
      ...
      <div className="flex items-end justify-between gap-3 flex-wrap">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Build new pages</h1>
          <p className="text-brand-purple-dark/80 text-sm mt-1">Stack hero, text, image, gallery, quote, embed and CTA blocks. Each page lives at <code className="text-brand-purple-dark/80">/p/&lt;slug&gt;</code>.</p>
        </div>
            ...
            <div key={p.id} className={`flex items-center gap-4 px-5 py-4 hover:bg-white/[0.02] ${p.hidden ? "opacity-50" : ""}`}>
              <Link href={`/admin/pages/${p.id}`} className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${p.status === "published" ? "bg-green-400/20 text-green-300" : "bg-white/10 text-brand-purple-dark/80"}`}>{p.status}</span>
                  {p.hidden && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-white/10 text-brand-purple-dark/80">hidden</span>}
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <Link href={`/p/${p.slug}`} target="_blank" className="text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark">View</Link>
                <button onClick={() => { if (confirm(`Delete "${p.title}"?`)) deletePage(p.id); }} className="text-[11px] text-brand-purple-dark/80 hover:text-brand-orange">Delete</button>
              </div>
            </div>
          ))}
        </div>
