    ...
    <main className="max-w-4xl mx-auto px-6 py-8 space-y-6">
      ...
      <header className="flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">Installed plugins</h1>
          <p className="text-[12px] text-brand-purple-dark/80 mt-1">Last-known healthcheck result per plugin. Refresh to re-run.</p>
        </div>
        <button
          type="button"
      ...
      {loading ? (
        ...
        <p className="text-[12px] text-brand-purple-dark/80">No plugins installed.</p>
      ) : (
            ...
            return (
              <article
                ...
                className={`rounded-xl border p-4 flex items-start gap-3 ${
                  unknown ? "border-pink-200/50 bg-white/[0.02]" :
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-[13px] text-brand-purple-dark font-medium">{row.pluginId}</p>
                    {!row.enabled && <span className="text-[9px] tracking-wider uppercase px-1.5 py-0.5 rounded bg-white/5 text-brand-purple-dark/80">disabled</span>}
                  </div>
                  {row.health?.message && (
                    <p className="text-[12px] text-brand-purple-dark/80 mt-1">{row.health.message}</p>
                  )}
                  {row.health?.components && (
                    <ul className="text-[11px] text-brand-purple-dark/80 mt-2 space-y-0.5">
