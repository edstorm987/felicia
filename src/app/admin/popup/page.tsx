      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-2">
            <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Discount popup</h1>
            <Tip
              id="popup.header"
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="text-xs px-3 py-1.5 rounded-lg border border-pink-200 text-brand-purple-dark/80 hover:text-brand-purple-dark"
          >
            Reset seen flag
          </button>
          <button
        ...
        <div className="space-y-5">
          ...
          <Card title="Styling">
            <Field label="Accent colour">
              <div className="flex items-center gap-2">
                <input type="color" value={cfg.accentColor} onChange={e => patch("accentColor", e.target.value)} className="w-12 h-10 rounded-lg border border-pink-200 cursor-pointer bg-transparent" />
        ...
        <div className="space-y-3 lg:sticky lg:top-6 lg:self-start">
          ...
          <div className="rounded-2xl border border-pink-200/50 bg-white/80 p-4 overflow-hidden">
            <div className="rounded-2xl bg-white border border-brand-purple/20 p-6 text-center relative overflow-hidden">
              ...
              <h3 className="font-display text-xl font-bold text-brand-purple-dark leading-tight mb-2">{cfg.headline}</h3>
              <p className="text-xs text-brand-purple-dark/80 leading-relaxed mb-4">{cfg.subheadline}</p>
              <div className="text-[10px] text-brand-purple-dark/80 mb-2 truncate">{cfg.consentLabel}</div>
              <button
                disabled
              ...
              <div className="mt-3 text-[9px] tracking-widest uppercase text-brand-purple-dark/80">{cfg.declineLabel}</div>
            </div>
            <p className="text-[11px] text-brand-purple-dark/80 mt-3 leading-relaxed">
              Trigger: <span className="text-brand-purple-dark/80">{cfg.trigger}{cfg.trigger === "delay" ? ` after ${cfg.delaySeconds}s` : cfg.trigger === "scroll" ? ` at ${cfg.scrollPercent}%` : ""}</span><br/>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-[11px] tracking-[0.18em] uppercase text-brand-purple-dark/80 mb-1.5">{label}</label>
      {children}
    </div>
  );
}
