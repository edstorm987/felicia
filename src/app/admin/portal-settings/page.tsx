...
export default function AdminPortalSettingsPage() {
  return (
    <Suspense fallback={<div className="p-6 text-[12px] text-brand-purple-dark/80">Loading…</div>}>
  ...
  if (loading) {
    return (
      <div className="p-6 sm:p-8 lg:p-10 max-w-5xl space-y-6">
        ...
        <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Portal settings</h1>
        <p className="text-brand-purple-dark/80 text-sm mt-3">Loading settings from the cloud store…</p>
      </div>
    );
  }
    ...
    <div className="p-6 sm:p-8 lg:p-10 max-w-5xl space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">
      ...
      <Card id="setup-database" title="Database" tip="Where the portal stores its own state (heartbeats, content overrides, embed registry, etc.). The actual backend choice is server-side via the PORTAL_BACKEND env var; this UI lets you record your intent + supply credentials.">
                ...
                <button
                  ...
                  className={`text-left px-3 py-3 rounded-xl border transition-colors ${
                    active
                      ...
                      : "border-pink-200 text-brand-purple-dark/80 hover:border-white/25"
                  }`}
                >
                  <p className="text-sm font-medium">{b.label}</p>
                  <p className="text-[11px] text-brand-purple-dark/80 mt-0.5">{b.sub}</p>
                </button>
              );
            })}
          </div>
        ...
        <div className="rounded-xl border border-pink-200 bg-white/[0.02] px-3 py-2.5 text-[11px] text-brand-purple-dark/80 leading-relaxed space-y-1">
          <p>
            <strong className="text-brand-purple-dark/80">Server-side selection.</strong>{" "}
            The active backend is decided by <code className="font-mono text-brand-purple-dark/80">PORTAL_BACKEND</code>{" "}
            on the server, not by this UI. Set it at deploy time:
          </p>
          <pre className="font-mono text-[10px] text-brand-purple-dark/80 bg-pink-50/40 border border-pink-200/50 rounded px-2 py-1.5 overflow-x-auto">PORTAL_BACKEND=file{"\n"}# or PORTAL_BACKEND=memory  (no persistence, ephemeral){"\n"}# or PORTAL_BACKEND=kv      (Upstash REST — slot in v1, not yet wired)</pre>
      ...
      <Card id="setup-integrations" title="Integrations" tip="Connect the portal to your hosting + repo provider so a brand-new site auto-fills its admin row when the tag first phones home. Vercel maps domain → project → linked repo; that repo becomes the default for promote PRs (D-3).">
        ...
        <div className="flex items-start gap-3 px-3 py-2.5 rounded-lg bg-pink-50 border border-pink-200/50">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium text-brand-purple-dark">Auto-discover new sites</p>
      ...
      <div>
        <button
          ...
          className="text-xs text-brand-purple-dark/80 hover:text-brand-orange"
        >
          Reset to defaults
        </button>
      </div>
  ...
  return (
    <div>
      <div className="flex items-center gap-1.5 mb-1.5">
        <label className="block text-[11px] tracking-[0.18em] uppercase text-brand-purple-dark/80">{label}</label>
        {tip && <Tip text={tip} />}
      </div>
      {children}
    </div>
    ...
    return (
      <Field label={label} tip={tip}>
        <div className="flex items-center gap-2">
          <code className="flex-1 font-mono text-xs text-brand-purple-dark/80 bg-white/5 border border-pink-200 rounded-xl px-3 py-2.5">
        ...
        <div className="absolute right-1 top-1/2 -translate-y-1/2 flex items-center gap-1">
          {editing && (
            <button
              type="button"
              onClick={cancel}
              className="text-[10px] uppercase tracking-wider px-2 py-1 rounded-md text-brand-purple-dark/80 hover:text-brand-purple-dark hover:bg-white/5"
            >
              Cancel
            </button>
          )}
...
function SecurityCard() {
  ...
  return (
    ...
    >
      <p className="text-[12px] text-brand-purple-dark/80 leading-relaxed">
        Effective mode: <strong className="text-brand-purple-dark/80 font-mono">{mode}</strong>
            ...
            <button
              ...
              className={`text-left p-3 rounded-xl border transition-colors ${
                active
                  ...
                  : "border-pink-200 bg-white/[0.02] hover:bg-white/[0.05]"
              }`}
            >
              <p className="text-[12px] font-semibold text-brand-purple-dark">{c.label}</p>
              <p className="text-[10px] text-brand-purple-dark/80 leading-relaxed mt-0.5">{c.sub}</p>
            </button>
          );
        })}
      </div>
      <div className="flex items-center justify-between text-[11px]">
        {hasOverride ? (
          <button
            ...
            className="text-brand-purple-dark/80 hover:text-brand-orange"
          >
      ...
      <div className="rounded-lg border border-pink-200/50 bg-pink-50 px-3 py-2.5 text-[11px] text-brand-purple-dark/80 leading-relaxed">
        Production env: set <code className="font-mono text-brand-purple-dark/80">NEXT_PUBLIC_PORTAL_SECURITY=strict</code> (or <code className="font-mono">dev</code> / <code className="font-mono">off</code>).
...
function SavedIndicator({ at }: { at: number | null }) {
  ...
  return (
    <span
      ...
      className="text-[11px] text-brand-purple-dark/80 px-2.5 py-1 rounded-full border border-pink-200 bg-white/5 animate-pulse shrink-0"
    >
      Saved
    </span>
  );
...
function BackendStatusRow({ info, intended }: {
  ...
  if (!info) {
    return (
      <div className="rounded-xl border border-pink-200 bg-white/[0.02] px-3 py-2.5 text-[11px] text-brand-purple-dark/80">
  ...
  const dot = info.persistent && info.writable
    ...
    : info.persistent
      ...
      : "bg-white/30";
  return (
    <div className="rounded-xl border border-pink-200 bg-white/[0.02] px-3 py-2.5 text-[11px] text-brand-purple-dark/80 leading-relaxed">
      <div className="flex flex-wrap items-center gap-2">
        <span className={`w-1.5 h-1.5 rounded-full ${dot}`} />
        <span className="text-brand-purple-dark/80 font-semibold">Active backend:</span>
        <code className="font-mono text-brand-purple-dark bg-white/5 px-1.5 py-0.5 rounded border border-pink-200">
          {info.kind}
        </code>
        <span className="text-brand-purple-dark/80">
      ...
      <p className="text-brand-purple-dark/80 mt-1">{info.description}</p>
    </div>
  );
}
...
function SetupChecklist({ settings, backendInfo, setupSite, setupMode }: {
  ...
  return (
    <div className="rounded-2xl border border-brand-orange/30 bg-brand-orange/5 overflow-hidden">
      ...
      <ul className="divide-y divide-white/5">
        {items.map(it => (
          <li key={it.id} className="px-5 py-3 flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm text-brand-purple-dark font-medium">{it.label}</p>
              <p className="text-[11px] text-brand-purple-dark/80 mt-0.5">{it.hint}</p>
            </div>
            <button
      ...
      <div className="px-5 py-4 border-t border-pink-200/50 bg-white/[0.02] space-y-3">
        <div className="flex items-center gap-2">
          <p className="text-[10px] uppercase tracking-[0.18em] text-brand-purple-dark/80">Optional · {setupMode} site</p>
        </div>
  ...
  return (
    <div className="rounded-lg border border-pink-200/50 bg-pink-50/40 px-3 py-2.5 space-y-1.5">
      ...
      <p className="text-[11px] text-brand-purple-dark/80 leading-relaxed">
        Opens a PR adding <code className="font-mono text-brand-purple-dark/80">portal-tag.html</code> at your repo root with the script tag for site <code className="font-mono">{siteId}</code> and instructions for where to paste it (Next.js / Astro / vanilla).
      </p>
      {result && (
    ...
    <div className="fixed inset-0 z-[300] bg-white/80 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="p-5 space-y-4 overflow-y-auto">
          ...
          <div>
            <label className="block text-[11px] tracking-[0.18em] uppercase text-brand-purple-dark/80 mb-1.5">
              Framework / stack
            </label>
            <input
              value={framework}
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="text-[11px] tracking-[0.18em] uppercase text-brand-purple-dark/80">Prompt</label>
              <button
                onClick={copyPrompt}
      ...
      <div className="rounded-2xl border border-brand-amber/40 bg-brand-amber/5 overflow-hidden">
        ...
        <div className="p-5 space-y-3">
          {backendInfo.schemaError && (
            <p className="text-[11px] text-brand-purple-dark/80">{backendInfo.schemaError}</p>
          )}

          {/* Primary action: Sync DB */}
          <div className="rounded-xl border border-brand-orange/30 bg-brand-orange/5 px-4 py-3 space-y-2">
            <div className="flex items-center gap-2 flex-wrap">
              <p className="text-sm font-semibold text-brand-purple-dark">Sync database</p>
...
{backendInfo.migrationSql}
          ...
          <div className="flex items-center justify-between gap-2 pt-1">
            <p className="text-[10px] text-brand-purple-dark/80">
              Source: <code className="font-mono text-brand-purple-dark/80">src/portal/server/storage.ts</code>. Idempotent.
            </p>
            <button
              onClick={() => setAiPromptOpen(true)}
              className="text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark underline"
            >
        ...
        {tabs.map(t => {
          ...
          return (
            ...
            >
              ...
              <p className="text-[11px] text-brand-purple-dark/80 mt-0.5">{t.sub}</p>
            </button>
          );
        })}
      </div>
...
function DeployStatusRow({
  ...
  return (
    <div className="rounded-xl border border-pink-200 bg-white/[0.02] px-3 py-2.5 text-[11px] text-brand-purple-dark/80 leading-relaxed">
      <div className="flex flex-wrap items-center gap-2">
        <span className={`w-1.5 h-1.5 rounded-full ${dot}`} />
        <span className="text-brand-purple-dark/80 font-semibold">{label}</span>
        <code className="font-mono text-brand-purple-dark bg-white/5 px-1.5 py-0.5 rounded border border-pink-200 ml-auto">
          {backendInfo.kind}
        </code>
        <span className="text-brand-purple-dark/80">
      ...
      <p className="text-brand-purple-dark/80 mt-1">{detail}</p>
    </div>
  );
}
...
function DownloadRepoZip() {
  ...
  return (
    <div className="rounded-xl border border-brand-orange/30 bg-brand-orange/5 px-4 py-3 space-y-2">
      <div className="flex items-center gap-2 flex-wrap">
        <p className="text-sm font-semibold text-brand-purple-dark">Download repo zip</p>
      ...
      <p className="text-[11px] text-brand-purple-dark/80 leading-relaxed">
        Bundles <code className="font-mono text-brand-purple-dark/80">src/</code>, <code className="font-mono text-brand-purple-dark/80">package.json</code>, and the build configs into a single archive. Push it to your own GitHub repo, point Vercel / your VM at it, set the env vars below, and you&apos;re live.
      </p>
      {info && (
        <p className="text-[11px] text-green-400">
...
PORTAL_PREVIEW_BASE_URL=${previewBaseUrl || "https://your-site.com"}`;
  ...
  return (
    <div className="space-y-3">
      <p className="text-[12px] text-brand-purple-dark/80 leading-relaxed">
        <strong className="text-brand-purple-dark/80">Recommended.</strong>{" "}
      ...
      <ol className="text-[11px] text-brand-purple-dark/80 leading-relaxed list-decimal list-inside space-y-1">
...
# (or: node node_modules/next/dist/bin/next start -p 3000)`;
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-xl border border-brand-amber/25 bg-brand-amber/5 px-3 py-2.5 text-[11px] text-brand-purple-dark/80 leading-relaxed">
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between">
        <label className="text-[11px] tracking-[0.18em] uppercase text-brand-purple-dark/80">{label}</label>
        <button
          onClick={copy}
          className="text-[11px] px-2.5 py-1 rounded-md bg-brand-orange/20 border border-brand-orange/40 text-brand-orange hover:bg-brand-orange/30 font-semibold"
        >
