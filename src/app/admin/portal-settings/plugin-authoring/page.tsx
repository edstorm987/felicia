  ...
  return (
    <main className="max-w-3xl mx-auto px-6 py-10 space-y-8">
      ...
      <header>
        ...
        <h1 className="font-display text-3xl text-brand-purple-dark">Build your own plugin</h1>
        <p className="text-[13px] text-brand-purple-dark/80 mt-1 leading-relaxed">
          ...
          <code className="font-mono mx-1 text-brand-purple-dark/80">src/plugins/&lt;id&gt;/index.ts</code>.
...
on("order.paid", async (event) => {
  ...
  // do your thing — sync to a CRM, kick off a webhook, send an email
      ...
      <Section title="Things to watch">
        <ul className="list-disc list-inside space-y-1 text-[12px] text-brand-purple-dark/80">
      ...
      <footer className="pt-6 border-t border-pink-200/50 text-[11px] text-brand-purple-dark/80">
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="space-y-3">
      <h2 className="font-display text-lg text-brand-purple-dark">{title}</h2>
      <div className="text-[13px] text-brand-purple-dark/80 space-y-3 leading-relaxed">
        {children}
      </div>
    </section>
  );
