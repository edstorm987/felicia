...
export default function AdminPortalsPage() {
  return (
    <Suspense fallback={<div className="p-8 text-[12px] text-brand-purple-dark/80">Loading…</div>}>
      <AdminPortalsInner />
    </Suspense>
  );
}
    ...
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-5xl">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Customer-facing portal designer</h1>
        ...
        {sites.length > 1 && (
          <select
            ...
            className="bg-white/5 border border-pink-200 rounded-lg px-3 py-2 text-sm text-brand-purple-dark focus:outline-none focus:border-brand-orange/50"
          >
              ...
              <li key={r.id}>
                <button
                  ...
                  className={`relative inline-flex items-center gap-1.5 px-3 py-2.5 text-[12px] tracking-wide whitespace-nowrap transition-colors ${
                    active ? "text-brand-purple-dark" : "text-brand-purple-dark/80 hover:text-brand-purple-dark/80"
                  }`}
      ...
      <section className="rounded-2xl border border-white/8 bg-white p-5 sm:p-6 flex flex-col sm:flex-row gap-4 sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-[12px] text-brand-purple-dark/80 mt-1 leading-relaxed max-w-prose">{meta.description}</p>
        </div>
            ...
            >
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap mb-0.5">
                  <p className="text-[14px] font-semibold text-brand-purple-dark truncate">{v.title}</p>
                  {v.isActivePortal && (
                    <span className="text-[9px] tracking-wider uppercase px-1.5 py-0.5 rounded bg-cyan-400/15 text-cyan-300">active</span>
                  )}
                  <span className="text-[9px] tracking-wider uppercase px-1.5 py-0.5 rounded bg-white/5 text-brand-purple-dark/80">{v.status}</span>
                </div>
                <p className="text-[11px] text-brand-purple-dark/80 font-mono truncate">{v.slug}</p>
                <p className="text-[10px] text-brand-purple-dark/80 mt-0.5">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <button
                  ...
                  disabled={busy}
                  className="text-[11px] px-2 py-1.5 rounded-md text-brand-purple-dark/80 hover:text-red-400 disabled:opacity-40"
