...
export default function AdminProductEditPage() {
  ...
  if (!product) return <div className="p-8 text-brand-purple-dark/80 text-sm">Loading product…</div>;
  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-5xl">
      <Link href="/admin/products" className="text-xs text-brand-purple-dark/80 hover:text-brand-purple-dark">← All products</Link>
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">{product.name}</h1>
          <p className="text-brand-purple-dark/80 text-sm mt-1 font-mono">{product.slug}</p>
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <Section title="Pricing">
            ...
            <label className={`mt-4 flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${
              ...
              />
              <div>
                <p className="text-sm text-brand-purple-dark">Put this product on sale</p>
          ...
          <Section title="Stock &amp; Inventory" hint="Link to an inventory SKU to track availability.">
            ...
            {linkedItem && (
              <div className="mt-4 p-4 rounded-xl bg-white/80 border border-pink-200/50 space-y-4">
                {!linkedItem.unlimited ? (
                  <>
                    <div className="grid grid-cols-4 gap-3">
                      <div>
                        <p className="text-xs text-brand-purple-dark/80">On hand</p>
                        <p className="font-display text-2xl text-brand-purple-dark">{linkedItem.onHand}</p>
                      </div>
                      <div>
                        <p className="text-xs text-brand-purple-dark/80">Reserved</p>
                        <p className="font-display text-2xl text-brand-purple-dark">{linkedItem.reserved}</p>
                      </div>
                      <div>
                        <p className="text-xs text-brand-purple-dark/80">Available</p>
                      ...
                      <div>
                        <p className="text-xs text-brand-purple-dark/80">Low at</p>
                        <input
                          type="number"
                          min={0}
                ...
                <label className="flex items-center gap-3 p-3 rounded-lg border border-pink-200 hover:border-pink-300 cursor-pointer">
                  ...
                  />
                  <div>
                    <p className="text-sm text-brand-purple-dark">Unlimited stock</p>
                    <p className="text-[11px] text-brand-purple-dark/80">For digital goods, gift cards, or made-to-order items. Never goes sold out.</p>
                  </div>
                </label>
              </div>
            )}
            <label className={`mt-4 flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${
              ...
              />
              <div>
                <p className="text-sm text-brand-purple-dark">Show low-stock badge on storefront</p>
                <p className="text-[11px] text-brand-purple-dark/80">Displays &quot;Only X left&quot; when stock is low — drives urgency.</p>
              </div>
            </label>
          </Section>
          ...
          <Section title="Display">
            ...
            <label className="mt-4 flex items-center gap-3 p-3 rounded-lg border border-pink-200 hover:border-pink-300 cursor-pointer">
              ...
              />
              <div>
                <p className="text-sm text-brand-purple-dark">Archive product</p>
                <p className="text-[11px] text-brand-purple-dark/80">Hide it from the storefront. Existing orders are unaffected.</p>
              </div>
            </label>
          </Section>
          ...
          <div className="rounded-2xl border border-pink-200/50 bg-white overflow-hidden">
            <div className="relative aspect-[4/3] bg-gradient-to-br from-brand-purple-muted via-brand-black-card to-brand-purple-dark flex items-center justify-center">
              {image ? (
                ...
                <span className="text-5xl text-brand-purple-dark/80">◆</span>
              )}
              {onSale && salePrice && (
            ...
            <div className="p-4 space-y-2">
              <p className="text-[10px] tracking-[0.22em] uppercase text-brand-purple-dark/80">{product.tagline}</p>
              <h3 className="text-base font-semibold text-brand-purple-dark">{product.name}</h3>
              <div className="flex items-baseline gap-2">
                {onSale && salePrice ? (
                  <>
                    <span className="font-display text-xl text-brand-orange">£{(activePrice || 0).toFixed(2)}</span>
                    <span className="text-sm text-brand-purple-dark/80 line-through">£{parseFloat(price || "0").toFixed(2)}</span>
                  </>
                ) : (
                  <span className="font-display text-xl text-brand-purple-dark">£{parseFloat(price || "0").toFixed(2)}</span>
                )}
              </div>
              <p className="text-xs text-brand-purple-dark/80 leading-relaxed line-clamp-3">{description.split(/\n{2,}/)[0]}</p>
            </div>
          </div>
          {hasOverride && (
...
function Field({ label, tip, children }: { label: string; tip?: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="block text-[10px] tracking-[0.22em] uppercase text-brand-purple-dark/80 mb-1.5 inline-flex items-center gap-1.5">
        {label}
        {tip && <Tip text={tip} />}
      </span>
      {children}
