...
export default function ProductVariantsPage() {
  ...
  if (!product) {
    return (
      <div className="max-w-2xl mx-auto p-6 text-[12px] text-brand-purple-dark/80">
        {error ?? "Loading…"}
      </div>
    );
  }
    ...
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      ...
      <header className="flex items-baseline justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">{product.name}</h1>
          <p className="text-[12px] text-brand-purple-dark/80 mt-1 font-mono">{product.slug}</p>
        </div>
        <Link href={`/admin/products/${product.slug}`} className="text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark">← Back to product</Link>
      </header>
      ...
      <div className="flex items-center gap-3">
        ...
        {savedAt && <span className="text-[11px] text-brand-purple-dark/80">Saved</span>}
      </div>
          ...
          <div key={option.id} className="rounded-2xl border border-pink-200 bg-white/[0.02]">
            <div className="px-4 py-3 border-b border-white/8 flex items-center gap-3">
              <input
                ...
                disabled={!isCustom}
                className="flex-1 bg-transparent text-brand-purple-dark text-sm font-semibold focus:outline-none focus:bg-white/5 px-2 py-1 rounded"
              />
              <select
              ...
              {option.displayType === "color-wheel" && (
                <label className="flex items-center gap-2 text-[11px] text-brand-purple-dark/80 whitespace-nowrap">
              ...
              <button onClick={() => removeOption(option.id)} disabled={!isCustom} className="text-[11px] text-brand-purple-dark/80 hover:text-red-400 disabled:opacity-30">Remove</button>
            </div>
            <div className="p-3 space-y-2">
              {option.values.length === 0 && (
                <p className="text-[11px] text-brand-purple-dark/80 px-2 py-2">No values yet. Add at least one.</p>
              )}
              {option.values.map(value => (
                <div key={value.id} className="grid grid-cols-12 gap-2 items-center">
                  ...
                  {(option.displayType === "swatch" || option.displayType === "color-wheel") && (
                    <div className="col-span-3 flex gap-1">
                      <input
                        ...
                        disabled={!isCustom}
                        className="w-9 h-8 rounded cursor-pointer bg-transparent border border-pink-200"
                      />
                      <input
                  ...
                  />
                  <label className="col-span-3 flex items-center gap-2 text-[11px] text-brand-purple-dark/80">
                  ...
                  <button onClick={() => removeValue(option.id, value.id)} disabled={!isCustom} className="col-span-1 text-[11px] text-brand-purple-dark/80 hover:text-red-400 disabled:opacity-30">×</button>
                </div>
              ))}
      ...
      {options.some(o => o.displayType === "color-wheel" && o.allowCustom) && (
        <div className="rounded-2xl border border-brand-amber/30 bg-brand-amber/5 p-4">
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <span className="text-[12px] text-brand-purple-dark/80">pence — added when a customer picks a custom colour from the wheel ({customSurcharge ? `+£${(customSurcharge / 100).toFixed(2)}` : "free"})</span>
          </div>
        </div>
      )}
      ...
      {options.length > 0 && (
        <section className="rounded-2xl border border-pink-200 bg-white/[0.02]">
          ...
          {variants.length === 0 ? (
            <p className="px-4 py-6 text-[12px] text-brand-purple-dark/80 text-center">
            ...
            <table className="w-full text-[12px]">
              ...
              <tbody>
                {variants.map(v => (
                  <tr key={v.id} className="border-b border-pink-200/50 hover:bg-white/[0.02]">
                    {options.map(o => {
                      const valueId = v.optionValues[o.id];
                      const value = o.values.find(val => val.id === valueId);
                      return <td key={o.id} className="px-3 py-2 text-brand-purple-dark/80">{value?.label ?? "—"}</td>;
                    })}
