    ...
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-7xl">
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Products</h1>
          <p className="text-brand-purple-dark/80 text-sm mt-1">{products.length} products · click any to edit</p>
        </div>
        <Link
          ...
          {["all", ...collections.map(c => c.slug)].map(r => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                range === r ? "bg-brand-orange/20 text-brand-purple-dark" : "text-brand-purple-dark/80 hover:text-brand-purple-dark"
              }`}
            >
          ...
          return (
            ...
            >
              ...
              <div className="p-4 space-y-3">
                <div>
                  <p className="text-[10px] tracking-[0.22em] uppercase text-brand-purple-dark/80">{rangeLabel(p.range)}</p>
                  <Link href={`/admin/products/${p.slug}`} className="block">
                    <h3 className="text-sm font-semibold text-brand-purple-dark truncate">{p.name}</h3>
                  </Link>
                  <p className="text-[11px] text-brand-purple-dark/80 truncate font-mono">{p.slug}</p>
                </div>
                  ...
                  <div>
                    {p.onSale && p.salePrice ? (
                      <div className="flex items-baseline gap-2">
                        ...
                        <span className="text-xs text-brand-purple-dark/80 line-through">£{p.price.toFixed(2)}</span>
                      </div>
                    ) : (
                      <span className="font-display text-lg text-brand-purple-dark">£{p.price.toFixed(2)}</span>
                    )}
                  </div>
                  <button
                ...
                <div className="flex gap-2">
                  ...
                  {"_custom" in p && (
                    <button
                      ...
                      className="px-2.5 py-2 rounded-lg border border-pink-200 text-xs text-brand-purple-dark/80 hover:text-brand-orange hover:border-brand-orange/30 transition-colors"
                    >
                      ✕
                    </button>
                  )}
