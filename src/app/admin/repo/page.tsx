    ...
    <div className="max-w-7xl mx-auto p-6">
      <header className="flex items-baseline justify-between gap-4 mb-4">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">Repository</h1>
          <p className="text-[12px] text-brand-purple-dark/80 mt-1">Browse + edit any file in the connected GitHub repo. Each save is a real commit.</p>
        </div>
        <Link href="/admin/portal-settings" className="text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark">
          Settings →
        </Link>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-4">
        ...
        <aside className="rounded-2xl border border-pink-200 bg-white/[0.02] overflow-hidden h-[70vh] flex flex-col">
            ...
            {breadcrumb.map((seg, i) => (
              <span key={i} className="flex items-center gap-1">
                ...
                <button
                  ...
                  className={`hover:text-brand-orange ${i === breadcrumb.length - 1 ? "text-brand-purple-dark" : "text-brand-purple-dark/80"}`}
                >
                  {seg}
                </button>
              </span>
          ...
          <div className="flex-1 overflow-y-auto p-2 text-[12px]">
            {loading && <p className="text-brand-purple-dark/80 px-2 py-1">Loading…</p>}
            {!loading && items.length === 0 && <p className="text-brand-purple-dark/80 px-2 py-1">Empty.</p>}
            {items.map(it => (
              <button
                key={it.path}
                onClick={() => it.type === "dir" ? loadTree(it.path) : openFileFromPath(it.path)}
                className="w-full text-left flex items-center gap-2 px-2 py-1 rounded text-brand-purple-dark/80 hover:bg-white/5"
              >
                <span className="text-base leading-none">{it.type === "dir" ? "📁" : "📄"}</span>
                <span className="truncate">{it.name}</span>
                {it.size !== undefined && <span className="ml-auto text-[10px] text-brand-purple-dark/80">{formatSize(it.size)}</span>}
              </button>
            ))}
          </div>
        </aside>
        ...
        <main className="rounded-2xl border border-pink-200 bg-white/[0.02] overflow-hidden h-[70vh] flex flex-col">
          {openFile ? (
            <>
              <div className="px-3 py-2 border-b border-pink-200/50 flex items-center gap-2">
                ...
                <input
                  ...
                  className="bg-white/5 border border-pink-200 rounded-lg px-2 py-1 text-[11px] text-brand-purple-dark w-72 placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-brand-orange/50"
                />
                <button
                  onClick={handleSave}
                  disabled={saving}
            ...
            <div className="flex-1 flex items-center justify-center text-[12px] text-brand-purple-dark/80 p-6 text-center">
