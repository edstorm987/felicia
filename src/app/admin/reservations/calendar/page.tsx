...
function CalendarPageInner() {
  ...
  if (loading) {
    return <main className="max-w-6xl mx-auto px-6 py-12 text-[12px] text-brand-purple-dark/80">Loading calendar…</main>;
  }
  ...
  if (resources.length === 0) {
    return (
      <main className="max-w-2xl mx-auto px-6 py-16 text-center space-y-4">
        ...
        <h1 className="font-display text-2xl text-brand-purple-dark">No bookable resources yet</h1>
        <p className="text-[13px] text-brand-purple-dark/80 max-w-md mx-auto">
    ...
    <main className="max-w-6xl mx-auto px-6 py-8 space-y-4">
      <header className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">Calendar</h1>
          <p className="text-[12px] text-brand-purple-dark/80 mt-1">Week of {weekStart.toLocaleDateString()}</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <button onClick={() => shiftWeek(-7)} className="px-3 py-1.5 rounded-md text-[11px] bg-white/5 hover:bg-white/10 text-brand-purple-dark">← Prev</button>
          <button onClick={() => setWeekStart(startOfWeek(new Date()))} className="px-3 py-1.5 rounded-md text-[11px] bg-white/5 hover:bg-white/10 text-brand-purple-dark">Today</button>
          <button onClick={() => shiftWeek(7)} className="px-3 py-1.5 rounded-md text-[11px] bg-white/5 hover:bg-white/10 text-brand-purple-dark">Next →</button>
      ...
      <div className="flex items-center gap-2 text-[11px] text-brand-purple-dark/80">
        <span>Resource:</span>
        <select value={resourceFilter} onChange={e => setResourceFilter(e.target.value)} className="bg-white/5 border border-pink-200 rounded-md px-2 py-1 text-[11px]">
          <option value="">All</option>
          {resources.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
        </select>
        <span>Staff:</span>
        <select value={staffFilter} onChange={e => setStaffFilter(e.target.value)} className="bg-white/5 border border-pink-200 rounded-md px-2 py-1 text-[11px]">
          <option value="">All</option>
      ...
      {/* Calendar grid */}
      <div className="rounded-xl border border-pink-200/50 bg-white/[0.02] overflow-x-auto">
        <div className="grid" style={{ gridTemplateColumns: `60px repeat(${DAYS_VISIBLE}, minmax(120px, 1fr))`, minWidth: 900 }}>
          {/* Header row */}
          <div className="border-b border-pink-200/50 p-2 text-[10px] text-brand-purple-dark/80"></div>
          {Array.from({ length: DAYS_VISIBLE }).map((_, i) => {
            const d = new Date(weekStart.getTime() + i * 24 * 3600_000);
            const isToday = d.toDateString() === new Date().toDateString();
            return (
              <div key={i} className={`border-b border-pink-200/50 p-2 text-[11px] tabular-nums ${isToday ? "text-cyan-300 font-semibold" : "text-brand-purple-dark/80"}`}>
                {fmtDay(d)}
              </div>
            );
          })}
...
function ColumnsForHour({
  ...
  return (
    <>
      <div className="border-r border-b border-pink-200/50 px-2 py-1 text-[10px] text-brand-purple-dark/80 tabular-nums">
        {hour}:00
      </div>
      {Array.from({ length: DAYS_VISIBLE }).map((_, day) => {
        ...
        return (
          <div key={day} className="relative border-r border-b border-pink-200/50 min-h-[40px] p-0.5">
            {inCell.map(b => {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end sm:items-center justify-center p-4 bg-white/80 backdrop-blur-sm" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="w-full max-w-md rounded-2xl border border-cyan-400/20 bg-[#0a0e1a] p-5 space-y-3">
        ...
        <h2 className="font-display text-xl text-brand-purple-dark">{booking.customerName}</h2>
        <ul className="text-[12px] text-brand-purple-dark/80 space-y-1">
          <li><strong className="text-brand-purple-dark/80">When:</strong> {new Date(booking.startMs).toLocaleString()} – {new Date(booking.endMs).toLocaleTimeString()}</li>
          {service  && <li><strong className="text-brand-purple-dark/80">Service:</strong> {service.name}</li>}
          {resource && <li><strong className="text-brand-purple-dark/80">Resource:</strong> {resource.name}</li>}
          {staff    && <li><strong className="text-brand-purple-dark/80">With:</strong> {staff.name}</li>}
          <li><strong className="text-brand-purple-dark/80">Email:</strong> {booking.customerEmail}</li>
          <li><strong className="text-brand-purple-dark/80">Party:</strong> {booking.partySize}</li>
