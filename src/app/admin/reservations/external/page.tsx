    ...
    <main className="max-w-3xl mx-auto px-6 py-8 space-y-6">
      <header className="flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">Subscribed calendars</h1>
      ...
      {feeds.length === 0 ? (
        <p className="text-[12px] text-brand-purple-dark/80">No external feeds yet.</p>
      ) : (
        <ul className="space-y-1">
          {feeds.map(f => (
            <li key={f.id} className="rounded-lg border border-pink-200/50 bg-white/[0.02] p-3 flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                <p className="text-[13px] text-brand-purple-dark truncate">{f.label}</p>
                <p className="text-[11px] text-brand-purple-dark/80 truncate">{f.url}</p>
                {f.lastSyncedAt && (
