    ...
    <main className="max-w-4xl mx-auto px-6 py-8 space-y-6">
      <header className="flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">Bookings</h1>
          <p className="text-[12px] text-brand-purple-dark/80 mt-1">{upcoming.length} upcoming · {past.length} past</p>
        </div>
      ...
      <Section title="Upcoming">
        {upcoming.length === 0 ? (
          <p className="text-[12px] text-brand-purple-dark/80">No upcoming bookings.</p>
        ) : upcoming.map(b => (
      ...
      <Section title="Past">
        {past.length === 0 ? (
          <p className="text-[12px] text-brand-purple-dark/80">No past bookings yet.</p>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="space-y-2">
      <h2 className="text-[10px] tracking-[0.28em] uppercase text-brand-purple-dark/80">{title}</h2>
...
function BookingRow({ booking, resourceName }: { booking: Booking; resourceName: string }) {
  return (
    <article className="rounded-lg border border-pink-200/50 bg-white/[0.02] px-4 py-3 flex items-center gap-3">
      ...
      <span className="text-[10px] text-brand-purple-dark/80 truncate hidden sm:inline">{booking.customerEmail}</span>
    </article>
  );
}
