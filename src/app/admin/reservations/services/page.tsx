    ...
    <main className="max-w-4xl mx-auto px-6 py-8 space-y-6">
      <header className="flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">Bookable services</h1>
          <p className="text-[12px] text-brand-purple-dark/80 mt-1">{services.length} services defined</p>
        </div>
        <button
      ...
      {services.length === 0 ? (
        <p className="text-[12px] text-brand-purple-dark/80">No services defined yet.</p>
      ) : (
        <ul className="space-y-1">
          {services.map(s => (
            <li key={s.id} className="rounded-lg border border-pink-200/50 bg-white/[0.02] p-4 flex items-start justify-between gap-3">
              <div>
                <p className="text-[14px] text-brand-purple-dark font-medium">{s.name}</p>
                {s.description && <p className="text-[12px] text-brand-purple-dark/80 mt-0.5">{s.description}</p>}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="block text-[11px] text-brand-purple-dark/80 mb-1">{label}</span>
      {children}
    </label>
  );
}
...
function MultiSelect({ options, selected, onChange, empty }: {
  ...
  if (options.length === 0) {
    return <p className="text-[11px] text-brand-purple-dark/80">{empty ?? "None available."}</p>;
  }
  return (
...
const INPUT = "w-full bg-white/5 border border-pink-200 rounded-md px-2.5 py-1.5 text-[12px] text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-cyan-400/40";
