    ...
    <main className="max-w-3xl mx-auto px-6 py-8 space-y-6">
      <header className="flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">Team members</h1>
          <p className="text-[12px] text-brand-purple-dark/80 mt-1">{list.length} active</p>
        </div>
      ...
      {list.length === 0 ? (
        <p className="text-[12px] text-brand-purple-dark/80">No staff yet.</p>
      ) : (
        <ul className="space-y-1">
          {list.map(s => (
            <li key={s.id} className="rounded-lg border border-pink-200/50 bg-white/[0.02] p-3 flex items-start justify-between gap-3">
              <div>
                <p className="text-[13px] text-brand-purple-dark">{s.name}</p>
                {s.email && <p className="text-[11px] text-brand-purple-dark/80">{s.email}</p>}
                {s.bio && <p className="text-[11px] text-brand-purple-dark/80 mt-1">{s.bio}</p>}
              </div>
