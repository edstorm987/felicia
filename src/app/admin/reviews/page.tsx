    ...
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-6xl">
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Reviews</h1>
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <span className="text-[11px] tracking-[0.22em] uppercase text-brand-purple-dark/80">Filter</span>
        <select
          value={filterSlug}
          onChange={e => setFilterSlug(e.target.value)}
          className="bg-white border border-pink-200 rounded-lg px-3 py-1.5 text-sm text-brand-purple-dark focus:outline-none focus:border-brand-orange/40"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div key={r.id} className={`rounded-2xl border bg-white p-5 ${r.hidden ? "opacity-50 border-pink-200/50" : "border-white/8"}`}>
            <div className="flex items-start justify-between gap-3 mb-2">
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-brand-purple-dark/80">{r.location}</p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <span className="text-brand-amber text-sm">{"★".repeat(r.stars)}{"☆".repeat(5 - r.stars)}</span>
              </div>
            </div>
            {r.title && <p className="text-sm text-brand-purple-dark/80 font-semibold mb-1">{r.title}</p>}
            <p className="text-sm text-brand-purple-dark/80 leading-relaxed mb-3">{r.body}</p>
            <div className="flex items-center justify-between gap-2 pt-3 border-t border-pink-200/50">
              <span className="text-[10px] tracking-[0.2em] uppercase text-brand-purple-dark/80 truncate">
        ...
        {filtered.length === 0 && (
          <p className="text-brand-purple-dark/80 col-span-2 text-sm py-12 text-center">
            No reviews yet. Click <span className="text-brand-purple-dark">+ Add review</span> to create one.
          </p>
        )}
      </div>
              ...
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map(n => (
                  <button
                    ...
                    className={`text-2xl transition-colors ${n <= editing.stars ? "text-brand-amber" : "text-brand-purple-dark/80 hover:text-brand-purple-dark/80"}`}
                  >
                    ★
                  </button>
                ))}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <label className="flex items-center gap-2 p-3 rounded-lg border border-pink-200 cursor-pointer hover:border-white/20">
                <input
                  ...
                  className="w-4 h-4 accent-brand-orange"
                />
                <span className="text-sm text-brand-purple-dark">Hidden</span>
              </label>
            </div>

            <div className="flex items-center justify-end gap-3 pt-2">
              <button onClick={() => setEditing(null)} className="text-sm text-brand-purple-dark/80 hover:text-brand-purple-dark px-4 py-2">
                Cancel
              </button>
              <button
                onClick={handleSave}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="block text-[10px] tracking-[0.22em] uppercase text-brand-purple-dark/80 mb-1.5">{label}</span>
      {children}
    </label>
  );
}
