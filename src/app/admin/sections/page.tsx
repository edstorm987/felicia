    ...
    <div className="p-6 sm:p-8 lg:p-10 max-w-2xl space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Section layout</h1>
      ...
      <div className="space-y-2">
        {sections.map((s, i) => {
          ...
          return (
            ...
            >
              {/* Drag handle */}
              <div className={`flex flex-col gap-0.5 shrink-0 ${s.locked ? "opacity-20" : "cursor-grab active:cursor-grabbing text-brand-purple-dark/80 hover:text-brand-purple-dark/80"}`}>
              ...
              {/* Position badge */}
              <span className="text-[11px] text-brand-purple-dark/80 font-mono w-5 text-center shrink-0">{i + 1}</span>

              {/* Label */}
              <div className="flex-1 min-w-0">
                <p className={`text-sm font-medium ${s.visible ? "text-brand-purple-dark" : "text-brand-purple-dark/80"}`}>
                  {s.label}
                  {s.locked && <span className="ml-2 text-[10px] text-brand-purple-dark/80">locked</span>}
                </p>
                <p className="text-xs text-brand-purple-dark/80 mt-0.5">{s.description}</p>
              </div>
              ...
              <div className="flex flex-col gap-0.5 shrink-0">
                <button
                  ...
                  className="w-7 h-6 rounded text-brand-purple-dark/80 hover:text-brand-purple-dark/80 hover:bg-white/10 disabled:opacity-20 disabled:cursor-default text-xs flex items-center justify-center transition-colors"
                >
                  ↑
                </button>
                <button
      ...
      <div className="rounded-xl border border-white/8 p-4 text-xs text-brand-purple-dark/80 space-y-1.5">
        ...
        <p>• The <strong className="text-brand-purple-dark/80">Hero</strong> section is locked and always appears first.</p>
      </div>
    </div>
  );
}
