  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <header>
        ...
        <h1 className="font-display text-3xl text-brand-purple-dark">SEO</h1>
        <p className="text-[12px] text-brand-purple-dark/80 mt-1">Per-page meta, sitemap, robots — all read from the visual editor.</p>
      </header>

      {sites.length > 1 && (
        <div className="flex items-center gap-2 text-[12px]">
          <span className="text-brand-purple-dark/80">Site:</span>
          <select
            value={active?.id ?? ""}
            onChange={e => { const s = sites.find(x => x.id === e.target.value); if (s) setActive(s); }}
            className="bg-white/5 border border-pink-200 rounded-lg px-2 py-1 text-brand-purple-dark"
          >
      ...
      <section className="rounded-2xl border border-pink-200 bg-white/[0.02]">
        ...
        {pages.length === 0 ? (
          <p className="p-6 text-[12px] text-brand-purple-dark/80 text-center">No pages yet — build one in the visual editor.</p>
        ) : (
          <table className="w-full text-[12px]">
            ...
            <tbody>
              {pages.map(p => {
                ...
                return (
                  <tr key={p.id} className="border-b border-pink-200/50 hover:bg-white/[0.02]">
                    <td className="px-3 py-2 font-mono text-brand-purple-dark/80">{p.slug}</td>
                    <td className="px-3 py-2">
                      <p className="text-brand-purple-dark truncate max-w-[280px]" title={title}>{title}</p>
                      <p className="text-[10px] text-brand-purple-dark/80">{title.length} chars</p>
                    </td>
                    <td className="px-3 py-2">
                      <p className="text-brand-purple-dark/80 truncate max-w-[320px]" title={desc}>{desc || <span className="italic opacity-50">(none)</span>}</p>
                      <p className="text-[10px] text-brand-purple-dark/80">{desc.length} chars</p>
                    </td>
                    <td className="px-3 py-2">
                      ...
                      <p className="text-[10px] text-brand-purple-dark/80 mt-0.5">{score.label}</p>
                    </td>
      ...
      <section className="rounded-2xl border border-pink-200 bg-white/[0.02]">
        ...
        {links.length === 0 ? (
          <p className="p-4 text-[12px] text-brand-purple-dark/80">Click Scan to walk every page block and ping each external link.</p>
        ) : (
          <div className="p-2">
            <p className="text-[11px] text-brand-purple-dark/80 px-2 pb-2">
              ...
              <span className="text-brand-purple-dark/80">{links.filter(l => !l.external).length} internal</span>
            </p>
            <ul className="space-y-1 max-h-80 overflow-y-auto">
              {links.map((l, i) => (
                <li key={i} className={`flex items-start gap-2 px-2 py-1.5 rounded text-[11px] ${
                  l.external && l.ok === false ? "bg-red-500/5 border border-red-500/20" : "border border-pink-200/50 bg-white/[0.02]"
                }`}>
                  ...
                  <a href={l.url} target="_blank" rel="noopener noreferrer" className="flex-1 truncate font-mono text-brand-purple-dark/80 hover:underline">{l.url}</a>
                  <span className="text-brand-purple-dark/80 truncate max-w-[260px]">
      ...
      <div className="grid lg:grid-cols-2 gap-3">
        ...
        <section className="rounded-2xl border border-pink-200 bg-white/[0.02] overflow-hidden">
          ...
          <pre className="p-3 text-[10px] font-mono text-brand-purple-dark/80 overflow-auto max-h-80">{robots}</pre>
        </section>
      </div>
    </div>
  );
