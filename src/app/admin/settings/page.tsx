...
const STATUS_TONE: Record<IntegrationStatus, { label: string; cls: string }> = {
  ...
  "not-installed": { label: "Not installed",  cls: "bg-white/5 text-brand-purple-dark/80 border-pink-200" },
};
  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 space-y-8 max-w-4xl">
      ...
      <header>
        ...
        <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">{org?.name ?? "Portal"} · integrations</h1>
            ...
            {installedMeta.map(({ install, meta }) => (
              <article
                ...
                className={`rounded-xl border p-3 flex items-start justify-between gap-2 ${
                  install.enabled ? "border-pink-200 bg-white/[0.02]" : "border-pink-200/50 bg-white/[0.01] opacity-60"
                }`}
              >
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-0.5">
                    <p className="text-[13px] text-brand-purple-dark truncate">{meta.name}</p>
                    {meta.core && <span className="text-[9px] tracking-wider uppercase px-1.5 py-0.5 rounded bg-cyan-400/15 text-cyan-300">core</span>}
                    {!install.enabled && <span className="text-[9px] tracking-wider uppercase px-1.5 py-0.5 rounded bg-white/5 text-brand-purple-dark/80">disabled</span>}
                  </div>
                  <p className="text-[11px] text-brand-purple-dark/80 line-clamp-2">{meta.tagline}</p>
                </div>
                <Link
    ...
    <article className="rounded-xl border border-white/8 bg-white p-4 flex flex-col gap-2">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="text-[11px] text-brand-purple-dark/80">{subtitle}</p>
        </div>
        <span className={`text-[9px] tracking-wider uppercase px-1.5 py-0.5 rounded border ${tone.cls} shrink-0`}>
          {tone.label}
        </span>
      </div>
      {info.detail && <p className="text-[11px] text-brand-purple-dark/80 truncate" title={info.detail}>{info.detail}</p>}
      <Link
        href={ctaHref}
