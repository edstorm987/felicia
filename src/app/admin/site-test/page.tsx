  ...
  return (
    <main className="max-w-5xl mx-auto p-6 space-y-6">
      <header>
        ...
        <h1 className="font-display text-3xl text-brand-purple-dark">Test my website</h1>
      ...
      {quota?.quota && (
        <div className="flex items-center gap-3 text-[11px]">
          ...
          <span className={`px-2 py-1 rounded-full border ${quota.anthropicKeyConfigured ? "bg-green-500/10 text-green-400 border-green-500/20" : "bg-white/5 text-brand-purple-dark/80 border-pink-200"}`}>
            Claude formatter: {quota.anthropicKeyConfigured ? "on" : "off (raw findings only)"}
          </span>
          <span className="text-brand-purple-dark/80 ml-auto">
      ...
      <section className="rounded-2xl border border-pink-200 bg-white/[0.02] p-4 space-y-3">
        <div className="grid sm:grid-cols-[1fr_140px] gap-3">
          <label className="block">
            <span className="block text-[10px] tracking-[0.18em] uppercase text-brand-purple-dark/80 mb-1">URL to test</span>
            <input value={url} onChange={e => setUrl(e.target.value)} placeholder="https://example.com" className={INPUT + " font-mono"} />
          </label>
          <label className="block">
            <span className="block text-[10px] tracking-[0.18em] uppercase text-brand-purple-dark/80 mb-1">Form factor</span>
          ...
          <ul className="divide-y divide-white/5">
            {history.slice(0, 10).map(r => (
              <li key={r.id} className="px-4 py-2 flex items-center gap-3 text-[12px]">
                ...
                <span className="font-mono text-brand-purple-dark/80 truncate flex-1">{r.url}</span>
                <span className="text-brand-purple-dark/80">{r.strategy}</span>
                <span className="text-brand-purple-dark/80">{new Date(r.createdAt).toLocaleString()}</span>
...
function ReportView({ report, onReformat, llmAvailable }: { report: SiteAuditReport; onReformat: (id: string) => void; llmAvailable: boolean }) {
  if (report.status === "running") return <p className="text-[12px] text-brand-purple-dark/80 p-4">Running…</p>;
    ...
    <article className="rounded-2xl border border-pink-200 bg-white/[0.02] overflow-hidden">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 border-b border-white/8">
        {cats.map(cat => {
          ...
          const tone = score === undefined ? "text-brand-purple-dark/80"
      ...
      <div className="p-4 space-y-4">
        <p className="text-[10px] uppercase tracking-[0.18em] text-brand-purple-dark/80">Raw findings ({report.findings.length})</p>
        {cats.map(cat => {
                ...
                {findings.map(f => (
                  <li key={f.id} className="px-3 py-2.5">
                    <div className="flex items-baseline gap-2 mb-1">
                      ...
                      <p className="text-[12px] font-semibold text-brand-purple-dark flex-1">{f.title}</p>
                      {f.scoreDisplay && <code className="text-[10px] text-brand-amber font-mono">{f.scoreDisplay}</code>}
                    </div>
                    <p className="text-[11px] text-brand-purple-dark/80 leading-relaxed">{stripMarkdown(f.description)}</p>
                  </li>
                ))}
              </ul>
            </details>
...
function Vital({ label, value, unit, goodMax, okMax, digits = 0 }: { label: string; value?: number; unit: string; goodMax: number; okMax: number; digits?: number }) {
  const v = value ?? null;
  const tone = v === null ? "text-brand-purple-dark/80"
  ...
  return (
    <div>
      <p className="text-[10px] uppercase tracking-wider text-brand-purple-dark/80">{label}</p>
