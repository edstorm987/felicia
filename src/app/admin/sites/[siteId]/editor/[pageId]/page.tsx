    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6">
        <div className="max-w-md text-center">
          <p className="text-brand-purple-dark font-display text-2xl mb-2">{error}</p>
    ...
    <div className="fixed inset-0 flex flex-col bg-pink-50">
      ...
      <header className="h-12 shrink-0 flex items-center gap-2 px-3 border-b border-white/8 bg-pink-50/50">
        ...
        <input
          ...
          className="bg-transparent text-brand-purple-dark text-sm font-medium focus:outline-none px-2 py-1 rounded hover:bg-white/5 focus:bg-white/5"
        />
        <span className="text-[11px] text-brand-purple-dark/80 font-mono">{page?.slug}</span>
        <span className="ml-auto" />
        {/* View toggle: simple text mode ↔ visual canvas ↔ raw JSON code */}
        <div className="flex items-center gap-0.5 border border-pink-200 rounded-lg p-0.5 mr-2">
          <button
            onClick={() => setView("text")}
            title="Simplest mode — edit only text + colours"
            className={`px-2.5 py-1 text-[11px] rounded ${view === "text" ? "bg-white/10 text-brand-purple-dark" : "text-brand-purple-dark/80 hover:text-brand-purple-dark"}`}
          >Text</button>
          <button
            onClick={() => setView("visual")}
            title="Drag-drop blocks, layout, styles"
            className={`px-2.5 py-1 text-[11px] rounded ${view === "visual" ? "bg-white/10 text-brand-purple-dark" : "text-brand-purple-dark/80 hover:text-brand-purple-dark"}`}
          >Visual</button>
          <button
            onClick={() => setView("code")}
            title="Raw JSON + custom code"
            className={`px-2.5 py-1 text-[11px] rounded ${view === "code" ? "bg-white/10 text-brand-purple-dark" : "text-brand-purple-dark/80 hover:text-brand-purple-dark"}`}
          >Code</button>
        </div>
        <button
          onClick={handleUndo}
          ...
          title="Undo (⌘Z)"
          className="text-[16px] w-7 h-7 rounded text-brand-purple-dark/80 hover:text-brand-purple-dark hover:bg-white/5 disabled:opacity-25"
        >↶</button>
        <button
          onClick={handleRedo}
          disabled={redoStack.current.length === 0}
          title="Redo (⌘⇧Z)"
          className="text-[16px] w-7 h-7 rounded text-brand-purple-dark/80 hover:text-brand-purple-dark hover:bg-white/5 disabled:opacity-25 mr-2"
        >↷</button>
        ...
        {themes.length > 0 && (
          <select
            ...
            className="bg-white/5 border border-pink-200 rounded-lg px-2 py-1 text-[11px] text-brand-purple-dark mr-2 focus:outline-none focus:border-brand-orange/50"
          >
            {themes.map(t => (
        ...
        {page && page.themeId !== activeThemeId && (
          <button
            ...
            className="text-[11px] text-brand-purple-dark/80 hover:text-brand-orange mr-2"
          >
            Pin
          </button>
        )}
        <DevicePreview state={deviceState} onChange={s => { setDeviceState(s); saveDeviceState(s); }} />
        <span className="text-[11px] text-brand-purple-dark/80 mx-3">
          {saving ? "Saving…" : savedAt ? "Saved" : page?.status === "published" ? "Published" : "Draft"}
        </span>
        <button
          onClick={handleRevert}
          disabled={!!busy || !page?.publishedBlocks}
          className="text-[12px] px-2.5 py-1.5 rounded-lg border border-pink-200 text-brand-purple-dark/80 hover:text-brand-purple-dark hover:bg-white/5 disabled:opacity-30"
        >
          Revert
        </button>
        <button
      ...
      {toast && (
        <div className="pointer-events-none absolute bottom-4 left-1/2 -translate-x-1/2 px-4 py-2 rounded-full bg-brand-cream text-brand-purple-dark text-[12px] font-medium shadow-lg animate-fade-in">
          {toast}
        </div>
      )}
    </div>
...
function TextView({
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto bg-pink-50">
      <div className="max-w-2xl mx-auto py-8 px-6 space-y-4">
        <div className="rounded-2xl border border-cyan-500/30 bg-cyan-500/5 p-4 text-[12px] text-cyan-400/85 leading-relaxed">
          Simple mode — edit the words on every block + the brand colour. Need more? Switch to <strong className="text-brand-purple-dark">Visual</strong> in the top bar.
        </div>
        {editable.length === 0 && (
          <p className="text-[12px] text-brand-purple-dark/80 text-center py-12">
        ...
        <div key={field.key} className="mb-2 last:mb-0">
          {field.kind === "input" ? (
            <input
              ...
              className="w-full bg-white/5 border border-pink-200 rounded-lg px-3 py-2 text-sm text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-brand-orange/50"
            />
          ) : (
            <textarea
              key={`${block.id}-${field.key}-${String(block.props[field.key] ?? "")}`}
          ...
          {field.label && <p className="text-[10px] text-brand-purple-dark/80 mt-1">{field.label}</p>}
        </div>
      ))}
      {/* Always offer a text-colour picker on text-bearing blocks. */}
      <label className="flex items-center gap-2 mt-2">
        <span className="text-[10px] uppercase tracking-[0.18em] text-brand-purple-dark/80 w-24">Text colour</span>
        <input
          type="color"
          defaultValue={(block.styles?.textColor as string | undefined) ?? "#ffffff"}
          onChange={e => onPatchStyles(block.id, { textColor: e.target.value })}
          className="w-8 h-7 rounded cursor-pointer bg-transparent border border-pink-200"
        />
        <button
          onClick={() => onPatchStyles(block.id, { textColor: undefined })}
          className="text-[10px] text-brand-purple-dark/80 hover:text-brand-orange ml-auto"
        >
          Reset
        </button>
      </label>
...
function SeoPanel({ page, onPageMutate }: { page: EditorPage; onPageMutate: (patch: Partial<EditorPage>) => void }) {
  ...
  return (
    <div className="rounded-2xl border border-white/8 bg-pink-50/50 p-3 space-y-3">
      ...
      <label className="block">
        <span className="block text-[10px] uppercase tracking-[0.18em] text-brand-purple-dark/80 mb-1">Meta description</span>
      ...
      <div className="grid grid-cols-2 gap-2">
        <label className="block">
          <span className="block text-[10px] uppercase tracking-[0.18em] text-brand-purple-dark/80 mb-1">Canonical</span>
        ...
        <label className="block">
          <span className="block text-[10px] uppercase tracking-[0.18em] text-brand-purple-dark/80 mb-1">og:image</span>
      ...
      <div className="grid grid-cols-2 gap-2">
        <label className="block">
          <span className="block text-[10px] uppercase tracking-[0.18em] text-brand-purple-dark/80 mb-1">og:type</span>
        ...
        <label className="block">
          <span className="block text-[10px] uppercase tracking-[0.18em] text-brand-purple-dark/80 mb-1">twitter:card</span>
      ...
      <div className="grid grid-cols-3 gap-2">
        <label className="flex items-center gap-2 text-[11px] text-brand-purple-dark/80">
      ...
      <div className="grid grid-cols-2 gap-2">
        <label className="block">
          <span className="block text-[10px] uppercase tracking-[0.18em] text-brand-purple-dark/80 mb-1">priority</span>
        ...
        <label className="block">
          <span className="block text-[10px] uppercase tracking-[0.18em] text-brand-purple-dark/80 mb-1">changefreq</span>
      ...
      <label className="block">
        <span className="block text-[10px] uppercase tracking-[0.18em] text-brand-purple-dark/80 mb-1">Custom JSON-LD (advanced)</span>
  ...
  return (
    <div className="rounded-2xl border border-white/8 bg-pink-50/50 overflow-hidden flex flex-col">
      ...
      <textarea
        ...
        spellCheck={false}
        className={`${height} bg-transparent p-3 text-[11px] font-mono leading-relaxed text-brand-purple-dark resize-none focus:outline-none`}
