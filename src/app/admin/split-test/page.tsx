...
const STATUS_STYLE: Record<ABTestStatus, string> = {
  draft: "bg-white/10 text-brand-purple-dark/80",
  running: "bg-green-500/20 text-green-400",
  paused: "bg-brand-amber/20 text-brand-amber",
  completed: "bg-white/5 text-brand-purple-dark/80",
};
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/80 backdrop-blur-sm p-4 overflow-y-auto">
      <form onSubmit={submit} className="w-full max-w-2xl bg-pink-50/50 border border-pink-200 rounded-2xl my-6 overflow-hidden">
        <div className="p-5 border-b border-pink-200/50 flex items-center justify-between">
          ...
          <button type="button" onClick={onClose} className="text-brand-purple-dark/80 hover:text-brand-purple-dark text-xl">✕</button>
        </div>
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <F label="Target path (URL to intercept)" tipId="split-test.target-path" tip="The URL where visitors land. They'll then be redirected to one of the variants (preserving query strings).">
              ...
              <p className="text-[11px] text-brand-purple-dark/80 mt-1">Visitors to this URL are split between variants.</p>
            </F>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-semibold uppercase tracking-[0.15em] text-brand-purple-dark/80">Variants</p>
              ...
              {variants.map((v, i) => (
                <div key={v.id} className="rounded-xl border border-pink-200/50 p-3 space-y-2">
                  ...
                  <details className="group">
                    <summary className="text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark/80 cursor-pointer list-none flex items-center gap-1">
        ...
        <div className="p-5 border-t border-pink-200/50 flex justify-end gap-2">
          <button type="button" onClick={onClose} className="text-sm px-4 py-2 text-brand-purple-dark/80 hover:text-brand-purple-dark">Cancel</button>
...
export default function AdminSplitTestPage() {
  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 max-w-5xl space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-2">
            <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Split testing</h1>
          ...
          <p className="text-brand-purple-dark/80 text-sm mt-1">A/B test landing pages and measure which variant converts better.</p>
        </div>
      ...
      {tests.length === 0 && (
        <div className="text-center py-16 border border-dashed border-pink-200 rounded-2xl">
          <p className="text-brand-purple-dark/80 text-sm">No tests yet. Create one to start optimising.</p>
            ...
            <div key={test.id} className="rounded-xl border border-pink-200/50 bg-white/[0.02] overflow-hidden">
              <div className="flex flex-wrap items-center gap-3 px-5 py-4 border-b border-pink-200/50">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h3 className="text-sm font-semibold text-brand-purple-dark">{test.name}</h3>
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${STATUS_STYLE[test.status]}`}>{test.status}</span>
                  </div>
                  <p className="text-xs text-brand-purple-dark/80 mt-0.5">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  {(test.status === "running" || test.status === "paused") && <button onClick={() => setABTestStatus(test.id, "completed")} className="text-xs px-3 py-1.5 border border-pink-200 text-brand-purple-dark/80 rounded-lg">End</button>}
                  <button onClick={() => setModal(test)} className="text-xs px-3 py-1.5 border border-pink-200 text-brand-purple-dark/80 hover:text-brand-purple-dark rounded-lg">Edit</button>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-pink-200/50 bg-white/[0.01]">
                      <th className="text-left px-5 py-2 text-xs text-brand-purple-dark/80 font-medium">Variant</th>
                      <th className="text-right px-4 py-2 text-xs text-brand-purple-dark/80 font-medium">Weight</th>
                      <th className="text-right px-4 py-2 text-xs text-brand-purple-dark/80 font-medium">Views</th>
                      <th className="text-right px-4 py-2 text-xs text-brand-purple-dark/80 font-medium">Conversions</th>
                      <th className="text-right px-4 py-2 text-xs text-brand-purple-dark/80 font-medium">Conv. rate</th>
                      <th className="text-right px-5 py-2 text-xs text-brand-purple-dark/80 font-medium">Page</th>
                    </tr>
                  </thead>
                  <tbody>
                    {test.variants.map((v) => {
                      ...
                      return (
                        <tr key={v.id} className={`border-b border-pink-200/50 last:border-0 ${isTop ? "bg-green-500/5" : ""}`}>
                          <td className="px-5 py-2.5 text-brand-purple-dark/80">
                            <span className="flex items-center gap-2">
                              {v.name}
                              {isTop && <span className="text-[10px] bg-green-500/20 text-green-400 px-1.5 py-0.5 rounded-full">winning</span>}
                            </span>
                          </td>
                          <td className="px-4 py-2.5 text-right text-brand-purple-dark/80 font-mono">{v.weight}%</td>
                          <td className="px-4 py-2.5 text-right text-brand-purple-dark/80 font-mono">{s.views.toLocaleString()}</td>
                          <td className="px-4 py-2.5 text-right text-brand-purple-dark/80 font-mono">{s.conversions.toLocaleString()}</td>
                          <td className="px-4 py-2.5 text-right font-mono font-semibold text-brand-purple-dark">{convRate(s.views, s.conversions)}</td>
                          <td className="px-5 py-2.5 text-right text-xs text-brand-purple-dark/80">{v.pageSlug ? `/p/${v.pageSlug}` : "Original"}</td>
                        </tr>
                      );
                    })}
                  </tbody>
...
function F({ label, tip, tipId, children }: { label?: string; tip?: string; tipId?: string; children: React.ReactNode }) {
  return (
    <div>
      {label && (
        <label className="text-xs text-brand-purple-dark/80 mb-1.5 flex items-center gap-1.5">
          {label}
