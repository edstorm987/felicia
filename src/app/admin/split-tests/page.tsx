...
function SplitTestsPageInner() {
  ...
  if (!site) return <main className="p-6 text-[12px] text-brand-purple-dark/80">No active site.</main>;
    ...
    <main className="max-w-5xl mx-auto p-6 space-y-6">
      <header className="flex items-baseline justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">Split tests</h1>
          <p className="text-[12px] text-brand-purple-dark/80 mt-1">Site: {site.name}</p>
        </div>
      ...
      <section className="space-y-3">
        {groups.length === 0 && <p className="text-[12px] text-brand-purple-dark/80 text-center py-8">No split-test groups yet. Create one above, then assign blocks to it from the visual editor&apos;s Split tab.</p>}
        {groups.map(g => {
          ...
          return (
            <details key={g.id} className="rounded-2xl border border-pink-200 bg-white/[0.02] overflow-hidden">
              ...
              <div className="p-4 border-t border-white/8 space-y-3">
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-pink-200/50">
                  <span className="text-[10px] text-brand-purple-dark/80">Status:</span>
                  {(["draft", "running", "paused", "completed"] as const).map(s => (
                    <button key={s} onClick={() => patchGroup(g.id, { setStatus: s })} className={`text-[10px] px-2 py-0.5 rounded-full transition-colors ${g.status === s ? statusTone(s) : "text-brand-purple-dark/80 hover:text-brand-purple-dark"}`}>{s}</button>
                  ))}
                  <button onClick={async () => { if (await confirm({ title: "Delete this group + all its results?", danger: true, confirmLabel: "Delete" })) { await deleteGroup(g.id); void refresh(); } }} className="ml-auto text-[10px] text-brand-purple-dark/80 hover:text-red-400">Delete</button>
                </div>
                    ...
                    <table className="w-full text-[11px]">
                      ...
                      <tbody>
                        {groupResults.map(r => {
                          ...
                          return (
                            <tr key={r.variantId} className="border-b border-pink-200/50">
                              <td className="px-3 py-1.5 font-mono text-brand-purple-dark/80">{r.variantId}</td>
                ...
                <p className="text-[10px] text-brand-purple-dark/80 leading-relaxed">
                  ...
                  <code className="font-mono text-brand-purple-dark/80">recordConversion(&quot;{g.id}&quot;, variantId)</code> from your storefront.
                </p>
              </div>
            </details>
          );
