  ...
  if (!ticket) {
    return (
      <div className="p-6 sm:p-8 lg:p-10 max-w-3xl">
        <Link href="/admin/support" className="text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark">← Support</Link>
        <p className="mt-6 text-brand-purple-dark/80">Ticket not found.</p>
      </div>
    );
  }
    ...
    <div className="p-6 sm:p-8 lg:p-10 max-w-4xl space-y-5">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h1 className="font-display text-2xl sm:text-3xl text-brand-purple-dark">{ticket.subject}</h1>
          <p className="text-brand-purple-dark/80 text-sm mt-1">
            {ticket.customerName} &lt;<span className="text-brand-purple-dark/80">{ticket.customerEmail}</span>&gt;
        ...
        <div className="flex flex-wrap items-center gap-2">
          <select value={ticket.status} onChange={e => setTicketStatus(ticket.id, e.target.value as TicketStatus)} className="bg-pink-50 border border-pink-200 rounded px-2 py-1.5 text-xs text-brand-purple-dark">
            {STATUSES.map(s => <option key={s}>{s}</option>)}
          </select>
          <select value={ticket.priority} onChange={e => setTicketPriority(ticket.id, e.target.value as TicketPriority)} className="bg-pink-50 border border-pink-200 rounded px-2 py-1.5 text-xs text-brand-purple-dark">
            {PRIORITIES.map(p => <option key={p}>{p}</option>)}
          </select>
          <button onClick={remove} className="text-xs text-brand-purple-dark/80 hover:text-brand-orange">Delete</button>
        </div>
      </div>
        ...
        {ticket.messages.map(m => (
          ...
          >
            <div className="flex items-center justify-between text-[11px] text-brand-purple-dark/80 mb-2">
              <span>
                <span className="font-semibold text-brand-purple-dark/80">{m.authorName}</span>
                {m.internal && <span className="ml-2 text-brand-amber font-bold uppercase tracking-widest">internal note</span>}
              </span>
              <span>{new Date(m.createdAt).toLocaleString()}</span>
            </div>
            <p className="text-sm text-brand-purple-dark/80 whitespace-pre-wrap leading-relaxed">{m.body}</p>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-2xl border border-pink-200/50 bg-white p-4 space-y-3">
        <textarea
          ...
          rows={5}
          className="w-full bg-pink-50 border border-pink-200 rounded-lg px-3 py-2.5 text-sm text-brand-purple-dark/80 focus:outline-none focus:border-brand-orange/40 resize-y"
        />
        <div className="flex items-center justify-between gap-2">
          <label className="inline-flex items-center gap-2 text-xs text-brand-purple-dark/80 cursor-pointer">
