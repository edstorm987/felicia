  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-6xl">
      <div>
        ...
        <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Customer tickets</h1>
        <p className="text-brand-purple-dark/80 text-sm mt-1">{tickets.length} total · {counts.open ?? 0} open · {counts.pending ?? 0} awaiting customer.</p>
      </div>
      ...
      <div className="flex flex-wrap gap-1 text-xs">
        {STATUS_FILTERS.map(f => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`px-3 py-1.5 rounded-full ${filter === f ? "bg-brand-orange/20 text-brand-orange border border-brand-orange/30" : "border border-pink-200 text-brand-purple-dark/80 hover:text-brand-purple-dark"}`}
          >
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border border-pink-200/50 bg-white px-6 py-10 text-center">
          <p className="text-brand-purple-dark/80 text-sm">No tickets in this view.</p>
        </div>
      ) : (
              ...
              <Link key={t.id} href={`/admin/support/${t.id}`} className="flex items-center gap-4 px-5 py-4 hover:bg-white/[0.02]">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className="text-sm font-medium text-brand-purple-dark truncate">{t.subject}</span>
                    <StatusPill status={t.status} />
                    <PriorityPill priority={t.priority} />
                    {t.orderId && <span className="text-[10px] text-brand-purple-dark/80">{t.orderId}</span>}
                  </div>
                  <p className="text-xs text-brand-purple-dark/80 truncate">
                    <span className="text-brand-purple-dark/80">{t.customerName}</span> · {last?.body.slice(0, 100)}
                  </p>
                </div>
                <div className="text-right text-[11px] text-brand-purple-dark/80 shrink-0">
                  <p>{t.id}</p>
...
function StatusPill({ status }: { status: TicketStatus }) {
  const cls = {
    ...
    closed: "bg-white/10 text-brand-purple-dark/80",
  }[status];
