  ...
  return (
    ...
    >
      <form
        ...
        className="w-full max-w-md bg-pink-50/50 border border-pink-200 rounded-2xl p-6 space-y-4"
        role="dialog"
        aria-modal="true"
        aria-labelledby="invite-title"
      >
        <h2 id="invite-title" className="font-display text-xl text-brand-purple-dark">Invite team member</h2>
        <Field label="Name">
          <input
            required
            value={name}
            onChange={(e) => setName(e.target.value)}
            className="w-full bg-white/5 border border-pink-200 rounded-xl px-3 py-2 text-sm text-brand-purple-dark focus:outline-none focus:border-brand-orange/50"
          />
        </Field>
        <Field label="Email">
          <input
        ...
        <div className="flex gap-2 justify-end pt-2">
          <button type="button" onClick={onClose} disabled={busy} className="px-4 py-2 text-sm text-brand-purple-dark/80 hover:text-brand-purple-dark disabled:opacity-40">Cancel</button>
...
function RoleEditor({ role, onSave, onDelete, onCancel }: {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-3xl bg-pink-50/50 border border-pink-200 rounded-2xl my-6 overflow-hidden"
        onClick={e => e.stopPropagation()}
        role="dialog"
        aria-modal="true"
        aria-labelledby="role-editor-title"
      >
        <div className="p-6 border-b border-pink-200/50 flex items-center justify-between gap-4">
          <h2 id="role-editor-title" className="font-display text-xl text-brand-purple-dark">
            {role.isSystem ? `Role: ${role.name}` : "Edit role"}
          </h2>
          <button onClick={onCancel} aria-label="Close" className="text-brand-purple-dark/80 hover:text-brand-purple-dark text-xl leading-none">✕</button>
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          {/* Permission matrix */}
          <div>
            <p className="text-xs font-semibold tracking-[0.15em] uppercase text-brand-purple-dark/80 mb-3">Permissions</p>
            {role.isSystem && (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {groups.map((group) => {
                    ...
                    return (
                      <>
                        <tr key={`group-${group}`} className="border-b border-pink-200/50 bg-white/[0.01]">
                          <td colSpan={6} className="px-4 py-1.5 text-[10px] tracking-[0.18em] uppercase text-brand-purple-dark/80 font-semibold">{group}</td>
                        </tr>
                        {resources.map((r) => (
                          <tr key={r.resource} className="border-b border-pink-200/50 last:border-0 hover:bg-white/[0.015]">
                            <td className="px-4 py-2.5 text-brand-purple-dark/80 text-sm">{r.label}</td>
                            {ALL_ACTIONS.map((a) => {
        ...
        <div className="p-5 border-t border-pink-200/50 flex items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            <button onClick={onCancel} className="text-xs px-4 py-2 text-brand-purple-dark/80 hover:text-brand-purple-dark">Cancel</button>
            {!role.isSystem && (
              <button
...
function CreateUserModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md bg-pink-50/50 border border-pink-200 rounded-2xl overflow-hidden"
        onClick={e => e.stopPropagation()}
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="p-5 border-b border-pink-200/50 flex items-center justify-between">
          <h2 id="create-user-title" className="font-display text-xl text-brand-purple-dark">Create user account</h2>
          <button onClick={onClose} aria-label="Close" className="text-brand-purple-dark/80 hover:text-brand-purple-dark text-xl">✕</button>
        </div>
        ...
        {result ? (
          <div className="p-6 space-y-4">
            <div className="rounded-xl bg-green-500/10 border border-green-500/20 p-4 space-y-2">
              ...
              <p className="text-xs text-brand-purple-dark/80">Share these credentials with the user — they&apos;ll be forced to set their own password on first login.</p>
            </div>
            <div className="rounded-xl bg-white/5 border border-pink-200 p-4 space-y-2 font-mono text-sm">
              <div className="flex justify-between gap-2">
                <span className="text-brand-purple-dark/80">Email</span>
                <span className="text-brand-purple-dark">{result.email}</span>
              </div>
              <div className="flex justify-between gap-2">
                <span className="text-brand-purple-dark/80">Temp password</span>
                <span className="text-brand-amber font-bold">{result.pass}</span>
              </div>
            </div>
            <p className="text-[11px] text-brand-purple-dark/80 text-center">Once they log in and set their own password, this temp password will stop working.</p>
          ...
          <form onSubmit={submit} className="p-5 space-y-4">
            ...
            <F label="Temporary password">
              <div className="flex gap-2">
                ...
                <button type="button" onClick={generate} className="shrink-0 px-3 py-2 text-xs bg-white/5 border border-pink-200 rounded-xl text-brand-purple-dark/80 hover:text-brand-purple-dark hover:border-white/25">Generate</button>
              </div>
              <p className="text-[11px] text-brand-purple-dark/80 mt-1">User must change this on first login.</p>
            </F>
    ...
    <div className="p-6 sm:p-8 lg:p-10 max-w-5xl space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Team &amp; users</h1>
      ...
      {tab === "users" && (
        <div className="space-y-3">
          <div className="rounded-xl border border-white/8 p-4 bg-brand-amber/5 text-xs text-brand-purple-dark/80 space-y-1">
            ...
            <p>
              <strong className="text-brand-purple-dark/80">Impersonate</strong> signs you in as the user so you see exactly what they
          ...
          {users.length === 0 && (
            <div className="text-center py-12 border border-dashed border-pink-200 rounded-2xl">
              <p className="text-brand-purple-dark/80 text-sm">No user accounts yet.</p>
            ...
            <div key={u.email} className="flex items-center gap-3 px-4 py-3.5 rounded-xl bg-white/[0.02] border border-white/8 hover:border-white/15">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="text-sm font-medium text-brand-purple-dark truncate">{u.name}</p>
                  {u.mustChangePassword && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-brand-amber/20 text-brand-amber">must change pw</span>
                  )}
                </div>
                <p className="text-xs text-brand-purple-dark/80 truncate">{u.email}</p>
              </div>
              ...
              {/* Provider */}
              <span className="text-[10px] text-brand-purple-dark/80 font-mono shrink-0 hidden sm:block">{u.provider}</span>
      ...
      {tab === "members" && (
        <div className="space-y-2">
          {members.length === 0 && (
            <div className="text-center py-12 border border-dashed border-pink-200 rounded-2xl">
              <p className="text-brand-purple-dark/80 text-sm">No team members yet.</p>
          ...
          {members.map((m) => {
            ...
            return (
              <div key={m.id} className="flex items-center gap-4 px-4 py-3.5 rounded-xl bg-white/[0.02] border border-white/8 hover:border-white/15">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-brand-purple-dark truncate">{m.name}</p>
                  <p className="text-xs text-brand-purple-dark/80 truncate">{m.email}</p>
                </div>
                {role && (
                  ...
                  {editMember === m.id ? (
                    <div className="flex items-center gap-2">
                      <select
                        ...
                        className="text-xs bg-white/5 border border-pink-200 rounded-lg px-2 py-1 text-brand-purple-dark"
                      >
                      ...
                      <button onClick={() => setEditMember(null)} className="text-xs text-brand-purple-dark/80 hover:text-brand-purple-dark px-1">✕</button>
                    </div>
                  ) : (
                    <>
            ...
            >
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  {r.isSystem && <p className="text-[10px] text-brand-purple-dark/80">System</p>}
                </div>
              </div>
              <p className="text-xs text-brand-purple-dark/80 mb-3 line-clamp-2">{r.description}</p>
...
const INPUT = "w-full bg-white/5 border border-pink-200 rounded-xl px-3 py-2 text-sm text-brand-purple-dark focus:outline-none focus:border-brand-orange/50";

function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-xs text-brand-purple-dark/80 mb-1.5">{label}</label>
      {children}
    </div>
  );
}
