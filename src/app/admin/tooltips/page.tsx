      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-2">
            <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Tooltips</h1>
            <Tip
              id="tooltips.header"
          ...
          <p className="text-brand-purple-dark/80 text-sm mt-1">
            ...
            <span className="text-brand-purple-dark/80 ml-2">{editedCount} edited · {totalCount} total</span>
          </p>
        </div>
        <button
          onClick={() => {
          ...
          {(["all", "edited", "default"] as const).map(f => (
            <button
              ...
              className={`px-3 py-1 rounded-md text-xs capitalize transition-colors ${
                filter === f ? "bg-brand-orange text-white" : "text-brand-purple-dark/80 hover:text-brand-purple-dark"
              }`}
            >
                ...
                return (
                  <div key={id} className="px-5 py-4">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      <span className="font-mono text-[11px] text-brand-purple-dark/80">{id}</span>
                      {edited && (
                      ...
                      <span className="ml-auto text-[11px] text-brand-purple-dark/80">{def.location}</span>
                    </div>
                    <textarea
                      value={current}
                      ...
                      className="w-full bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-sm text-brand-purple-dark/80 leading-relaxed focus:outline-none focus:border-brand-orange/40 resize-y"
                    />
                    <div className="mt-2 flex items-center gap-2">
                      ...
                      {edited && (
                        <button
                          ...
                          className="text-[11px] px-3 py-1.5 rounded-lg border border-pink-200 text-brand-purple-dark/80 hover:text-brand-purple-dark"
                        >
                          Reset to default
                        </button>
                      )}
                    ...
                    {edited && !dirty && (
                      <p className="text-[11px] text-brand-purple-dark/80 mt-1.5">
