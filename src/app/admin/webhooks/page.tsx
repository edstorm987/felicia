    ...
    <main className="max-w-4xl mx-auto px-6 py-8 space-y-6">
      <header className="flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">Outbound webhooks</h1>
          <p className="text-[12px] text-brand-purple-dark/80 mt-1">Subscribe external systems to internal events. HMAC-signed, retried, logged.</p>
        </div>
      ...
      {creating && (
        <section className="rounded-xl border border-pink-200 bg-white/[0.02] p-4 space-y-3">
          <div>
            <label className="text-[11px] text-brand-purple-dark/80 block mb-1">URL</label>
            <input
              type="url"
              value={form.url}
              onChange={e => setForm({ ...form, url: e.target.value })}
              placeholder="https://example.com/webhooks/aqua"
              className="w-full bg-white/5 border border-pink-200 rounded-md px-3 py-2 text-[12px] text-brand-purple-dark"
            />
          </div>
          <div>
            <label className="text-[11px] text-brand-purple-dark/80 block mb-1">Events (* = all)</label>
                ...
                return (
                  <button
                    ...
                    className={`px-2 py-1 rounded-md text-[10px] font-mono transition-colors ${
                      active ? "bg-cyan-500/15 text-cyan-200 border border-cyan-400/20" : "bg-white/5 text-brand-purple-dark/80 hover:text-brand-purple-dark"
                    }`}
                  >
                    {ev}
                  </button>
          ...
          <div>
            <label className="text-[11px] text-brand-purple-dark/80 block mb-1">Description (optional)</label>
            <input
              type="text"
      ...
      {loading ? (
        ...
        <p className="text-[12px] text-brand-purple-dark/80">No webhooks yet.</p>
      ) : (
        <section className="space-y-2">
          {hooks.map(h => (
            <article key={h.id} className="rounded-xl border border-pink-200/50 bg-white/[0.02] p-4 space-y-2">
              <div className="flex items-start justify-between gap-3 flex-wrap">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-[12px] text-brand-purple-dark font-mono truncate">{h.url}</p>
                    <span className={`text-[9px] tracking-wider uppercase px-1.5 py-0.5 rounded ${
                      h.enabled ? "bg-emerald-500/10 text-emerald-300" : "bg-white/5 text-brand-purple-dark/80"
                    }`}>
                      {h.enabled ? "active" : "disabled"}
                    </span>
                  </div>
                  {h.description && <p className="text-[11px] text-brand-purple-dark/80 mt-0.5">{h.description}</p>}
                  <p className="text-[10px] text-brand-purple-dark/80 font-mono mt-1">{h.events.join(", ")}</p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={() => toggle(h.id, !h.enabled)} className="text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark">
              ...
              <details>
                <summary className="text-[10px] text-brand-purple-dark/80 cursor-pointer">Show secret</summary>
                <code className="block mt-2 text-[11px] font-mono text-brand-purple-dark/80 break-all">{h.secret}</code>
              </details>
            </article>
          ))}
        </section>
