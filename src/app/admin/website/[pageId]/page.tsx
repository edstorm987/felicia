  ...
  if (!schema) {
    return (
      <div className="p-6 sm:p-8 lg:p-10 max-w-3xl">
        <Link href="/admin/website" className="text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark">← Website</Link>
        <p className="mt-6 text-brand-purple-dark/80">No page schema found for &quot;{pageId}&quot;.</p>
      </div>
    );
  }
    ...
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-4xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">{schema.label}</h1>
          {schema.description && (
            <p className="text-brand-purple-dark/80 text-sm mt-1">{schema.description}</p>
          )}
          <p className="text-brand-purple-dark/80 text-xs mt-2">
            {draftCount > 0 && <span className="text-brand-amber font-semibold">{draftCount} unpublished draft{draftCount === 1 ? "" : "s"}</span>}
            {draftCount > 0 && " · "}
            <a href={schema.href} target="_blank" rel="noreferrer" className="underline underline-offset-4 hover:text-brand-purple-dark">
              View live →
            </a>
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          {draftCount > 0 && (
            <>
              <button
                onClick={discardAll}
                className="text-xs px-3 py-2 rounded-lg border border-pink-200 text-brand-purple-dark/80 hover:text-brand-orange hover:border-brand-orange/30 transition-colors"
              >
                Discard drafts
              </button>
              <button
          ...
          {editsCount > 0 && (
            <button
              onClick={resetAll}
              className="text-xs px-3 py-2 rounded-lg border border-pink-200 text-brand-purple-dark/80 hover:text-brand-purple-dark/80 transition-colors"
            >
              Reset all
            </button>
          )}
    ...
    <div className="rounded-2xl border border-white/8 bg-white overflow-hidden">
      ...
      <div className="p-5 flex flex-col sm:flex-row gap-5 items-start">
        <div className={`shrink-0 rounded-xl border ${palette.border} ${palette.bg} px-5 py-4 text-center min-w-[110px]`}>
          <div className={`text-3xl font-semibold ${palette.fg}`}>{result.score}<span className="text-base text-brand-purple-dark/80">/100</span></div>
          <div className={`text-[10px] tracking-[0.2em] uppercase mt-1 ${palette.fg}`}>{grade}</div>
        </div>
        <div className="flex-1 min-w-0">
          {top.length === 0 ? (
            <p className="text-sm text-brand-purple-dark/80">All key checks passing — looking solid.</p>
          ) : (
            <>
              <p className="text-[11px] text-brand-purple-dark/80 mb-2">Top suggestions</p>
              <ul className="space-y-1.5 text-sm text-brand-purple-dark/80 list-disc pl-4">
                {top.map((s, i) => (<li key={i}>{s}</li>))}
              </ul>
              {result.suggestions.length > top.length && (
                <p className="text-[11px] text-brand-purple-dark/80 mt-2">+ {result.suggestions.length - top.length} more — fix the SEO fields above to raise the score.</p>
              )}
            </>
          )}
        </div>
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-baseline justify-between gap-3">
        <label className="text-xs font-medium text-brand-purple-dark/80">
          {field.label}
          ...
          {savedFlash && <span className="ml-2 text-[10px] text-brand-purple-dark/80">saved</span>}
        </label>
        <div className="flex items-center gap-2">
          {draftPending && (
            <>
              <button onClick={publish} className="text-[10px] text-brand-orange hover:underline">publish</button>
              <button onClick={discard} className="text-[10px] text-brand-purple-dark/80 hover:text-brand-purple-dark">discard</button>
            </>
          )}
          {isOverride && !draftPending && (
            <button onClick={reset} className="text-[10px] text-brand-purple-dark/80 hover:text-brand-purple-dark underline underline-offset-2">reset</button>
          )}
        </div>
      </div>
      {field.hint && <p className="text-[11px] text-brand-purple-dark/80">{field.hint}</p>}
      ...
      {(field.type === "text" || field.type === "url") && (
        <input
          ...
          className="w-full bg-pink-50 border border-pink-200 rounded-lg px-3 py-2.5 text-sm text-brand-purple-dark focus:outline-none focus:border-brand-orange/40"
        />
      )}
      ...
      {field.type === "textarea" && (
        <textarea
          ...
          onBlur={() => save(draft)}
          rows={4}
          className="w-full bg-pink-50 border border-pink-200 rounded-lg px-3 py-2.5 text-sm text-brand-purple-dark focus:outline-none focus:border-brand-orange/40 resize-y leading-relaxed"
        />
      )}
      ...
      {field.type === "code" && (
        <textarea
          ...
          className="w-full bg-pink-50 border border-pink-200 rounded-lg px-3 py-2.5 text-xs text-brand-purple-dark font-mono focus:outline-none focus:border-brand-orange/40 resize-y leading-relaxed"
        />
      )}
      ...
      {field.type === "boolean" && (
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-brand-purple-dark/80">{draft === "true" ? "On" : "Off"}</span>
        </div>
      )}
  ...
  return (
    <div className="space-y-2">
      <div className="flex gap-3 items-start">
        <div className="w-24 h-24 rounded-lg overflow-hidden bg-pink-50 border border-pink-200 shrink-0">
          {preview ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={preview} alt="" className="w-full h-full object-cover" />
          ) : (
            <div className="w-full h-full flex items-center justify-center text-[10px] text-brand-purple-dark/80">no image</div>
          )}
        </div>
        <div className="flex-1 space-y-2">
          <input
            ...
            className="w-full bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-xs text-brand-purple-dark font-mono focus:outline-none focus:border-brand-orange/40"
          />
          <div className="flex flex-wrap gap-2">
            <button
            ...
            <label className={`text-[11px] px-3 py-1.5 rounded-md border cursor-pointer transition-colors ${
              uploading
                ? "border-pink-200 text-brand-purple-dark/80"
            ...
            <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
              {media.map(item => (
                <button
                  key={item.id}
                  onClick={() => pick(item)}
                  className="relative aspect-square rounded-md overflow-hidden border border-pink-200 hover:border-brand-orange/50 transition-colors"
                  title={item.name}
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
