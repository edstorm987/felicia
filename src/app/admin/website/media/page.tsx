...
function AdminMediaPageInner() {
  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 space-y-6 max-w-7xl">
      {/* Breadcrumb */}
      <Link href="/admin/website" className="text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark transition-colors">
        ← Website
      </Link>
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Images</h1>
      ...
      {/* Tip */}
      <div className="rounded-xl border border-brand-amber/20 bg-brand-amber/5 px-4 py-3 text-xs text-brand-purple-dark/80 leading-relaxed">
      ...
      {/* Grid */}
      {filtered.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-pink-200 bg-white/50 px-8 py-16 text-center">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {filtered.map(item => (
            <div key={item.id} className="rounded-2xl border border-pink-200/50 bg-white overflow-hidden group">
              ...
              <div className="p-3">
                <p className="text-xs text-brand-purple-dark truncate" title={item.name}>{item.name}</p>
                <p className="text-[10px] text-brand-purple-dark/80 mt-0.5">{formatBytes(item.size)}</p>
                <div className="flex gap-1.5 mt-2">
                  ...
                  <button
                    ...
                    className="px-2 py-1.5 rounded-md text-[10px] font-medium border border-pink-200 text-brand-purple-dark/80 hover:text-brand-orange hover:border-brand-orange/30 transition-colors"
                  >
                    Delete
                  </button>
                </div>
