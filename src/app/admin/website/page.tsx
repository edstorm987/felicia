    ...
    <div className="p-6 sm:p-8 lg:p-10 space-y-8 max-w-7xl">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-brand-purple-dark">Sitemap &amp; content</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        >
          <div>
            ...
            <p className="text-sm font-medium text-brand-purple-dark">Global SEO, analytics & cookies →</p>
          </div>
        </Link>
        <Link
        ...
        >
          <div>
            ...
            <p className="text-sm font-medium text-brand-purple-dark">Media library →</p>
          </div>
        </Link>
        <Link
          href="/admin/pages"
        ...
        >
          <div>
            ...
            <p className="text-sm font-medium text-brand-purple-dark">Custom pages →</p>
          </div>
        </Link>
      </div>
              ...
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className="text-sm font-medium text-brand-purple-dark truncate">{schema.label}</span>
                    {drafts > 0 && (
                  ...
                  <p className="text-xs text-brand-purple-dark/80">
                    {schema.description ?? "—"}
                    <span className="text-brand-purple-dark/80"> · {fullSchema.sections.length} section{fullSchema.sections.length === 1 ? "" : "s"} · {totalFields} fields</span>
                  </p>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  <a
                    ...
                    className="text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark underline underline-offset-4"
                  >
                    View
                  </a>
          ...
          <div className="rounded-2xl border border-pink-200/50 bg-white overflow-hidden divide-y divide-white/5">
            {pages.map(p => (
              ...
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className="text-sm font-medium text-brand-purple-dark truncate">{p.title}</span>
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${p.status === "published" ? "bg-green-400/20 text-green-300" : "bg-white/10 text-brand-purple-dark/80"}`}>
                      {p.status}
                    </span>
                  </div>
                  <p className="text-xs text-brand-purple-dark/80">/p/{p.slug} · {p.blocks.length} block{p.blocks.length === 1 ? "" : "s"}</p>
                </div>
                <span className="text-brand-purple-dark/80">→</span>
              </Link>
            ))}
