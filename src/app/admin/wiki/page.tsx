    ...
    <main className="max-w-4xl mx-auto px-6 py-8 space-y-6">
      ...
      <header className="flex items-center justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark">Documentation pages</h1>
          <p className="text-[12px] text-brand-purple-dark/80 mt-1">{pages.length} pages</p>
        </div>
        <button
          type="button"
      ...
      {editor && (
        <section className="rounded-xl border border-cyan-400/20 bg-white/[0.02] p-4 space-y-3">
          <input
            ...
            className="w-full bg-white/5 border border-pink-200 rounded-md px-3 py-2 text-[14px] text-brand-purple-dark"
          />
          <textarea
            value={editor.body}
            ...
            rows={12}
            className="w-full bg-white/5 border border-pink-200 rounded-md px-3 py-2 text-[12px] font-mono text-brand-purple-dark"
          />
          <div className="flex items-center justify-end gap-2">
            <button onClick={() => setEditor(null)} className="text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark">
              Cancel
            </button>
            <button
              onClick={save}
      ...
      {pages.length === 0 ? (
        <div className="rounded-xl border border-pink-200/50 bg-white/[0.02] p-6 text-center">
          <p className="text-[13px] text-brand-purple-dark/80">No wiki pages yet.</p>
        ...
        <ul className="space-y-1">
          {pages.map(p => (
            <li key={p.id} className="rounded-lg border border-pink-200/50 bg-white/[0.02] px-4 py-2 flex items-center justify-between text-[12px]">
              <button
                onClick={() => setEditor({ slug: p.slug, title: p.title, body: "" })}
                className="text-left text-brand-purple-dark hover:text-cyan-200"
              >
                <span className="font-medium">{p.title}</span>
                <span className="text-brand-purple-dark/80 ml-2 font-mono text-[10px]">/{p.slug}</span>
              </button>
