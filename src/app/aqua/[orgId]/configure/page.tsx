...
const INPUT = "w-full bg-white/5 border border-pink-200 rounded-lg px-2.5 py-1.5 text-[13px] text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-brand-orange/50";
...
export default function ConfigurePortalPage() {
  ...
  if (!org && !error) return <main className="max-w-3xl mx-auto px-6 py-10 text-[12px] text-brand-purple-dark/80">Loading…</main>;
    ...
    <main className="max-w-3xl mx-auto px-6 py-10 space-y-6">
      <header className="flex items-baseline justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl text-brand-purple-dark mt-3">{org.name}</h1>
          <p className="text-[12px] text-brand-purple-dark/80 mt-1 font-mono">{org.slug}</p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          <Field label="Brand colour">
            <div className="flex gap-2">
              <input type="color" value={brandColor} onChange={e => { setBrandColor(e.target.value); save({ brandColor: e.target.value }); }} className="w-12 h-10 rounded-lg cursor-pointer bg-transparent border border-pink-200" />
      ...
      <section className="rounded-2xl border border-pink-200 bg-white/[0.02] p-4 space-y-3">
        <p className="text-[10px] tracking-[0.18em] uppercase text-brand-purple-dark/80">Database</p>
        ...
        <p className="text-[10px] text-brand-purple-dark/80 leading-relaxed">
          The agency-wide bootstrap backend (set via <code className="font-mono text-brand-purple-dark/80">PORTAL_BACKEND</code>) stores the org records themselves. Per-tenant routing reads these credentials at request time to read/write that org&apos;s state from the configured backend. Switching backends never migrates data; export from the old DB first.
        </p>
      </section>

      <section className="rounded-2xl border border-pink-200 bg-white/[0.02] p-4">
        <p className="text-[10px] tracking-[0.18em] uppercase text-brand-purple-dark/80 mb-3">Sites in this portal</p>
        {sites.length === 0 ? (
          <p className="text-[12px] text-brand-purple-dark/80">No sites yet — create one inside the portal admin.</p>
        ) : (
          <ul className="space-y-1.5">
            {sites.map(s => (
              <li key={s.id} className="flex items-center gap-3 text-[12px] text-brand-purple-dark/80">
                <span className="font-medium">{s.name}</span>
                <span className="text-brand-purple-dark/80 font-mono">{s.primaryDomain || s.domains?.[0] || "—"}</span>
      ...
      {savedAt && <p className="text-[11px] text-brand-purple-dark/80">Saved</p>}
    </main>
  );
}
