  ...
  return (
    <main className="max-w-5xl mx-auto px-6 py-10 space-y-8">
      <header>
        ...
        <h1 className="font-display text-4xl text-brand-purple-dark">Example portal</h1>
        <p className="text-[13px] text-brand-purple-dark/80 max-w-2xl mt-1 leading-relaxed">
      ...
      <section className="rounded-2xl border border-cyan-500/30 bg-gradient-to-br from-cyan-500/10 via-blue-500/5 to-indigo-500/10 p-6">
        <div className="flex items-start gap-5">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-display text-2xl text-brand-purple-dark mb-1">Example Co</p>
            <p className="text-[12px] text-brand-purple-dark/80 mb-3">demo@example.com · {data?.pages.length ?? 0} pages seeded</p>
      ...
      {/* Pages preview */}
      <section>
        <p className="text-[11px] tracking-[0.18em] uppercase text-brand-purple-dark/80 mb-3">Pre-built pages</p>
        {data && data.pages.length > 0 ? (
          <div className="grid sm:grid-cols-2 gap-3">
            {data.pages.map(page => (
              <div key={page.id} className="rounded-xl border border-pink-200 bg-white/[0.02] p-4 flex items-center gap-4">
                <div className="w-12 h-12 rounded-lg bg-white/5 border border-pink-200 flex items-center justify-center text-lg shrink-0">
                  {page.slug === "/" ? "🏠" : page.slug === "/shop" ? "🛍" : page.slug === "/cart" ? "🛒" : page.slug === "/checkout" ? "💳" : "📄"}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-brand-purple-dark">{page.title}</p>
                  <p className="text-[11px] text-brand-purple-dark/80 font-mono truncate">{page.slug}</p>
                  <p className="text-[10px] text-brand-purple-dark/80 mt-0.5">{(page.blocks?.length ?? 0)} top-level block{page.blocks?.length === 1 ? "" : "s"}</p>
                </div>
                <button
          ...
          <p className="text-[12px] text-brand-purple-dark/80">No pages yet. Click <em>Seed example data</em> above to create them.</p>
        )}
      </section>

      {/* Walkthrough steps */}
      <section>
        <p className="text-[11px] tracking-[0.18em] uppercase text-brand-purple-dark/80 mb-3">Walkthrough</p>
        <ol className="space-y-2">
          {STEPS.map(s => (
            <li key={s.id} className="flex gap-3 items-start p-3 rounded-xl border border-pink-200/50 bg-white/[0.02]">
              ...
              <div>
                <p className="text-sm font-semibold text-brand-purple-dark">{s.title}</p>
