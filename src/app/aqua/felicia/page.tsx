...
export default function FeliciaPortalPage() {
  ...
  if (loading) {
    return <main className="max-w-5xl mx-auto px-6 py-12 text-[12px] text-brand-purple-dark/80">Loading Felicia&apos;s portal…</main>;
  }
  ...
  if (!org) {
    return (
      <main className="max-w-2xl mx-auto px-6 py-16 text-center space-y-4">
        ...
        <h1 className="font-display text-2xl text-brand-purple-dark">Org not seeded yet.</h1>
        <p className="text-[13px] text-brand-purple-dark/80 max-w-md mx-auto">
          Create an org with id <code className="font-mono text-brand-purple-dark/80">felicia</code>
    ...
    <main className="max-w-5xl mx-auto px-6 py-10 space-y-8">
      <header className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-display text-3xl text-brand-purple-dark">{org.name}</h1>
          </div>
          <p className="text-[12px] text-brand-purple-dark/80 mt-2 max-w-xl">
        ...
        <div className="flex flex-col gap-2">
          ...
          <Link
            ...
            className="px-3 py-2 rounded-lg border border-pink-200 text-brand-purple-dark/80 text-[12px] text-center hover:bg-white/5"
          >
            Configure
          </Link>
        </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-[10px] tracking-[0.28em] uppercase text-brand-purple-dark/80">Installed plugins</h2>
          <span className="text-[11px] text-brand-purple-dark/80 tabular-nums">{installedMeta.length} active</span>
        </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
            {installedMeta.map(({ install, meta }) => (
              <Link
                ...
                className="rounded-xl border border-pink-200/50 bg-white/[0.02] p-3 hover:bg-white/[0.04] transition-colors"
              >
                <p className="text-[12px] text-brand-purple-dark font-medium">{meta.name}</p>
                <p className="text-[11px] text-brand-purple-dark/80 line-clamp-2 mt-1">{meta.tagline}</p>
              </Link>
            ))}
          </div>
        )}
      ...
      <section className="rounded-xl border border-pink-200/50 bg-white/[0.02] p-4">
        <h2 className="text-[10px] tracking-[0.28em] uppercase text-brand-purple-dark/80 mb-3">Embedding in the storefront</h2>
...
export default function AdminGate() {
  return (
    ...
    />
      ...
      <section className="rounded-xl border border-pink-200/50 bg-white/[0.02] p-4">
        <h2 className="text-[10px] tracking-[0.28em] uppercase text-brand-purple-dark/80 mb-3">Extraction status</h2>
        <p className="text-[12px] text-brand-purple-dark/80 leading-relaxed">
          ...
          Run <code className="font-mono text-brand-purple-dark/80">bash scripts/extract.sh</code> to split into
