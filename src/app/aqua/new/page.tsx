...
export default function NewPortalPage() {
  ...
  return (
    <main className="max-w-3xl mx-auto px-6 py-10 space-y-6">
      <header>
        <Link href="/aqua" className="text-[12px] text-brand-purple-dark/80 hover:text-brand-purple-dark">← Back to portals</Link>
        <p className="text-[10px] tracking-[0.32em] uppercase text-cyan-400 mt-3 mb-1">Onboard</p>
        <h1 className="font-display text-3xl text-brand-purple-dark">New client portal</h1>
        <p className="text-[13px] text-brand-purple-dark/80 mt-1">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <section>
          <p className="text-[10px] tracking-[0.18em] uppercase text-brand-purple-dark/80 mb-2">Pick a preset</p>
          {presetsLoading ? (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="rounded-xl border border-pink-200 bg-white/[0.02] p-3 animate-pulse">
            ...
            <p className="text-[12px] text-brand-purple-dark/80">No presets available. The portal will be created empty.</p>
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {presets.map(p => {
                ...
                return (
                  <button
                    ...
                    aria-pressed={active}
                    className={`text-left p-3 rounded-xl border transition-colors ${active ? "border-cyan-400/60 bg-cyan-500/10" : "border-pink-200 bg-white/[0.02] hover:bg-white/[0.05]"}`}
                  >
                    <p className="text-base mb-1" style={{ color: hint.color }}>{hint.icon}</p>
                    <p className="text-[12px] font-semibold text-brand-purple-dark">{p.name}</p>
                    <p className="text-[10px] text-brand-purple-dark/80 leading-relaxed mt-0.5">{p.tagline}</p>
                    <p className="text-[10px] text-brand-purple-dark/80 mt-1">
          ...
          {activePreset && (
            <p className="text-[11px] text-brand-purple-dark/80 mt-3 leading-relaxed">{activePreset.description}</p>
          )}
        </section>

        <section className="rounded-2xl border border-pink-200 bg-white/[0.02] p-4 space-y-3">
          <div className="flex items-start gap-4">
            {/* Live brand chip — preview the new portal card before it exists */}
            <div
              className="w-14 h-14 rounded-xl shrink-0 flex items-center justify-center text-white text-lg font-bold border border-pink-200"
            ...
            <div className="flex-1">
              <p className="text-[10px] tracking-[0.18em] uppercase text-brand-purple-dark/80">Identity</p>
              <p className="text-[11px] text-brand-purple-dark/80 mt-1">Live preview of how this portal will appear in the agency dashboard.</p>
            </div>
          </div>
        ...
        <section className="rounded-2xl border border-pink-200 bg-white/[0.02] p-4 space-y-3">
          <p className="text-[10px] tracking-[0.18em] uppercase text-brand-purple-dark/80">Branding</p>
          <div className="grid sm:grid-cols-2 gap-3">
            <Field label="Brand colour">
              <div className="flex gap-2">
                <input type="color" value={brandColor} onChange={e => setBrandColor(e.target.value)} className="w-12 h-10 rounded-lg cursor-pointer bg-transparent border border-pink-200" aria-label="Pick brand colour" />
        ...
        <div className="flex items-center gap-3 pt-1">
          ...
          <Link href="/aqua" className="text-[12px] text-brand-purple-dark/80 hover:text-brand-purple-dark">Cancel</Link>
        </div>
      </form>
    </main>
  );
...
const INPUT = "w-full bg-white/5 border border-pink-200 rounded-lg px-2.5 py-1.5 text-[13px] text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-cyan-400/50";

function Field({ label, required, hint, children }: { label: string; required?: boolean; hint?: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="flex items-baseline gap-2 text-[10px] tracking-[0.18em] uppercase text-brand-purple-dark/80 mb-1">
        <span>{label}{required && <span className="text-cyan-400 ml-1">*</span>}</span>
        {hint && <span className="ml-auto normal-case tracking-normal text-[10px] text-brand-purple-dark/80 font-mono">{hint}</span>}
      </span>
      {children}
    </label>
  );
