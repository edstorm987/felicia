  ...
  if (!hydrated) {
    return (
      <main className="max-w-6xl mx-auto px-6 py-12">
        <p className="text-[12px] text-brand-purple-dark/80">Loading portals…</p>
      </main>
    );
  }
  ...
  return (
    <main className="max-w-6xl mx-auto px-6 py-10 space-y-8">
      <header>
        ...
        <h1 className="font-display text-4xl text-brand-purple-dark mb-2">Your client portals</h1>
        <p className="text-[13px] text-brand-purple-dark/80 max-w-2xl leading-relaxed">
      ...
      {orgs.filter(o => !o.isPrimary).length === 0 && (
        <section className="rounded-2xl border border-cyan-500/30 bg-gradient-to-br from-cyan-500/10 via-blue-500/5 to-indigo-500/10 p-6">
          ...
          <h2 className="font-display text-2xl text-brand-purple-dark mb-2">Try the example portal</h2>
          <p className="text-[12px] text-brand-purple-dark/80 max-w-2xl leading-relaxed mb-4">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {orgs.map(org => {
          ...
          return (
            <div key={org.id} className="rounded-2xl border border-pink-200 bg-white/[0.02] hover:bg-white/[0.04] transition-colors p-5 flex flex-col gap-3">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-display text-lg font-bold text-brand-purple-dark truncate">
                    {org.name}
                  ...
                  <p className="text-[11px] text-brand-purple-dark/80 font-mono truncate">{org.slug}</p>
                  {org.ownerEmail && <p className="text-[10px] text-brand-purple-dark/80 truncate mt-0.5">{org.ownerEmail}</p>}
                </div>
              </div>

              <div className="flex items-center gap-3 text-[11px] text-brand-purple-dark/80">
              ...
              <div className="mt-auto flex flex-col gap-1.5 pt-2 border-t border-pink-200/50">
                <button
        ...
        <Link
          href="/aqua/new"
          className="rounded-2xl border-2 border-dashed border-pink-200 hover:border-brand-orange/40 hover:bg-brand-orange/5 transition-colors p-5 flex flex-col items-center justify-center gap-2 min-h-[220px] text-center"
        >
          <span className="text-3xl">＋</span>
          <span className="font-display text-base text-brand-purple-dark">New portal</span>
          <span className="text-[11px] text-brand-purple-dark/80 max-w-[200px] leading-relaxed">Onboard a new client. Creates an org + first site.</span>
        </Link>
      </div>
    </main>
  );
