...
export default function BillingPage() {
  ...
  return (
    <main className="max-w-4xl mx-auto px-6 py-10 space-y-6">
      <header>
        <Link href="/aqua/support" className="text-[12px] text-brand-purple-dark/80 hover:text-brand-purple-dark">← Aqua support</Link>
        <h1 className="font-display text-3xl text-brand-purple-dark mt-2">Plan + invoices</h1>
        <p className="text-[12px] text-brand-purple-dark/80 mt-1">{orgName}</p>
      </header>

      {/* Current subscription */}
      <section className="rounded-2xl border border-pink-200 bg-white/[0.02] p-5">
        <p className="text-[11px] uppercase tracking-[0.2em] text-brand-purple-dark/80 mb-3">Current plan</p>
        {subscription ? (
          <div className="flex items-center gap-3">
            <p className="font-display text-xl text-brand-purple-dark">{plans.find(p => p.id === subscription.planId)?.name ?? subscription.planId}</p>
            ...
            <button onClick={handleCancelSub} disabled={!!busy || subscription.status === "canceled"} className="ml-auto text-[11px] text-brand-purple-dark/80 hover:text-red-400 disabled:opacity-40">
              {busy === "cancel" ? "Cancelling…" : "Cancel"}
            </button>
          </div>
        ) : (
          <p className="text-[12px] text-brand-purple-dark/80">On the free Starter tier.</p>
        )}
      </section>

      {/* Plan switcher */}
      <section>
        <p className="text-[11px] uppercase tracking-[0.2em] text-brand-purple-dark/80 mb-3">Switch plan</p>
        <div className="grid sm:grid-cols-3 gap-3">
          {plans.map(plan => {
            const active = plan.id === activePlanId;
            return (
              <div key={plan.id} className={`rounded-2xl border p-4 ${active ? "border-brand-orange/60 bg-brand-orange/10" : "border-pink-200 bg-white/[0.02]"}`}>
                <p className="font-display text-xl text-brand-purple-dark">{plan.name}</p>
                <p className="text-[12px] text-brand-purple-dark/80 mb-3">
                  {plan.priceMonthly === 0 ? "Free" : `£${(plan.priceMonthly / 100).toFixed(2)}/mo`}
                </p>
                <ul className="text-[11px] text-brand-purple-dark/80 space-y-0.5 mb-3">
                  {plan.features.slice(0, 6).map(f => <li key={f}>· {f}</li>)}
                </ul>
                <button
                  onClick={() => handlePlan(plan.id)}
                  disabled={!!busy || active}
                  className={`w-full px-3 py-2 rounded-xl text-sm font-semibold disabled:opacity-40 ${active ? "bg-white/10 text-brand-purple-dark/80 cursor-default" : "bg-brand-orange text-white hover:opacity-90"}`}
                >
      ...
      <section className="rounded-2xl border border-pink-200 bg-white/[0.02] p-5">
        <p className="text-[11px] uppercase tracking-[0.2em] text-brand-purple-dark/80 mb-3">Active features</p>
      ...
      <section className="rounded-2xl border border-pink-200 bg-white/[0.02]">
        <div className="px-4 py-3 border-b border-pink-200/50 flex items-center justify-between">
          ...
          <details className="relative">
            ...
            <div className="absolute right-0 top-full mt-2 w-72 rounded-xl border border-pink-200 bg-pink-50/50 p-3 z-10 space-y-2">
              ...
              <p className="text-[10px] text-brand-purple-dark/80">Stripe webhook will replace this when wired.</p>
            </div>
          </details>
        </div>
        {invoices.length === 0 ? (
          <p className="px-4 py-6 text-[12px] text-brand-purple-dark/80 text-center">No invoices yet.</p>
        ) : (
          <table className="w-full text-[12px]">
            <thead>
              <tr className="text-left text-brand-purple-dark/80 border-b border-pink-200/50">
            ...
            <tbody>
              {invoices.map(inv => (
                <tr key={inv.id} className="border-b border-pink-200/50 hover:bg-white/[0.02]">
                  <td className="px-3 py-2 font-mono text-brand-purple-dark/80">{inv.number}</td>
                  <td className="px-3 py-2 text-brand-purple-dark/80">{new Date(inv.date).toLocaleDateString()}</td>
                  <td className="px-3 py-2 text-brand-purple-dark/80">{inv.description ?? inv.lines[0]?.description ?? "—"}</td>
                  <td className="px-3 py-2">
                    <span className={`text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded-full ${
                      ...
                      : "bg-white/5 text-brand-purple-dark/80"
                    }`}>{inv.status}</span>
                  </td>
                  <td className="px-3 py-2 text-right font-mono text-brand-purple-dark font-bold tabular-nums">{formatMoney(inv.amountTotal, inv.currency)}</td>
