  ...
  return (
    <main className="max-w-3xl mx-auto px-6 py-10 space-y-6">
      <header>
        ...
        <h1 className="font-display text-3xl text-brand-purple-dark mt-2">Book a meeting</h1>
        <p className="text-[12px] text-brand-purple-dark/80 mt-1 leading-relaxed">
          Suggest 2–3 times that work for you. We&apos;ll confirm one + send the call link.
        </p>
      </header>

      <form onSubmit={handleBook} className="rounded-2xl border border-pink-200 bg-white/[0.02] p-4 space-y-3">
        <Field label="Topic">
      ...
      <section className="space-y-2">
        <p className="text-[11px] uppercase tracking-[0.18em] text-brand-purple-dark/80">Your meetings ({items.length})</p>
        {items.length === 0 && <p className="text-[12px] text-brand-purple-dark/80">No meetings booked yet.</p>}
        {items.map(m => (
          <article key={m.id} className="rounded-xl border border-pink-200 bg-white/[0.02] p-4">
            <div className="flex items-baseline gap-2 mb-1">
              <p className="text-sm font-semibold text-brand-purple-dark">{m.topic}</p>
              <span className={`text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded-full ${STATUS_TONE[m.status]}`}>{m.status}</span>
            </div>
            <p className="text-[11px] text-brand-purple-dark/80 mb-2">Suggested: {m.preferredDates.map(d => new Date(d).toLocaleString()).join(" · ")}</p>
            {m.notes && <p className="text-[12px] text-brand-purple-dark/80 leading-relaxed">{m.notes}</p>}
            ...
            <div className="flex items-center gap-2 mt-3 pt-2 border-t border-pink-200/50 flex-wrap">
              <span className="text-[10px] text-brand-purple-dark/80">Agency:</span>
              {(["requested", "confirmed", "completed", "cancelled"] as const).map(s => (
                <button
                  key={s}
                  onClick={async () => { await patchMeeting(m.id, { status: s }); void refresh(); }}
                  className={`text-[10px] px-2 py-0.5 rounded-full ${m.status === s ? STATUS_TONE[s] : "text-brand-purple-dark/80 hover:text-brand-purple-dark"}`}
                >
                  {s}
                </button>
              ))}
              <input
                ...
                className="ml-auto bg-white/5 border border-pink-200 rounded px-2 py-1 text-[10px] text-brand-purple-dark w-56 focus:outline-none focus:border-brand-orange/50"
              />
            </div>
          </article>
        ))}
      ...
      {toast && (
        <div className="pointer-events-none fixed bottom-6 left-1/2 -translate-x-1/2 px-4 py-2 rounded-full bg-brand-cream text-brand-purple-dark text-[12px] font-medium shadow-lg">
          {toast}
        </div>
      )}
    </main>
