  ...
  return (
    <main className="max-w-5xl mx-auto px-6 py-10 space-y-8">
      <header>
        ...
        <h1 className="font-display text-4xl text-brand-purple-dark">Aqua support</h1>
      ...
      {orgs.length > 1 && (
        <div className="flex items-center gap-2 text-[12px]">
          <span className="text-brand-purple-dark/80">Viewing:</span>
          <select
            value={activeOrg?.id ?? ""}
            onChange={async e => {
...
function SupportCard({ icon, title, body, href, cta }: { icon: string; title: string; body: string; href: string; cta: string }) {
  return (
    <Link
      href={href}
      className="rounded-2xl border border-pink-200 bg-white/[0.02] hover:bg-white/[0.04] transition-colors p-5 flex flex-col gap-3 group"
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <p className="font-display text-xl text-brand-purple-dark">{title}</p>
          <p className="text-[12px] text-brand-purple-dark/80 leading-relaxed mt-1">{body}</p>
        </div>
      </div>
