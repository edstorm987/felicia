  ...
  return (
    <main className="max-w-4xl mx-auto px-6 py-10 space-y-8">
      <header>
        ...
        <h1 className="font-display text-3xl text-brand-purple-dark mt-2">Resources + guides</h1>
        <p className="text-[12px] text-brand-purple-dark/80 mt-1">Quick links to every part of the portal.</p>
      </header>

      {RESOURCES.map(group => (
        <section key={group.category}>
          <p className="text-[11px] tracking-[0.18em] uppercase text-brand-purple-dark/80 mb-2">{group.category}</p>
          <div className="grid sm:grid-cols-2 gap-2">
            {group.items.map(item => (
              <Link
                key={item.title}
                href={item.href}
                className="rounded-xl border border-pink-200 bg-white/[0.02] hover:bg-white/[0.04] p-4 flex items-start gap-3 transition-colors group"
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-brand-purple-dark group-hover:text-brand-orange transition-colors">{item.title}</p>
                  <p className="text-[11px] text-brand-purple-dark/80 leading-relaxed mt-0.5">{item.body}</p>
                </div>
              </Link>
            ))}
          </div>
      ...
      <section className="rounded-2xl border border-cyan-500/30 bg-gradient-to-br from-cyan-500/5 via-blue-500/5 to-indigo-500/5 p-5">
        ...
        <p className="font-display text-xl text-brand-purple-dark mb-2">Talk to the team</p>
        <p className="text-[12px] text-brand-purple-dark/80 max-w-xl mb-3 leading-relaxed">
