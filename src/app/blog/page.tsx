...
const CATEGORY_COLOURS: Record<string, string> = {
  ...
  "Journal":        "text-brand-purple-dark/80 bg-white/5",
};
      ...
      <main className="w-full min-h-screen bg-pink-50">
        <section className="w-full pt-28 pb-16 sm:pt-32 sm:pb-20 bg-pink-50/50 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-12 xl:px-16 relative">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-brand-purple-dark/80 text-base sm:text-lg leading-relaxed max-w-xl">
                {intro}
              </p>
            </div>
          </div>
        ...
        <section className="w-full py-12 sm:py-16">
          <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-12 xl:px-16 space-y-12">
              ...
              >
                ...
                <div className="p-8 sm:p-10 lg:p-12">
                  <div className="flex items-center gap-3 mb-6">
                    <span className={`text-[10px] tracking-[0.2em] uppercase px-3 py-1 rounded-full font-medium ${CATEGORY_COLOURS[featured.category] ?? "text-brand-purple-dark/80 bg-white/5"}`}>
                      {featured.category}
                    </span>
                    <span className="text-brand-orange text-xs tracking-widest uppercase font-medium">Featured</span>
                  </div>
                  <h2 className="font-display font-bold text-brand-purple-dark text-2xl sm:text-3xl lg:text-4xl leading-snug mb-4 max-w-3xl group-hover:text-brand-orange transition-colors duration-200">
                    {featured.title}
                  </h2>
                  <p className="text-brand-purple-dark/80 text-base leading-relaxed max-w-2xl mb-6">
                    {featured.excerpt}
                  </p>
                  ...
                  <div className="flex items-center gap-4 text-xs text-brand-purple-dark/80 tracking-wide">
                ...
                {rest.map(post => (
                  ...
                  >
                    ...
                    <div className="flex flex-col flex-1 p-6 gap-4">
                      <span className={`text-[10px] tracking-[0.2em] uppercase px-3 py-1 rounded-full font-medium w-fit ${CATEGORY_COLOURS[post.category] ?? "text-brand-purple-dark/80 bg-white/5"}`}>
                        {post.category}
                      </span>
                      <h3 className="font-display font-bold text-brand-purple-dark text-lg leading-snug group-hover:text-brand-orange transition-colors duration-200 flex-1">
                        {post.title}
                      </h3>
                      ...
                      <div className="flex items-center gap-3 text-xs text-brand-purple-dark/80 tracking-wide pt-2 border-t border-pink-200/50">
            ...
            <div className="flex flex-col items-center text-center py-10 border-t border-pink-200/50 gap-2">
              <p className="text-brand-purple-dark/80 text-sm">More stories coming soon.</p>
              <p className="text-brand-purple-dark/80 text-xs">Follow us on Instagram for the latest from Accra.</p>
              <a
                href="/blog/rss.xml"
