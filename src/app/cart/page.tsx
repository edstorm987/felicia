...
export default function CartPage() {
  ...
  return (
    <>
      <Navbar />
      <main className="w-full min-h-screen bg-pink-50 pt-28 pb-16 sm:pt-32">
          ...
          {items.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <aside className="rounded-2xl border border-pink-200 bg-white p-5 h-fit space-y-4">
                ...
                {discounts.map((d) => (
                  <div key={d.code} className="flex items-center justify-between gap-3 p-3 rounded-lg bg-brand-amber/10 border border-brand-amber/25">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      <button onClick={() => removeDiscount(d.code)} className="text-xs text-brand-purple-dark/80 hover:text-brand-purple-dark underline">Remove</button>
                    </div>
                  </div>
                ))}
                ...
                <form onSubmit={handleApplyDiscount} className="space-y-2">
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 min-w-0 bg-pink-50 border border-pink-200 rounded-lg px-3 py-2.5 text-xs text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-brand-amber/40 transition-colors"
                    />
                ...
                <div className="flex justify-between text-brand-purple-dark/80 text-sm"><span>Subtotal</span><span>£{subtotal.toFixed(2)}</span></div>
                {discounts.map((d) => (
                ...
                <div className="pt-3 border-t border-white/8">
                  ...
                  <select
                    ...
                    className="w-full bg-pink-50 border border-pink-200 rounded-lg px-3 py-2.5 text-xs text-brand-purple-dark focus:outline-none focus:border-brand-amber/40"
                  >
                  ...
                  {(source === "other" || source === "press") && (
                    <input
                      ...
                      className="mt-2 w-full bg-pink-50 border border-pink-200 rounded-lg px-3 py-2 text-xs text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-brand-amber/40"
                    />
                  )}
                </div>
