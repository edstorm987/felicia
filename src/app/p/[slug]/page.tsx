...
export default function CustomPageView() {
  ...
  if (page === undefined) {
    return (
      <>
        <Navbar />
        <main className="w-full pt-32 pb-20 min-h-screen bg-pink-50" />
        <Footer />
      </>
    );
  }
      ...
      <>
        ...
        <main className="w-full pt-32 pb-20 min-h-screen bg-pink-50">
          <div className="max-w-2xl mx-auto px-6 text-center">
            ...
            <h1 className="font-display text-3xl text-brand-purple-dark mb-3">Page not found</h1>
  ...
  return (
    <>
      ...
      <Navbar />
      <main className="w-full pt-20 sm:pt-24 bg-pink-50">
...
function BlockView({ block }: { block: Block }) {
  switch (block.type) {
    case "hero": return (
      <section className="w-full bg-pink-50/50 border-b border-pink-200/50">
        <div className="max-w-4xl mx-auto px-6 sm:px-10 lg:px-12 py-14 sm:py-20">
          ...
          <h1 className="font-display font-bold text-brand-purple-dark text-4xl sm:text-5xl xl:text-6xl leading-tight mb-5">{block.title}</h1>
          {block.intro && <p className="text-brand-purple-dark/80 text-base sm:text-lg leading-relaxed max-w-2xl">{block.intro}</p>}
          {block.image && (
            /* eslint-disable-next-line @next/next/no-img-element */
            <img src={resolveMediaRef(block.image)} alt="" className="mt-8 w-full rounded-2xl border border-pink-200/50 max-h-[28rem] object-cover" />
          )}
        </div>
      </section>
    );
    case "richText": return (
      <section className="w-full">
        <div className="max-w-2xl mx-auto px-6 sm:px-10 lg:px-12 py-10 sm:py-14 lk-rich text-brand-purple-dark/80 text-base sm:text-lg leading-relaxed" dangerouslySetInnerHTML={{ __html: block.html }} />
      </section>
    );
    case "image": return (
      <section className="w-full">
        <figure className="max-w-4xl mx-auto px-6 sm:px-10 lg:px-12 py-8">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={resolveMediaRef(block.src)} alt={block.alt ?? ""} className="w-full rounded-2xl border border-pink-200/50" />
          {block.caption && <figcaption className="text-xs text-brand-purple-dark/80 mt-3 text-center italic">{block.caption}</figcaption>}
        </figure>
      </section>
    );
    case "gallery": return (
      <section className="w-full">
        <div className="max-w-5xl mx-auto px-6 sm:px-10 lg:px-12 py-10 grid grid-cols-2 sm:grid-cols-3 gap-3">
          {block.images.map((img, i) => (
            /* eslint-disable-next-line @next/next/no-img-element */
            <img key={i} src={resolveMediaRef(img.src)} alt={img.alt ?? ""} className="w-full aspect-square object-cover rounded-xl border border-pink-200/50" />
          ))}
        </div>
      </section>
    );
    case "quote": return (
      <section className="w-full">
        <div className="max-w-2xl mx-auto px-6 sm:px-10 lg:px-12 py-12">
          <blockquote className="border-l-4 border-brand-orange pl-6 text-xl sm:text-2xl font-display italic text-brand-purple-dark/80 leading-relaxed">
            “{block.quote}”
          </blockquote>
          {block.attribution && <p className="mt-4 text-xs tracking-widest uppercase text-brand-purple-dark/80 pl-6">— {block.attribution}</p>}
        </div>
      </section>
    );
    case "embed": {
      ...
      return (
        <section className="w-full">
          <div className="max-w-3xl mx-auto px-6 sm:px-10 lg:px-12 py-8">
            ...
            {block.caption && <p className="text-xs text-brand-purple-dark/80 mt-3 text-center italic">{block.caption}</p>}
          </div>
        </section>
      );
    }
    case "divider": return (
      <div className="max-w-2xl mx-auto px-6 sm:px-10 lg:px-12">
        <hr className="border-0 border-t border-pink-200 my-8" />
      </div>
    );
    case "cta": return (
      <section className="w-full bg-pink-50/50 border-y border-pink-200/50">
        <div className="max-w-3xl mx-auto px-6 sm:px-10 lg:px-12 py-14 text-center">
          <h2 className="font-display font-bold text-brand-purple-dark text-3xl sm:text-4xl mb-3">{block.headline}</h2>
          {block.subhead && <p className="text-brand-purple-dark/80 mb-6">{block.subhead}</p>}
