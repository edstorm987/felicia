  ...
  return (
    <>
      ...
      <main className="w-full min-h-screen bg-pink-50">
        ...
        {/* Hero */}
        <section className="w-full pt-28 pb-16 sm:pt-32 sm:pb-20 bg-pink-50/50 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-12 xl:px-16 relative">
            <div className="flex flex-col items-center text-center">
              ...
              <h1 className="font-display font-bold text-brand-purple-dark leading-[1.05] mb-5 text-4xl sm:text-5xl xl:text-6xl 2xl:text-7xl">
                Begin your ritual
              </h1>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full max-w-3xl">
                ...
                >
                  ...
                  <span className="font-display text-xl font-bold text-brand-purple-dark mb-1">Heritage skincare</span>
                  <span className="text-sm text-brand-purple-dark/80">Rooted in love &amp; Ghanaian tradition</span>
                ...
                <button
                  ...
                  className="group flex flex-col p-6 rounded-2xl border border-pink-200 bg-gradient-to-br from-brand-black-card to-brand-purple-muted/30 hover:border-brand-amber/30 transition-all text-left"
                >
                  <span className="text-xs tracking-[0.2em] uppercase text-brand-amber mb-2">Nkrabea · For Him</span>
                  <span className="font-display text-xl font-bold text-brand-purple-dark mb-1">Strength &amp; destiny</span>
                  <span className="text-sm text-brand-purple-dark/80">Power rituals built for men</span>
                ...
                >
                  <span className="text-xs tracking-[0.2em] uppercase text-brand-purple-dark/80 mb-2">For Everyone</span>
                  <span className="font-display text-xl font-bold text-brand-purple-dark mb-1">African Black Soap</span>
                  <span className="text-sm text-brand-purple-dark/80">World renowned. One formula.</span>
                  <span className="mt-4 text-brand-purple-dark/80 text-sm group-hover:translate-x-1 group-hover:text-brand-purple-dark transition-all inline-block">Shop Black Soap →</span>
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-12 xl:px-16">
              ...
              <div className="flex gap-1 p-1 bg-white border border-white/8 rounded-2xl w-full min-w-max sm:min-w-0">
                <button
                  ...
                  className={`flex flex-col flex-1 items-start px-4 py-3 rounded-xl transition-all duration-200 text-left min-w-0 ${
                    activeTab === "all" ? "bg-pink-50/50 border border-pink-200" : "hover:bg-white/5"
                  }`}
                >
                  <span className={`text-sm font-medium transition-colors truncate w-full ${activeTab === "all" ? "text-brand-purple-dark" : "text-brand-purple-dark/80"}`}>
                    All Products
                  </span>
                  <span className="text-[10px] tracking-wide text-brand-purple-dark/80 hidden sm:block mt-0.5 truncate w-full">
                ...
                {collections.map(col => (
                  <button
                    ...
                    className={`flex flex-col flex-1 items-start px-4 py-3 rounded-xl transition-all duration-200 text-left min-w-0 ${
                      activeTab === col.slug ? "bg-pink-50/50 border border-pink-200" : "hover:bg-white/5"
                    }`}
                  >
                    <span className={`text-sm font-medium transition-colors truncate w-full ${activeTab === col.slug ? "text-brand-purple-dark" : "text-brand-purple-dark/80"}`}>
                      {col.label}
                    </span>
                    <span className="text-[10px] tracking-wide text-brand-purple-dark/80 hidden sm:block mt-0.5 truncate w-full">
                      {col.sub}
            ...
            {activeTab === "all" && (
              <div className="flex flex-wrap items-center gap-2 mb-10">
                <span className="text-xs tracking-[0.2em] uppercase text-brand-purple-dark/80 mr-1">Filter</span>
                {([
                  { id: "all",         label: "All" },
            ...
            {activeTab === "all" && allSelector === "accessories" && (
              <RangeHeader
                name="Accessories" tagline="Complete the ritual" colour="text-brand-purple-dark"
            ...
            {activeTab === "all" && allSelector === "clothing" && (
              <div className="rounded-2xl border border-brand-amber/30 bg-gradient-to-br from-brand-amber/10 to-brand-black-card p-8 sm:p-10 mb-8">
                ...
                <h3 className="font-display font-bold text-brand-purple-dark text-3xl mb-3">Support Tees</h3>
                <p className="text-brand-purple-dark/80 max-w-2xl">
            ...
            <div className="mt-16 grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                <div key={label} className="flex flex-col items-center text-center gap-2.5 p-5 rounded-xl bg-white border border-pink-200/50">
                  <span className="text-2xl sm:text-3xl">{icon}</span>
                  <p className="text-xs sm:text-sm text-brand-purple-dark/80 leading-snug">{label}</p>
                </div>
              ))}
            </div>
            ...
            <div className="mt-10 flex flex-col sm:flex-row items-center justify-between gap-4 p-6 rounded-2xl border border-brand-purple/20 bg-brand-purple-muted/20">
              <div>
                <p className="font-display font-bold text-brand-purple-dark text-lg">Not sure which range?</p>
                <p className="text-brand-purple-dark/80 text-sm mt-1">Give the gift of choice — Luv &amp; Ker Gift Cards work across both Odo and Nkrabea.</p>
              </div>
              <button
  ...
  return (
    <div className="mb-10 pb-10 border-b border-white/8">
      <div className="flex items-baseline gap-3 mb-3">
        ...
        <span className="ml-auto text-xs text-brand-purple-dark/80">{count} products</span>
      </div>
      {description && <p className="text-brand-purple-dark/80 text-sm sm:text-base leading-relaxed max-w-3xl">{description}</p>}
    </div>
  );
}
