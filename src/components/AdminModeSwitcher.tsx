...
export default function AdminModeSwitcher() {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between gap-2 px-2 py-1.5 rounded-lg border border-pink-200 hover:border-white/25 text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark transition-colors"
      >
      ...
      {open && (
        <div className="absolute bottom-full left-0 right-0 mb-2 bg-white border border-pink-200 rounded-xl shadow-2xl overflow-hidden z-50">
          {MODES.map(m => (
            <button
              key={m.id}
              onClick={() => choose(m.id)}
              className={`w-full flex items-center gap-2.5 px-3 py-2 text-left text-xs transition-colors ${
                m.id === mode ? "bg-brand-orange/15 text-brand-purple-dark" : "text-brand-purple-dark/80 hover:bg-white/5"
              }`}
            >
              <span>{m.icon}</span>
