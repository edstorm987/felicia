...
export default function CartDrawer() {
  ...
  return (
    <>
      ...
      <div className="fixed top-0 right-0 bottom-0 z-50 w-full sm:max-w-sm md:max-w-md 2xl:max-w-lg bg-pink-50/50 flex flex-col shadow-2xl">

        {/* Header */}
        <div className="flex items-center justify-between px-5 sm:px-7 py-5 sm:py-6 border-b border-pink-200">
          <div>
            <h2 className="font-display text-lg sm:text-xl 2xl:text-2xl text-brand-purple-dark">Your Bag</h2>
            <p className="text-xs 2xl:text-sm text-brand-purple-dark/80 mt-0.5">
              {count} {count === 1 ? "item" : "items"}
            </p>
          </div>
          <button
            onClick={closeCart}
            className="w-8 h-8 flex items-center justify-center text-brand-purple-dark/80 hover:text-brand-purple-dark transition-colors"
          >
        ...
        <div className="flex-1 overflow-y-auto px-5 sm:px-7 py-5 space-y-4">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-12 sm:py-16">
              ...
              <p className="text-brand-purple-dark/80 text-sm">Your bag is empty</p>
              <p className="text-brand-purple-dark/80 text-xs mt-1">Add Odo to begin your ritual</p>
            </div>
          ) : (
            items.map((item) => (
              <div
                key={item.id}
                className="flex gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl bg-white border border-pink-200/50"
              >
                {/* Thumbnail */}
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-sm 2xl:text-base font-medium text-brand-purple-dark truncate">{item.name}</h3>
                  {item.variant && (
                    <p className="text-xs text-brand-purple-dark/80 mt-0.5">{item.variant}</p>
                  )}
                ...
                <div className="flex flex-col items-end gap-2 shrink-0">
                  <button
                    ...
                    className="text-brand-purple-dark/80 hover:text-brand-orange transition-colors"
                  >
                  ...
                  <div className="flex items-center gap-1.5 sm:gap-2">
                    <button
                      ...
                      className="w-6 h-6 rounded border border-pink-200 text-brand-purple-dark/80 hover:border-brand-orange hover:text-brand-orange transition-colors text-xs flex items-center justify-center"
                    >
                      −
                    </button>
                    <span className="text-sm text-brand-purple-dark w-4 text-center">{item.quantity}</span>
                    <button
                      onClick={() => updateQty(item.id, item.quantity + 1)}
                      className="w-6 h-6 rounded border border-pink-200 text-brand-purple-dark/80 hover:border-brand-orange hover:text-brand-orange transition-colors text-xs flex items-center justify-center"
                    >
        ...
        {/* Footer — checkout */}
        {items.length > 0 && (
          <div className="px-5 sm:px-7 py-5 sm:py-6 border-t border-pink-200 space-y-4">
            ...
            {discounts.map((d) => (
              <div key={d.code} className="flex items-center justify-between gap-3 p-3 rounded-lg bg-brand-amber/10 border border-brand-amber/25">
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-brand-purple-dark truncate">{d.code}</p>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  <span className="text-sm font-semibold text-brand-amber">−£{d.amountOff.toFixed(2)}</span>
                  <button
                    onClick={() => removeDiscount(d.code)}
                    className="text-xs text-brand-purple-dark/80 hover:text-brand-purple-dark underline"
                  >
                    Remove
                  </button>
                </div>
            ...
            <form onSubmit={handleApplyDiscount} className="space-y-2">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 min-w-0 bg-white border border-pink-200 rounded-lg px-3 py-2.5 text-xs text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-brand-amber/40 transition-colors"
                />
                <button
                  type="submit"
            ...
            <div className="flex items-center justify-between text-sm text-brand-purple-dark/80">
              <span>Subtotal</span>
            ...
            <div className="flex items-center justify-between">
              <span className="text-brand-purple-dark/80 text-sm 2xl:text-base">Total</span>
              <span className="font-display text-xl 2xl:text-2xl text-brand-purple-dark">£{total.toFixed(2)}</span>
            </div>
            <button
            ...
            <button
              onClick={closeCart}
              className="w-full py-2.5 sm:py-3 text-brand-purple-dark/80 hover:text-brand-purple-dark transition-colors text-sm 2xl:text-base"
            >
              Continue Shopping
            </button>
          </div>
