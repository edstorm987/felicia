...
export default function ChatBot() {
  ...
  return (
    <>
      ...
      {open && (
        <div className={`fixed ${panelPositionCls} z-40 w-[calc(100vw-2rem)] sm:w-[24rem] max-h-[70vh] bg-pink-50/50 border border-pink-200 rounded-2xl shadow-2xl flex flex-col overflow-hidden`}>
          <div className="px-5 py-4 border-b border-pink-200 flex items-center gap-3">
            <div
            ...
            <div className="flex-1 min-w-0">
              <p className="font-display text-brand-purple-dark text-sm font-semibold">Odo Assistant</p>
              <p className="text-[10px] tracking-widest uppercase text-brand-purple-dark/80 flex items-center gap-1.5">
            ...
            {messages.map((m) => (
              <div key={m.id} className={`flex flex-col ${m.role === "user" ? "items-end" : "items-start"}`}>
                <div
                  ...
                  className={`max-w-[85%] px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed whitespace-pre-line ${m.role === "user" ? `${accent ? "" : "bg-brand-orange"} text-white rounded-br-sm` : "bg-white text-brand-purple-dark/80 border border-pink-200/50 rounded-bl-sm"}`}
                >
                  {m.text}
                </div>
            ...
            <div className="px-4 pb-3 flex flex-wrap gap-2">
              {QUICK_PROMPTS.map((p) => (
                <button
                  key={p}
                  onClick={() => send(p)}
                  className="px-3 py-1.5 text-xs rounded-full border border-pink-200 text-brand-purple-dark/80 hover:border-brand-orange/40 hover:text-brand-purple-dark transition-colors"
                >
                  {p}
                </button>
              ))}
          ...
          <form onSubmit={handleSubmit} className="border-t border-pink-200 px-3 py-3 flex gap-2">
            <input
              value={input}
              onChange={(e) => setInput(e.target.value)}
              placeholder="Ask anything…"
              className="flex-1 min-w-0 bg-white border border-pink-200 rounded-full px-4 py-2.5 text-sm text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-brand-orange/40 transition-colors"
            />
            <button
              type="submit"
              disabled={!input.trim()}
