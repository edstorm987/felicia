...
function ComplianceModePill({ mode }: { mode: ComplianceMode }) {
  if (mode === "none") {
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold tracking-wider uppercase bg-white/8 text-brand-purple-dark/80 border border-pink-200">
        No compliance mode
      </span>
    );
  }
    ...
    <div className="fixed inset-0 z-[400] flex items-end sm:items-center justify-center bg-white/80 backdrop-blur-sm p-0 sm:p-4">
      <div className="w-full sm:max-w-lg bg-pink-50/50 border border-pink-200 rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden max-h-[92vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-pink-200/50 shrink-0">
          ...
          <button onClick={onClose} className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-white/10 text-brand-purple-dark/80 hover:text-brand-purple-dark text-lg shrink-0">✕</button>
        </div>
          ...
          {(["cookies", "data"] as const).map(t => (
            <button
              ...
              className={`flex-1 py-2.5 text-sm font-medium transition-colors border-b-2 -mb-px ${
                tab === t ? "border-brand-orange text-brand-purple-dark" : "border-transparent text-brand-purple-dark/80 hover:text-brand-purple-dark/80"
              }`}
        ...
        <div className="overflow-y-auto flex-1 px-5 py-5 space-y-4">
          {tab === "cookies" && (
            <>
              ...
              <div className="rounded-xl border border-pink-200/50 p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-brand-purple-dark/80 mt-0.5">Required for the site to function. Cannot be disabled.</p>
                  </div>
                  <div
                    ...
                    <div className="flex items-start gap-3">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-brand-purple-dark flex items-center gap-1.5">
                          ...
                          {disabled && (
                            <span className="text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded bg-white/8 text-brand-purple-dark/80 border border-pink-200">
                              Always on
                            </span>
                          )}
                        </p>
                        <p className="text-xs text-brand-purple-dark/80 mt-0.5 leading-relaxed">{cat.description}</p>
                      </div>
                      <button
                        onClick={() => {
                          if (disabled) return;
              ...
              {prefs.updatedAt > 0 && (
                <p className="text-[11px] text-brand-purple-dark/80 text-center">
          ...
          {tab === "data" && (
            <>
              ...
              <div className="rounded-xl border border-red-500/20 p-4 space-y-2">
                <p className="text-sm font-semibold text-brand-purple-dark">Delete my data</p>
                <p className="text-xs text-brand-purple-dark/80 leading-relaxed">
