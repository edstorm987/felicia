      ...
      <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
        <div className="relative w-full max-w-md bg-white border border-brand-purple/20 rounded-3xl overflow-hidden shadow-2xl animate-[float_4s_ease-in-out_infinite]">
          ...
          <div className="relative z-10 p-8 sm:p-10 flex flex-col items-center text-center">
            ...
            <h2 className="font-display text-3xl font-bold text-brand-purple-dark leading-tight mb-3">
              {cfg.headline}
            </h2>
            <p className="text-sm text-brand-purple-dark/80 leading-relaxed mb-8">
              {cfg.subheadline}
            </p>

            {submitted ? (
              <div className="w-full flex flex-col items-center gap-4 py-6">
                <p className="text-brand-purple-dark/80 text-sm">{cfg.successText}</p>
                <div className="w-full py-4 rounded-xl bg-brand-purple/20 border border-brand-purple/30 text-brand-purple-dark font-bold text-2xl tracking-[0.2em]">
                  {cfg.discountCode}
                </div>
                <button
                  onClick={() => {
              ...
              >
                <input
                  ...
                  className="w-full px-5 py-4 rounded-xl bg-pink-50 border border-pink-200 text-brand-purple-dark text-sm focus:outline-none transition-colors"
                ...
                <label className="flex items-start gap-3 text-left cursor-pointer group">
                  ...
                  <span className="text-xs text-brand-purple-dark/80 leading-relaxed select-none group-hover:text-brand-purple-dark/80 transition-colors">
                    {cfg.consentLabel}
                  </span>
                </label>
                <button
            ...
            <button
              onClick={() => setIsOpen(false)}
              className="mt-6 text-[10px] tracking-widest uppercase text-brand-purple-dark/80 hover:text-brand-purple-dark/80 transition-colors"
            >
              {cfg.declineLabel}
            </button>
          </div>
