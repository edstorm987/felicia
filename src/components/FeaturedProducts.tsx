...
export default function FeaturedProducts() {
  ...
  return (
    <section id="featured" className="w-full py-20 sm:py-24 bg-pink-50 relative overflow-hidden">
      {/* top divider */}
      ...
      <div className="w-full max-w-7xl xl:max-w-screen-xl mx-auto px-6 sm:px-10 lg:px-12 xl:px-16">
        ...
        <div className="flex flex-col items-center text-center mb-12 sm:mb-14">
          ...
          <h2 className="font-display font-bold text-brand-purple-dark text-4xl sm:text-5xl xl:text-6xl mb-3">
            {headline}
          </h2>
          <p className="text-brand-purple-dark/80 text-base sm:text-lg max-w-sm mx-auto">
            {tagline}
          </p>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col flex-1 p-6 pt-0 gap-4">
                  <div>
                    <h3 className="font-display font-bold text-brand-purple-dark text-2xl mb-1">{product.name}</h3>
                    <p className="text-brand-purple-dark/80 text-sm">{product.tagline}</p>
                  </div>

                  {/* Bullets */}
                  <ul className="space-y-1.5 flex-1">
                    {product.shortBullets.slice(0, 3).map((bullet, i) => (
                      <li key={i} className="flex items-start gap-2 text-brand-purple-dark/80 text-sm leading-snug">
                    ...
                    <div>
                      {product.salePrice ? (
                        <div className="flex items-baseline gap-2">
                          <span className="font-display font-bold text-brand-purple-dark text-xl">£{product.salePrice.toFixed(2)}</span>
                          <span className="text-brand-purple-dark/80 text-sm line-through">£{product.price.toFixed(2)}</span>
                        </div>
                      ) : (
                        <span className="font-display font-bold text-brand-purple-dark text-xl">from £{product.price.toFixed(2)}</span>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      <Link
                        href={`/products/${product.slug}`}
                        className="text-brand-purple-dark/80 hover:text-brand-purple-dark text-sm transition-colors"
                      >
                        View
                      </Link>
                      {product.available === 0 ? (
                        <span className="px-4 py-2 rounded-xl text-sm font-medium bg-white/10 text-brand-purple-dark/80 cursor-not-allowed">
                          Sold out
                        </span>
                      ) : (
                        <button
                          ...
                          className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                            product.range === "odo"
                              ...
                              ? "bg-brand-amber hover:bg-brand-amber/90 text-brand-purple-dark"
                              : "bg-brand-cream hover:bg-white text-brand-purple-dark"
                          }`}
                        >
                          Add
                        </button>
        ...
        <div className="flex justify-center mt-10">
          <Link
            href="/products"
            className="inline-flex items-center gap-2 text-brand-purple-dark/80 hover:text-brand-purple-dark text-sm transition-colors group"
          >
            Browse all products
