      ...
      <div className="w-full max-w-7xl xl:max-w-screen-xl mx-auto px-6 sm:px-10 lg:px-12 xl:px-16 py-14 sm:py-16 lg:py-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 sm:gap-12 xl:gap-14 mb-14 sm:mb-16">
          ...
          <div className="sm:col-span-2 lg:col-span-1">
            <Link href="/" className="font-display text-2xl sm:text-3xl font-bold text-brand-purple-dark mb-1.5 inline-block">
              LUV <span className="text-brand-orange">&amp;</span> KER
            </Link>
            <div className="text-[10px] tracking-[0.28em] text-gray-400 uppercase mb-5">{tagline}</div>
            <p className="text-sm xl:text-base text-gray-500 leading-relaxed max-w-xs">
              {description}
            </p>
          </div>
          ...
          {/* Support + Newsletter */}
          <div>
            <h4 className="text-xs tracking-[0.25em] uppercase text-gray-500 mb-5">Support</h4>
            ...
            <p className="text-xs text-gray-500 mb-3">Stay connected</p>
            <form
            ...
            >
              <input
                ...
                required
                className="flex-1 min-w-0 bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 text-xs xl:text-sm text-brand-purple-dark placeholder:text-gray-400 focus:outline-none focus:border-brand-orange/40 transition-colors"
              />
              <button
                type="submit"
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-xs xl:text-sm text-gray-400 text-center">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            {SOCIAL_LINKS.map(({ label, href, icon }) => (
              <a
                ...
                aria-label={label}
                className="w-9 h-9 rounded-full border border-gray-200 flex items-center justify-center text-gray-400 hover:text-brand-purple-dark hover:border-brand-orange/50 hover:bg-brand-orange/10 transition-all duration-300"
              >
                {icon}
              </a>
            ))}
