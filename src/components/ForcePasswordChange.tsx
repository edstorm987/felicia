...
export default function ForcePasswordChange() {
  ...
  return (
    ...
    <div
      className="fixed inset-0 z-[300] flex items-center justify-center bg-pink-50/90 backdrop-blur-md p-4"
      onMouseDown={(e) => e.stopPropagation()}
    >
      <div className="w-full max-w-md bg-pink-50/50 border border-brand-orange/30 rounded-2xl shadow-2xl overflow-hidden">
        {/* Header stripe */}
        <div className="bg-brand-orange/10 border-b border-brand-orange/20 px-6 py-5">
          ...
          <h2 className="font-display text-2xl text-brand-purple-dark">
            Welcome, {userName}!
          </h2>
          <p className="text-sm text-brand-purple-dark/80 mt-1">
        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          {done ? (
            <div className="py-6 text-center">
              ...
              <p className="text-brand-purple-dark font-medium">Password updated!</p>
            </div>
          ) : (
            <>
              <div>
                <label className="block text-xs text-brand-purple-dark/80 mb-1.5">New password</label>
                <input
                  type="password"
                  required
                  minLength={8}
                  ...
                  className="w-full bg-white/5 border border-pink-200 rounded-xl px-3 py-2.5 text-sm text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-brand-orange/50"
                />
              </div>
              <div>
                <label className="block text-xs text-brand-purple-dark/80 mb-1.5">Confirm new password</label>
                <input
                  type="password"
                  required
                  minLength={8}
              ...
              {pw.length > 0 && (
                <div className="flex gap-1">
                  ...
                  <span className="text-[10px] text-brand-purple-dark/80 ml-1">
