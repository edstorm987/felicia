    ...
    <>
        ...
        {visible.map((ing) => (
          <li key={ing.slug}>
            <button
              ...
              className="group w-full text-left overflow-hidden rounded-2xl bg-white border border-pink-200/50 hover:border-brand-amber/30 transition-all"
            >
              ...
              <div className="p-5">
                <div className="flex items-center justify-between gap-3 mb-1.5">
                  <h3 className="font-display text-lg font-semibold text-brand-purple-dark group-hover:text-brand-amber transition-colors">
                    {ing.name}
                  </h3>
                  <span className="text-brand-purple-dark/80 group-hover:text-brand-amber transition-colors text-sm">→</span>
                </div>
                <p className="text-sm text-brand-purple-dark/80 leading-relaxed">{ing.oneLiner}</p>
              </div>
            </button>
          </li>
        ))}
      ...
      {showViewAll && (
        <div className="mt-10 flex justify-center">
          <Link
            href="/ingredients"
            className="inline-flex items-center gap-3 px-8 py-4 rounded-full border border-brand-amber/40 bg-white text-brand-amber text-sm font-medium hover:border-brand-amber/70 hover:bg-brand-amber/5 transition-all duration-200 group"
          >
...
function IngredientModal({
  ...
  return (
    ...
    >
      ...
      {/* Panel */}
      <div className="relative z-10 w-full max-w-3xl max-h-[90vh] bg-pink-50/50 border border-pink-200 rounded-3xl shadow-2xl flex flex-col overflow-hidden">
        {/* Header image */}
        <div className="relative aspect-[16/9] sm:aspect-[2/1] shrink-0">
          ...
          <div className="absolute bottom-5 left-5 right-5 sm:left-7 sm:right-7">
            ...
            <h2 className="font-display font-bold text-brand-purple-dark text-2xl sm:text-3xl xl:text-4xl leading-tight">
              {ingredient.name}
            </h2>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 sm:px-7 py-6 sm:py-7 space-y-6">
          <p className="text-brand-purple-dark/80 text-base sm:text-lg italic leading-relaxed">
            {ingredient.oneLiner}
          </p>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {ingredient.benefits.map((b) => (
                <li key={b} className="flex items-start gap-3 text-sm sm:text-base text-brand-purple-dark/80 leading-relaxed">
          ...
          <div>
            ...
            <p className="font-display text-brand-purple-dark font-medium mb-2">{ingredient.partner}</p>
            <p className="text-sm sm:text-base text-brand-purple-dark/80 leading-relaxed">{ingredient.sourcing}</p>
          </div>

          {/* Processing */}
          <div>
            <p className="text-[10px] tracking-[0.28em] uppercase text-brand-purple-light mb-2">How it's processed</p>
            <p className="text-sm sm:text-base text-brand-purple-dark/80 leading-relaxed">{ingredient.processing}</p>
          </div>

          {/* Used in */}
          <div className="pt-2">
            <p className="text-[10px] tracking-[0.28em] uppercase text-brand-purple-dark/80 mb-3">Found in</p>
