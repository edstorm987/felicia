  ...
  return (
    <div>
      <div className="flex items-baseline justify-between mb-2">
        <p className="text-[11px] tracking-[0.18em] uppercase text-brand-purple-dark/80">
          {option.name}
        </p>
        {selectedLabel && <p className="text-[11px] text-brand-purple-dark/80">{selectedLabel}</p>}
      </div>
  ...
  return (
    <div>
      ...
      {wheelOpen && option.allowCustom && (
        <div className="mt-3 p-3 rounded-xl border border-pink-200 bg-white/[0.03]">
          <div className="flex items-center gap-3">
            <input
              type="color"
              value={pendingHex}
              onChange={e => setPendingHex(e.target.value)}
              className="w-14 h-14 rounded-lg cursor-pointer border border-pink-200 bg-transparent"
            />
            <div className="flex-1">
              <input
                ...
                onChange={e => setPendingHex(e.target.value)}
                placeholder="#ff6b35"
                className="w-full bg-white/5 border border-pink-200 rounded-lg px-2.5 py-1.5 text-sm text-brand-purple-dark font-mono focus:outline-none focus:border-brand-orange/50"
              />
              {customSurcharge > 0 && (
      ...
      {option.values.map(value => {
        ...
        return (
          <button
            ...
            onClick={() => onPick(value.id)}
            disabled={value.available === false}
            className={`px-4 py-2 rounded-lg text-[12px] font-semibold transition-colors disabled:opacity-30 disabled:line-through ${active ? "bg-brand-orange text-white" : "bg-white/5 text-brand-purple-dark/80 border border-pink-200 hover:bg-white/10"}`}
          >
            {value.label}
      ...
      {option.values.map(value => {
        ...
        return (
          <button
            ...
            onClick={() => onPick(value.id)}
            disabled={value.available === false}
            className={`text-left p-3 rounded-xl border transition-colors disabled:opacity-30 ${active ? "border-brand-orange/60 bg-brand-orange/10" : "border-pink-200 bg-white/[0.02] hover:bg-white/[0.05]"}`}
          >
            <p className="text-[12px] font-semibold text-brand-purple-dark">{value.label}</p>
        ...
        return (
          ...
          >
            {value.image
              ...
              : <div className="w-full h-full bg-white/[0.04] flex items-center justify-center text-[10px] text-brand-purple-dark/80">{value.label}</div>
            }
          </button>
        );
      })}
