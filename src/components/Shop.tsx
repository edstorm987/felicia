...
export default function Shop() {
  return (
    <section id="shop" className="w-full py-20 sm:py-24 lg:py-32 bg-pink-50 relative overflow-hidden">
      ...
      <div className="w-full max-w-7xl xl:max-w-screen-xl mx-auto px-6 sm:px-10 lg:px-12 xl:px-16">
        ...
        <div className="flex flex-col items-center text-center mb-14 sm:mb-16">
          ...
          <h2 className="font-display font-bold text-brand-purple-dark mb-4 text-4xl sm:text-5xl xl:text-6xl 2xl:text-7xl">
            Shop the full ritual.
          </h2>
          <p className="text-brand-purple-dark/80 text-base sm:text-lg leading-relaxed max-w-md mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 xl:gap-6">
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <h3 className="font-display font-bold text-brand-purple-dark text-3xl sm:text-4xl mb-2">Odo</h3>
              <p className="text-brand-purple-dark/80 text-sm leading-relaxed mb-6">
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <h3 className="font-display font-bold text-brand-purple-dark text-3xl sm:text-4xl mb-2">Nkrabea</h3>
              <p className="text-brand-purple-dark/80 text-sm leading-relaxed mb-6">
          ...
          >
            ...
            <div className="relative z-10">
              <span className="text-[10px] tracking-[0.28em] uppercase text-brand-purple-dark/80 mb-3 block">For Everyone</span>
              <h3 className="font-display font-bold text-brand-purple-dark text-3xl sm:text-4xl mb-2">Black Soap</h3>
              <p className="text-brand-purple-dark/80 text-sm leading-relaxed mb-6">
                The original formula. One soap for face, body, and hands — bar, jar, or sachet.
              </p>
              <span className="inline-flex items-center gap-2 text-brand-purple-dark/80 text-sm font-medium group-hover:text-brand-purple-dark group-hover:gap-3 transition-all">
                Shop Black Soap
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M5 12h14M12 5l7 7-7 7" />
                </svg>
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3">
          <Link
            href="/products"
            className="flex-1 px-6 py-4 rounded-2xl border border-pink-200 bg-white hover:border-white/25 hover:bg-white/[0.02] text-center text-sm text-brand-purple-dark/80 hover:text-brand-purple-dark transition-all"
          >
        ...
        <div className="mt-14 sm:mt-16 grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-5">
          {[
            ...
            <div key={label} className="flex flex-col items-center text-center gap-2.5 p-5 sm:p-6 rounded-xl bg-white border border-pink-200/50">
...
export function ProductCard({ product, isAdded, onAdd }: { product: Product; isAdded: boolean; onAdd: () => void }) {
  ...
  return (
    <div className={`group relative flex flex-col rounded-2xl bg-white border transition-all duration-300 overflow-hidden card-glow ${
      soldOut ? "border-pink-200/50 opacity-70" : "border-pink-200/50 hover:border-brand-orange/20"
    }`}>
      ...
      <Link href={`/products/${product.id}`} className="relative h-52 sm:h-56 xl:h-60 2xl:h-72 bg-gradient-to-br from-brand-purple-muted via-brand-black-card to-brand-purple-dark flex items-center justify-center overflow-hidden">
        {product.image ? (
          ...
          <>
            ...
            <div className="relative z-10 w-32 h-20 rounded-2xl bg-gradient-to-br from-brand-orange/20 to-brand-purple/30 border border-pink-200 flex flex-col items-center justify-center shadow-2xl group-hover:scale-105 transition-transform duration-500">
              <p className="font-display text-brand-purple-dark text-xs font-bold tracking-widest">ODO</p>
              <div className="w-8 h-px bg-brand-amber/50 my-1.5" />
              <p className="text-brand-purple-dark/80 text-[8px] tracking-widest uppercase">by Felicia</p>
            </div>
          </>
        )}
      </Link>
      ...
      <div className="flex flex-col flex-1 p-6 xl:p-7">
        <div className="flex items-start justify-between gap-3 mb-1.5">
          <Link href={`/products/${product.id}`} className="font-display text-xl sm:text-2xl font-bold text-brand-purple-dark hover:text-brand-orange transition-colors">
            {product.name}
          </Link>
          <div className="shrink-0 text-right">
            {product.onSale && product.salePrice ? (
              <>
                ...
                <span className="block text-xs text-brand-purple-dark/80 line-through">£{product.price.toFixed(2)}</span>
              </>
            ) : (
        ...
        <p className="text-[10px] tracking-[0.18em] uppercase text-brand-purple-dark/80 mb-4">{product.tagline}</p>
        <p className="text-sm xl:text-base text-brand-purple-dark/80 leading-relaxed flex-1 mb-4">{product.description[0]}</p>
        {product.note && <p className="text-xs sm:text-sm italic text-brand-amber/60 mb-5">✦ {product.note}</p>}
        <Link
          href={`/products/${product.id}`}
          className="w-full py-3 mb-2.5 rounded-xl text-center text-sm font-medium text-brand-purple-dark/80 border border-pink-200 hover:border-brand-cream/30 hover:text-brand-purple-dark transition-colors"
        >
          View details
        </Link>
        <button
          ...
          className={`w-full py-4 rounded-xl font-semibold text-sm sm:text-base tracking-wide transition-all duration-300 ${
            soldOut
              ? "bg-white/10 text-brand-purple-dark/80 cursor-not-allowed"
              : isAdded
