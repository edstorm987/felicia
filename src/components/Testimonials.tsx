...
export default function Testimonials() {
  ...
  return (
    <section id="verified-reviews" className="w-full py-20 sm:py-24 lg:py-32 2xl:py-40 bg-white overflow-hidden scroll-mt-24 lg:scroll-mt-32">
      <div className="w-full max-w-7xl xl:max-w-screen-xl mx-auto px-6 sm:px-10 lg:px-12 xl:px-16">
        ...
        <div ref={statsReveal.ref} style={revealStyle(statsReveal.visible, 150)} className="grid grid-cols-2 lg:grid-cols-4 gap-px overflow-hidden rounded-2xl bg-gray-200 mb-14 sm:mb-16">
          {[
            ...
            <div key={s.small} className="bg-gray-50 px-5 py-6 sm:py-7 flex flex-col items-center text-center">
              <span className="font-display text-3xl sm:text-4xl xl:text-5xl font-bold text-brand-orange leading-none mb-2">
                {s.big}
              </span>
              <span className="text-[11px] sm:text-xs tracking-wide text-gray-500">{s.small}</span>
            </div>
          ))}
        </div>
...
function ReviewCard({ review }: { review: Review }) {
  ...
  return (
    <div className="shrink-0 w-[85vw] sm:w-[26rem] xl:w-[28rem] flex flex-col p-7 xl:p-8 rounded-2xl bg-gray-50 border border-gray-200 hover:border-brand-purple/30 transition-colors">
      <div className="flex gap-1 mb-5">
        {Array.from({ length: stars }).map((_, i) => (
          <span key={i} className="text-brand-orange text-lg">★</span>
        ))}
      </div>
      <p className="text-brand-purple-dark/80 text-sm xl:text-base leading-relaxed flex-1 mb-6">
        &ldquo;{quote}&rdquo;
      </p>
      <div className="flex items-center gap-3 pt-5 border-t border-gray-200">
        ...
        <div className="min-w-0">
          <p className="text-sm xl:text-base font-medium text-brand-purple-dark truncate">{name}</p>
          <p className="text-xs xl:text-sm text-gray-500 truncate">{location}</p>
        </div>
      </div>
    </div>
  );
...
function StoryTile({ tile }: { tile: IgStory }) {
  return (
    <div className="h-full rounded-2xl overflow-hidden bg-pink-50 border-[3px] border-white shadow-2xl shadow-black/50 flex flex-col">
  ...
  return (
    <div className="h-full rounded-2xl bg-white shadow-2xl shadow-black/50 overflow-hidden flex flex-col">
      <div className="px-3 py-2.5 border-b border-gray-200 flex items-center gap-2 shrink-0">
        ...
        <span className="text-xs text-brand-purple-dark font-semibold truncate flex-1">{tile.handle}</span>
      ...
      <div className="flex-1 px-3 pt-3 pb-4 bg-white flex flex-col justify-end">
        <div className="bg-gray-100 rounded-2xl rounded-tl-sm px-3.5 py-2.5">
          <p className="text-[11px] sm:text-xs text-brand-purple-dark/90 leading-relaxed">{tile.body}</p>
        </div>
...
function PhotoTileEl({ tile }: { tile: PhotoTile }) {
  return (
    <div className="relative h-full rounded-xl overflow-hidden bg-white border-[3px] border-white shadow-2xl shadow-black/50">
