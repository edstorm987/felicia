    ...
    >
      ...
      >
        ...
        <div ref={listRef} className="max-h-[60vh] overflow-y-auto">
          {filtered.length === 0 && (
            <div className="px-4 py-12 text-center text-sm text-brand-purple-dark/80">
              No matches.
            </div>
          )}
          {grouped.map(([group, items]) => (
            <div key={group}>
              <p className="px-4 pt-3 pb-1.5 text-[10px] uppercase tracking-[0.2em] text-brand-purple-dark/80">{group}</p>
              {items.map(item => {
                runningIdx++;
                ...
                return (
                  ...
                  >
                    <span className="text-brand-purple-dark/80 w-5 text-center shrink-0">{item.icon}</span>
                    <span className="text-sm text-brand-purple-dark truncate">{item.label}</span>
                    {item.hint && (
