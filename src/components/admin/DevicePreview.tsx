...
export default function DevicePreview({ state, onChange }: Props) {
  ...
  return (
    <div className="flex items-center gap-2 px-2.5 py-1.5 bg-white/[0.02] border-b border-pink-200/50 text-[11px] text-brand-purple-dark/80">
      {/* Device dropdown — categorised */}
      <div className="relative">
        <select
          value={state.deviceId}
          onChange={e => update({ deviceId: e.target.value })}
          className="appearance-none bg-white/5 hover:bg-white/10 border border-pink-200 rounded-md pl-2.5 pr-7 py-1 text-[11px] text-brand-purple-dark cursor-pointer focus:outline-none focus:border-cyan-400/40"
        ...
        <span className="pointer-events-none absolute right-1.5 top-1/2 -translate-y-1/2 text-brand-purple-dark/80 text-[10px]">▾</span>
      </div>

      {/* W × H readout / custom inputs in Responsive mode */}
      {spec.id === "responsive" ? (
        <div className="flex items-center gap-1 text-brand-purple-dark/80">
          <input
            type="number"
            ...
            className="w-16 bg-white/5 border border-pink-200 rounded-md px-1.5 py-0.5 text-[11px] text-brand-purple-dark text-center focus:outline-none focus:border-cyan-400/40"
        ...
        <span className="text-brand-purple-dark/80 font-mono">{viewport.width} × {viewport.height}</span>
      )}
      ...
      {spec.id !== "responsive" && (
        <button
          ...
          className={`px-2 py-1 rounded-md transition-colors ${state.rotated ? "bg-cyan-400/15 text-cyan-200" : "bg-white/5 hover:bg-white/10 text-brand-purple-dark/80"}`}
      ...
      <div className="flex items-center gap-1">
        ...
        <select
          ...
          className="appearance-none bg-white/5 hover:bg-white/10 border border-pink-200 rounded-md px-2 py-0.5 text-[11px] cursor-pointer focus:outline-none focus:border-cyan-400/40"
          title="Zoom level"
        >
          {ZOOM_STEPS.map(z => (
      ...
      {(spec.category === "phone" || spec.category === "tablet") && spec.bezel && (
        <>
          ...
          <button
            ...
            className={`px-2 py-1 rounded-md transition-colors ${state.showChrome ? "bg-cyan-400/15 text-cyan-200" : "bg-white/5 hover:bg-white/10 text-brand-purple-dark/80"}`}
      ...
      <span className="ml-auto text-brand-purple-dark/80 text-[10px] hidden sm:inline">
