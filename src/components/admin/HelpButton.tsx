  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-20 right-5 z-30 w-10 h-10 rounded-full bg-brand-amber/85 hover:bg-brand-amber text-brand-purple-dark font-bold text-lg shadow-lg shadow-brand-amber/20 hover:shadow-brand-amber/40 transition-all hover:scale-105"
      >
        ?
      </button>
          ...
          >
            <div className="sticky top-0 flex items-center justify-between gap-3 px-5 py-3 bg-pink-50/50 border-b border-pink-200/50">
              ...
              <button
                ...
                className="text-brand-purple-dark/80 hover:text-brand-purple-dark w-8 h-8 flex items-center justify-center rounded hover:bg-white/5"
              >
                ×
              </button>
            </div>
            ...
            <div className="px-5 py-5 space-y-5">
              {doc ? (
                <>
                  {doc.intro && (
                    <p className="text-[13px] text-brand-purple-dark/80 leading-relaxed">{doc.intro}</p>
                  )}
                  {doc.sections.map((s, i) => (
                    <section key={i} className="space-y-2">
                      <h3 className="text-[11px] uppercase tracking-[0.22em] text-brand-purple-dark/80">{s.heading}</h3>
                      <p className="text-[12px] text-brand-purple-dark/80 leading-relaxed">{s.body}</p>
                      {s.bullets && (
                        <ul className="list-disc list-outside ml-5 space-y-1 text-[12px] text-brand-purple-dark/80 leading-relaxed">
              ...
              <section className="space-y-2">
                ...
                <ul className="list-disc list-outside ml-5 space-y-1 text-[12px] text-brand-purple-dark/80">
                ...
                <p className="text-[10px] text-brand-purple-dark/80 mt-2">
                  Tip: hit <kbd className="px-1.5 py-0.5 rounded bg-white/8 text-brand-purple-dark/80 text-[10px] font-mono">?</kbd> on any admin page to open this drawer.
                </p>
              </section>
            </div>
          </aside>
...
function FallbackHelp({ pathname }: { pathname: string | null }) {
  return (
    <>
      ...
      {pathname && (
        <p className="text-[10px] text-brand-purple-dark/80 font-mono">{pathname}</p>
      )}
    </>
  );
}
  ...
  return (
    <section className="space-y-3">
      <div className="flex items-baseline justify-between gap-2">
        ...
        <span className="text-[9px] uppercase tracking-[0.18em] text-brand-purple-dark/80">AI · grounded in these docs</span>
      </div>
      ...
      {keyMissing ? (
        ...
        <form onSubmit={ask} className="space-y-2">
          <textarea
            ...
            rows={2}
            className="w-full bg-white/5 border border-pink-200 rounded-xl px-3 py-2 text-[12px] text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-brand-amber/50 resize-none"
          />
          <div className="flex items-center justify-between gap-2">
            <span className="text-[9px] text-brand-purple-dark/80">⌘+↵ to send</span>
            <button
              type="submit"
              disabled={!question.trim() || busy}
              className="text-[11px] uppercase tracking-[0.2em] text-brand-purple-dark bg-brand-amber hover:bg-brand-amber/90 rounded-lg px-3 py-1.5 disabled:opacity-40 disabled:cursor-not-allowed"
            >
              {busy ? "Thinking…" : "Ask"}
            </button>
          </div>
        ...
        <div className="space-y-3">
          {history.map((turn, i) => (
            <div key={i} className="space-y-1.5">
              <div className="text-[11px] text-brand-purple-dark/80 italic">{turn.question}</div>
              {turn.loading ? (
                <div className="text-[12px] text-brand-purple-dark/80">
                ...
                <div className="text-[12px] text-brand-purple-dark/80 leading-relaxed whitespace-pre-wrap">
                  {turn.answer}
                </div>
              )}
            </div>
