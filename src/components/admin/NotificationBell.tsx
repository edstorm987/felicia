    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 rounded-xl border border-cyan-400/15 bg-[#0a0e1a] shadow-2xl z-30 overflow-hidden">
          <header className="px-4 py-2 border-b border-pink-200/50 flex items-center justify-between">
          ...
          {list.length === 0 ? (
            <p className="px-4 py-6 text-[12px] text-brand-purple-dark/80 text-center">Nothing yet.</p>
          ) : (
            <ul className="max-h-80 overflow-y-auto">
              {list.map(n => (
                <li key={n.id} className={`px-3 py-2 border-b border-pink-200/50 last:border-0 ${n.read ? "opacity-60" : ""}`}>
                  {n.link ? (
...
function NotificationContent({ n }: { n: { title: string; body: string; createdAt: number } }) {
  return (
    <>
      <p className="text-[12px] text-brand-purple-dark truncate">{n.title}</p>
      <p className="text-[10px] text-brand-purple-dark/80 truncate">{n.body}</p>
      <p className="text-[9px] text-brand-purple-dark/80 mt-0.5 tabular-nums">{new Date(n.createdAt).toLocaleString()}</p>
    </>
  );
}
