    ...
    <div className="relative">
      ...
      >
        ...
        <span className="flex-1 min-w-0">
          <span className="text-[10px] tracking-[0.18em] uppercase text-brand-purple-dark/80 block">Org</span>
          <span className="text-xs font-semibold text-brand-purple-dark truncate block">{active.name}</span>
        </span>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={`text-brand-purple-dark/80 shrink-0 transition-transform ${open ? "rotate-180" : ""}`}>
          <polyline points="6 9 12 15 18 9" />
        </svg>
      </button>

      {open && (
        <div className="absolute left-0 right-0 mt-1.5 z-30 rounded-xl border border-pink-200 bg-pink-50/50 shadow-2xl overflow-hidden">
          {orgs.map(o => {
            ...
            return (
              ...
              >
                ...
                <span className="flex-1 min-w-0">
                  <span className="text-xs font-medium text-brand-purple-dark truncate block">{o.name}</span>
          ...
          <a href="/admin/orgs" className="block px-3 py-2 text-[11px] text-brand-purple-dark/80 hover:bg-white/5 border-t border-pink-200/50 text-center">
            Manage orgs →
          </a>
        </div>
      )}
