...
export default function PluginMarketplace({ orgId, configureHrefForPlugin, onChange }: Props) {
  ...
  if (loading) {
    return <p className="text-[12px] text-brand-purple-dark/80">Loading marketplace…</p>;
  }
    ...
    <div className="space-y-8">
      ...
      {Object.entries(byCategory).map(([category, list]) => (
        <section key={category} className="space-y-3">
          <h2 className="text-[10px] tracking-[0.28em] uppercase text-brand-purple-dark/80">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {list.map(plugin => {
              ...
              return (
                <article
                  key={plugin.id}
                  className="rounded-xl border border-pink-200/50 bg-white/[0.02] p-4 flex flex-col gap-3"
                  style={{ minHeight: 200 }}
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <h3 className="font-display text-base text-brand-purple-dark truncate">{plugin.name}</h3>
                        {plugin.core && (
                      ...
                      <p className="text-[11px] text-brand-purple-dark/80 leading-relaxed">{plugin.tagline}</p>
                    </div>
                  </div>

                  {plugin.requires.length > 0 && (
                    <p className="text-[10px] text-brand-purple-dark/80">
                      Requires: <span className="text-brand-purple-dark/80">{plugin.requires.join(", ")}</span>
                    </p>
                  )}
                      ...
                      <>
                        <span
                          className={`text-[10px] tracking-wider uppercase px-2 py-1 rounded ${
                            install.enabled ? "bg-emerald-500/10 text-emerald-300" : "bg-white/5 text-brand-purple-dark/80"
                          }`}
                        >
                        ...
                        <Link
                          ...
                          className="px-2.5 py-1 rounded-md text-[11px] bg-white/5 hover:bg-white/10 text-brand-purple-dark/80 transition-colors"
                        >
                          Configure
                        </Link>
                        {!plugin.core && (
                          <>
                            <button
                              ...
                              disabled={isBusy}
                              className="px-2.5 py-1 rounded-md text-[11px] bg-white/5 hover:bg-white/10 text-brand-purple-dark/80 transition-colors disabled:opacity-40"
                            >
