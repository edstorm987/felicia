...
export default function PluginRequired({ plugin, feature, children }: Props) {
  ...
  if (!hydrated) {
    return <div className="px-6 py-10 text-[12px] text-brand-purple-dark/80">Loading…</div>;
  }
  ...
  if (!install || !install.enabled) {
    return (
      <main className="max-w-2xl mx-auto px-6 py-16 text-center space-y-4">
        ...
        <h1 className="font-display text-2xl text-brand-purple-dark">{pluginName} isn&apos;t set up for this portal yet.</h1>
        <p className="text-[13px] text-brand-purple-dark/80 max-w-md mx-auto leading-relaxed">
          This page belongs to the <span className="text-brand-purple-dark/80">{pluginName}</span> plugin.
        ...
        <div className="flex items-center justify-center gap-3 pt-2">
          ...
          <Link
            href="/aqua"
            className="text-[12px] text-brand-purple-dark/80 hover:text-brand-purple-dark transition-colors"
          >
            Back to portals
          </Link>
        </div>
  ...
  if (feature && install.features?.[feature] !== true) {
    return (
      <main className="max-w-2xl mx-auto px-6 py-16 text-center space-y-4">
        ...
        <h1 className="font-display text-2xl text-brand-purple-dark">This feature is turned off for this portal.</h1>
        <p className="text-[13px] text-brand-purple-dark/80 max-w-md mx-auto">
          The <span className="text-brand-purple-dark/80">{feature}</span> feature is part of the {pluginName} plugin
