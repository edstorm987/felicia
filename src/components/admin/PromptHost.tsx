...
export default function PromptHost() {
  ...
  return (
    <div
      className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-white/80 backdrop-blur-sm"
      onClick={cancel}
    >
      <div
        ...
        className="w-full max-w-md rounded-2xl border border-pink-200 bg-[#0a0e1a] p-5 space-y-4 shadow-2xl shadow-black/40"
      >
        <div>
          <h2 id="prompt-title" className="font-display text-base text-brand-purple-dark">{title}</h2>
          {message && <p className="text-[12px] text-brand-purple-dark/80 mt-1 leading-relaxed">{message}</p>}
        </div>
        ...
        {multiline ? (
          <textarea
            ...
            rows={4}
            className="w-full bg-white/5 border border-pink-200 rounded-md px-3 py-2 text-[13px] text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-cyan-400/40"
          />
        ) : (
          <input
            ref={r => { inputRef.current = r; }}
        ...
        <div className="flex items-center justify-end gap-2">
          <button
            onClick={cancel}
            className="text-[12px] text-brand-purple-dark/80 hover:text-brand-purple-dark px-3 py-1.5"
          >
            {cancelLabel ?? "Cancel"}
          </button>
          <button
