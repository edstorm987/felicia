...
export default function RichEditor({ value, onChange, placeholder, minHeight = 320 }: Props) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap gap-1 p-1.5 rounded-lg bg-pink-50 border border-pink-200">
        ...
        <Sep />
        <label className={`px-2.5 py-1 text-[11px] rounded-md border cursor-pointer ${uploading ? "border-pink-200 text-brand-purple-dark/80" : "border-brand-orange/30 text-brand-orange hover:bg-brand-orange/10"}`}>
      ...
      <div
        ...
        className="w-full bg-pink-50 border border-pink-200 rounded-lg px-4 py-3 text-sm text-brand-purple-dark/80 leading-relaxed focus:outline-none focus:border-brand-orange/40 prose-editor"
        style={{ minHeight }}
      />
      <style jsx>{`
...
function Btn({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      ...
      onClick={onClick}
      className="px-2.5 py-1 text-[11px] rounded-md text-brand-purple-dark/80 hover:bg-white/5 hover:text-brand-purple-dark transition-colors"
    >
      {children}
    </button>
  );
