    ...
    <section className="rounded-2xl border border-brand-amber/25 bg-gradient-to-br from-brand-amber/8 via-orange-500/5 to-transparent p-5 sm:p-6 relative">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3 mb-4">
        <div>
          ...
          <h2 className="font-display text-xl sm:text-2xl text-brand-purple-dark">
          ...
          >
            <span
              className={`shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${
                item.done
                  ...
                  : "border border-brand-cream/25 text-brand-purple-dark/80"
              }`}
              aria-hidden
            >
              {item.done ? "✓" : ""}
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[13px] text-brand-purple-dark">
                {item.label}
                {item.doneHint && (
                  <span className="ml-2 text-[11px] text-brand-purple-dark/80">— {item.doneHint}</span>
                )}
              </div>
              {!item.done && (
                <div className="text-[11px] text-brand-purple-dark/80">{item.hint}</div>
              )}
            </div>
            {!item.done && (
              <Link
      ...
      <p className="text-[10px] text-brand-purple-dark/80 mt-3">
