      ...
      <div className={`rounded-2xl border ${accent.ring} ${accent.bg} p-6 sm:p-8 space-y-5`}>
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h1 className="font-display text-xl sm:text-2xl text-brand-purple-dark">{title}</h1>
            {message && (
              <p className="text-[13px] text-brand-purple-dark/80 mt-2 leading-relaxed">{message}</p>
            )}
          </div>
        </div>
        ...
        {steps && steps.length > 0 && (
          <ol className="space-y-1.5 pl-2">
            {steps.map((step, i) => (
              <li key={i} className="flex items-start gap-2 text-[12px] text-brand-purple-dark/80">
                <span className="w-5 h-5 rounded-full bg-white/5 border border-pink-200 text-brand-purple-dark/80 flex items-center justify-center text-[10px] tabular-nums shrink-0 mt-0.5">
                  {i + 1}
                </span>
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            {secondaryCta && (
              <Link
                ...
                className="px-3 py-2 rounded-md text-[12px] text-brand-purple-dark/80 hover:text-brand-purple-dark"
              >
                {secondaryCta.label}
              </Link>
            )}
