    ...
    <div className="relative" data-site-switcher>
      ...
      >
        <span className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-xs text-brand-purple-dark truncate">{active.name}</span>
        </span>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
          className={`text-brand-purple-dark/80 transition-transform shrink-0 ${open ? "rotate-180" : ""}`}>
          <polyline points="6 9 12 15 18 9" />
        </svg>
      </button>

      {open && (
        <div className="absolute top-full left-0 right-0 mt-1 z-30 rounded-lg bg-pink-50 border border-pink-200 shadow-xl shadow-black/50 overflow-hidden">
            ...
            {sites.map(s => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-brand-purple-dark truncate">{s.name}</p>
                  {s.primaryDomain && (
                    <p className="text-[10px] text-brand-purple-dark/80 truncate font-mono">{s.primaryDomain}</p>
                  )}
                </div>
                {s.id === activeId && (
