  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        {toast.title && <p className="text-[12px] font-semibold text-brand-purple-dark mb-0.5">{toast.title}</p>}
        <p className="text-[12px] text-brand-purple-dark/80 leading-relaxed whitespace-pre-line">{toast.message}</p>
      </div>
      <button
        onClick={onDismiss}
        aria-label="Dismiss"
        className="text-brand-purple-dark/80 hover:text-brand-purple-dark text-[14px] leading-none -mt-0.5"
      >
        ×
      </button>
    </div>
