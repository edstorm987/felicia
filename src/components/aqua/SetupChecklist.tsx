    ...
    <section className="rounded-2xl border border-pink-200 bg-white/[0.02] p-5">
      <header className="flex items-baseline justify-between gap-4 mb-3">
        <div>
          ...
          <h2 className="font-display text-2xl text-brand-purple-dark">End-to-end flow</h2>
        </div>
        <span className="text-[12px] text-brand-purple-dark/80">{okCount} / {total} done</span>
      </header>
      <ol className="space-y-1.5">
        {items.map((item, i) => (
          <li key={item.id} className="flex items-start gap-3 p-3 rounded-xl border border-pink-200/50 bg-white/[0.01] hover:bg-white/[0.03]">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-brand-purple-dark">{item.label}</p>
              <p className="text-[12px] text-brand-purple-dark/80 leading-relaxed">{item.description}</p>
              {item.detail && <p className="text-[10px] text-brand-purple-dark/80 font-mono mt-0.5">{item.detail}</p>}
            </div>
            <Link
              href={item.href}
              className={`shrink-0 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold transition-colors ${
                item.status === "ok" ? "border border-pink-200 text-brand-purple-dark/80 hover:bg-white/5"
