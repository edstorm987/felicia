    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/80 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-2xl border border-cyan-400/20 bg-[#0a0e1a] shadow-2xl overflow-hidden">
        <header className="px-5 py-4 border-b border-pink-200/50 flex items-center justify-between">
          ...
          <button
            onClick={onCancel}
            className="text-brand-purple-dark/80 hover:text-brand-purple-dark text-lg leading-none"
        ...
        <div className="p-5 space-y-4">
          <div>
            <h2 className="font-display text-xl text-brand-purple-dark mb-1">{step.title}</h2>
            <p className="text-[12px] text-brand-purple-dark/80 leading-relaxed">{step.description}</p>
          </div>

          <div className="space-y-3">
            {step.fields.map(field => (
              <div key={field.id} className="space-y-1">
                <label className="text-[11px] text-brand-purple-dark/80 font-medium block">
                  {field.label}
                  {field.required && <span className="text-cyan-400 ml-1">*</span>}
                </label>
                {field.helpText && (
                  <p className="text-[10px] text-brand-purple-dark/80">{field.helpText}</p>
                )}
                {field.type === "select" ? (
                  <select
        ...
        <footer className="px-5 py-4 border-t border-pink-200/50 flex items-center justify-between gap-2">
          <div>
            {stepIdx > 0 && (
              <button
                type="button"
                onClick={back}
                disabled={submitting}
                className="text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark transition-colors disabled:opacity-40"
              >
                ← Back
              </button>
            )}
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              onClick={onCancel}
              disabled={submitting}
              className="px-3 py-1.5 rounded-md text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark transition-colors disabled:opacity-40"
            >
              Cancel
            </button>
            <button
...
const INPUT_CLASS = "w-full bg-white/5 border border-pink-200 rounded-md px-3 py-2 text-[12px] text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-cyan-400/40";
