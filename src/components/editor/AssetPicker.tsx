    ...
    <div className="space-y-2">
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 bg-white/5 border border-pink-200 rounded-lg px-2.5 py-1.5 text-[12px] text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-brand-orange/50 font-mono"
        />
        <button
          type="button"
          onClick={() => setOpen(o => !o)}
          className="px-2.5 py-1.5 rounded-lg border border-pink-200 bg-white/5 text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark hover:bg-white/10"
        >
          {open ? "Close" : "Library"}
        </button>
        <button
      ...
      {value && (
        ...
        <img src={value} alt="" className="w-full h-32 object-cover rounded-lg bg-white/[0.04] border border-pink-200" />
      )}

      {/* Inline library */}
      {open && (
        <div className="rounded-lg border border-pink-200 bg-pink-50 p-2 max-h-64 overflow-y-auto">
          {assets.length === 0 ? (
            <p className="text-[11px] text-brand-purple-dark/80 p-3 text-center">No assets yet — click Upload above or visit <code className="font-mono">/admin/assets</code>.</p>
          ) : (
              ...
              {assets.map(asset => (
                ...
                >
                  {asset.contentType.startsWith("image/")
                    ...
                    : <span className="text-[9px] text-brand-purple-dark/80">{asset.filename}</span>
                  }
                </button>
              ))}
            </div>
