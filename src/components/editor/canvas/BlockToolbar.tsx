...
export default function BlockToolbar({
  ...
  return (
    <div
      className="absolute z-20 flex items-center gap-0.5 px-1 py-0.5 rounded-lg bg-pink-50 border border-brand-orange shadow-lg"
      style={{ top: -32, right: 0 }}
      onClick={e => e.stopPropagation()}
    >
      <div
        ref={dragHandleRef}
        className="px-1.5 py-1 text-brand-purple-dark/80 hover:text-brand-purple-dark cursor-grab active:cursor-grabbing"
        title="Drag to move"
        aria-label="Drag handle"
      >
        ⋮⋮
      </div>
      <span className="text-[10px] tracking-wider uppercase text-brand-purple-dark/80 px-1">{label}</span>
    ...
    <button
      ...
      className={`px-1.5 py-1 text-[12px] rounded transition-colors ${
        danger
          ...
          : "text-brand-purple-dark/80 hover:text-brand-purple-dark hover:bg-white/5"
