...
export default function Canvas({
  ...
  const frame = (
    <div
      className="mx-auto my-6 transition-all duration-200 bg-pink-50 rounded-lg shadow-2xl shadow-black/40 overflow-hidden"
      style={frameStyle}
    >
      <div
        className="relative"
  ...
  const framed = chrome ? (
    <div
      className="mx-auto my-6 rounded-[36px] bg-white border-4 border-[#2a2a2a] shadow-2xl shadow-black/60"
      style={{
  ...
  return (
    <div className="flex-1 min-w-0 overflow-auto bg-pink-50">
      {framed}
    </div>
  );
}
...
function EmptyState({ onDropType }: { onDropType: (type: BlockType) => void }) {
  return (
    ...
    >
      <p className="font-display text-2xl text-brand-purple-dark mb-2">Start designing</p>
      <p className="text-[12px] text-brand-purple-dark/80 max-w-md mx-auto">
