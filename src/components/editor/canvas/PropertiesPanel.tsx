...
const INPUT = "w-full bg-white/5 border border-pink-200 rounded-lg px-2.5 py-1.5 text-[12px] text-brand-purple-dark placeholder:text-brand-purple-dark/80 focus:outline-none focus:border-brand-orange/50";
    ...
    <aside className="lg:w-72 lg:shrink-0 lg:border-l lg:border-white/8 lg:relative lg:flex lg:flex-col fixed lg:static bottom-0 left-0 right-0 max-h-[55vh] lg:max-h-none border-t lg:border-t-0 border-white/8 bg-pink-50/50 flex flex-col z-30 overflow-y-auto">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        ...
        {tab === "props" && (def?.fields.length ?? 0) === 0 && (
          <p className="text-[11px] text-brand-purple-dark/80 leading-relaxed">No editable props for this block. Use the Styles tab to tweak appearance.</p>
        )}
...
function tabClass(active: boolean) {
  return `flex-1 py-1.5 text-[10px] font-semibold tracking-[0.18em] uppercase ${active ? "text-brand-orange border-b-2 border-brand-orange" : "text-brand-purple-dark/80 hover:text-brand-purple-dark"}`;
}
  ...
  return (
    <div className="space-y-3">
      ...
      {memberGroups.length === 0 && (
        <p className="text-[11px] text-brand-purple-dark/80 leading-relaxed">Block is not in any split-test group yet.</p>
      )}
        ...
        return (
          <div key={group.id} className="rounded-lg border border-pink-200 bg-white/[0.03] overflow-hidden">
            ...
            <div className="p-2 space-y-1.5">
              ...
              {variants.map(v => (
                <div key={v.id} className="rounded border border-brand-orange/20 bg-brand-orange/5 p-2 space-y-1.5">
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => removeVariant(group.id, v.id)} className="text-[10px] text-brand-purple-dark/80 hover:text-red-400">×</button>
                  </div>
                  {/* Per-prop variant overrides — only the most-relevant fields */}
                  {suggestedKeys.length > 0 && (
                    <details className="text-[10px]">
                      <summary className="cursor-pointer text-brand-purple-dark/80 hover:text-brand-purple-dark">Override props ({Object.keys(v.props ?? {}).length})</summary>
                      <div className="mt-1.5 space-y-1.5">
                        {suggestedKeys.map(key => {
                          const cur = (v.props as Record<string, unknown> | undefined)?.[key];
                          return (
                            <div key={key} className="flex items-center gap-2">
                              <span className="text-[10px] text-brand-purple-dark/80 w-16 shrink-0 font-mono truncate">{key}</span>
                              <input
              ...
              <button onClick={() => addVariant(group.id)} className="w-full px-2 py-1 rounded border border-pink-200 text-[10px] text-brand-purple-dark/80 hover:text-brand-purple-dark hover:bg-white/5">
                + Add variant
              </button>
            </div>
          </div>
      ...
      {eligibleGroups.length > 0 && (
        <details className="rounded-lg border border-pink-200/50 bg-white/[0.02] overflow-hidden">
          <summary className="cursor-pointer px-2.5 py-1.5 text-[10px] uppercase tracking-[0.18em] text-brand-purple-dark/80 hover:text-brand-purple-dark">+ Add to existing group ({eligibleGroups.length})</summary>
          <div className="p-2 space-y-1">
            {eligibleGroups.map(g => (
              <button key={g.id} onClick={() => addToGroup(g.id)} className="w-full text-left flex items-center gap-2 px-2 py-1.5 rounded text-[11px] text-brand-purple-dark/80 hover:bg-white/5">
                ...
                <span className="text-[10px] text-brand-purple-dark/80">{g.trafficPercent ?? 100}%</span>
              </button>
            ))}
          </div>
        </details>
      ...
      {!creating ? (
        ...
        <form onSubmit={createInlineGroup} className="rounded-lg border border-cyan-500/30 bg-cyan-500/5 p-2 space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => setCreating(false)} className="px-2 py-1 rounded text-[11px] text-brand-purple-dark/80 hover:text-brand-purple-dark">Cancel</button>
          </div>
        </form>
      )}
      ...
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5, 6].map(lvl => (
          <button
            ...
            className={`flex-1 py-1.5 rounded text-[12px] font-semibold ${value === lvl ? "bg-brand-orange text-white" : "bg-white/5 text-brand-purple-dark/80 hover:bg-white/10"}`}
  ...
  switch (field.type) {
    ...
    case "color":
      return (
        <Field label={field.label} help={field.help}>
          <div className="flex gap-2">
            <input type="color" value={typeof v === "string" && v.startsWith("#") ? v : "#000000"} onChange={e => onChange(e.target.value)} className="w-10 h-8 rounded cursor-pointer bg-transparent border border-pink-200" />
...
function StyleEditor<K extends keyof BlockStyles>({ styles, onChange }: { styles: BlockStyles; onChange: (key: K, value: BlockStyles[K]) => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex border border-pink-200 rounded-lg p-0.5 text-[10px]">
        {(["base", "tablet", "mobile"] as const).map(d => (
          <button
            key={d}
            type="button"
            onClick={() => setDevice(d)}
            className={`flex-1 py-1 rounded ${device === d ? "bg-brand-orange text-white" : "text-brand-purple-dark/80 hover:text-brand-purple-dark"}`}
          >
      ...
      {STYLE_FIELD_GROUPS.map(group => (
        <details key={group.label} className="rounded-lg border border-pink-200/50 bg-white/[0.02] overflow-hidden" open>
          <summary className="cursor-pointer px-2.5 py-1.5 text-[11px] uppercase tracking-[0.18em] text-brand-purple-dark/80 hover:text-brand-purple-dark">{group.label}</summary>
              ...
              return (
                <Field key={String(key)} label={String(key)}>
                  {String(key) === "background" || String(key) === "textColor" ? (
                    <div className="flex gap-2">
                      <input type="color" value={typeof valueRaw === "string" && (valueRaw as string).startsWith("#") ? (valueRaw as string) : "#000000"} onChange={e => setActive(key, e.target.value as BlockStyles[typeof key])} className="w-10 h-8 rounded cursor-pointer bg-transparent border border-pink-200" />
      ...
      {device === "base" && (
        <details className="rounded-lg border border-pink-200/50 bg-white/[0.02] overflow-hidden" open>
          <summary className="cursor-pointer px-2.5 py-1.5 text-[11px] uppercase tracking-[0.18em] text-brand-purple-dark/80 hover:text-brand-purple-dark">Animation</summary>
          <div className="p-2.5 pt-0 space-y-2">
            ...
            <p className="text-[10px] text-brand-purple-dark/80 leading-relaxed">Plays once when the block enters the viewport on the live site. The editor canvas shows the resting state.</p>
          </div>
        </details>
      )}
    </div>
...
function Field({ label, help, children }: { label: string; help?: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="block text-[10px] tracking-[0.18em] uppercase text-brand-purple-dark/80 mb-1">{label}</span>
      {children}
      {help && <span className="block text-[10px] text-brand-purple-dark/80 mt-1">{help}</span>}
    </label>
  );
}
