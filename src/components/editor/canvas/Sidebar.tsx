    ...
    <>
      ...
      <div className="flex border-b border-pink-200/50">
        ...
        <button
          ...
          className={`flex-1 py-2 text-[11px] font-semibold tracking-[0.18em] uppercase ${tab === "layers" ? "text-brand-orange border-b-2 border-brand-orange" : "text-brand-purple-dark/80 hover:text-brand-purple-dark"}`}
        >
          Layers
        </button>
      </div>
  ...
  return (
    <div className="p-3 space-y-4">
      {CATEGORIES.map(cat => {
        ...
        return (
          <div key={cat.id}>
            <p className="text-[10px] tracking-[0.2em] uppercase text-brand-purple-dark/80 mb-1.5 px-1">{cat.label}</p>
...
function LayersTree({ blocks, selectedId, onSelect }: { blocks: Block[]; selectedId: string | null; onSelect: (id: string) => void }) {
  if (blocks.length === 0) {
    return <p className="p-4 text-[11px] text-brand-purple-dark/80 leading-relaxed">No blocks yet. Drag one from the Blocks tab onto the canvas.</p>;
  }
  ...
  return (
    <li>
      <button
        ...
        className={`w-full flex items-center gap-2 py-1 pr-2 rounded text-left text-[12px] transition-colors ${active ? "bg-brand-orange/15 text-brand-purple-dark" : "text-brand-purple-dark/80 hover:bg-white/5 hover:text-brand-purple-dark"}`}
